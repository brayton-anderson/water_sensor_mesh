    ...
    if (!skipPreflight) {
        ...
        while (!doPreflightSleep()) {
            ...
            if (!Throttle::isWithinTimespanMs(now,
                ...
                RECORD_CRITICALERROR(water_sensor_mesh_CriticalErrorCode_SLEEP_ENTER_WAIT);
...
void doDeepSleep(uint32_t msecToWake, bool skipPreflight = false, bool skipSaveNodeDb = false)
{
    ...
    powerMon->setState(water_sensor_mesh_PowerMon_State_CPU_DeepSleep);
    if (screen)
...
#if !WATER_SENSOR_MESH_EXCLUDE_I2C && defined(ARCH_ESP32) && defined(I2C_SDA)
    // Added by https://github.com/water_sensor_mesh/firmware/pull/4418
...
{
    return msecToWake < portMAX_DELAY && (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER ||
