...
size_t RedirectablePrint::vprintf(const char *logLevel, const char *format, va_list arg)
{
    ...
    if (color && logLevel != nullptr) {
        if (strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_DEBUG) == 0)
            Print::write("\u001b[34m", 5);
        if (strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_INFO) == 0)
            Print::write("\u001b[32m", 5);
        if (strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_WARN) == 0)
            Print::write("\u001b[33m", 5);
        if (strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_ERROR) == 0)
            Print::write("\u001b[31m", 5);
    }
...
void RedirectablePrint::log_to_serial(const char *logLevel, const char *format, va_list arg)
{
    ...
    if (color) {
        ...
        if (strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_ERROR) == 0)
            Print::write("\u001b[31m", 5);
        if (strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_TRACE) == 0)
            Print::write("\u001b[35m", 5);
    }
...
void RedirectablePrint::log_to_ble(const char *logLevel, const char *format, va_list arg)
{
    ...
    if (config.security.debug_log_api_enabled && !pauseBluetoothLogging) {
        ...
        if (isBleConnected) {
            ...
            auto thread = concurrency::OSThread::currentThread;
            water_sensor_mesh_LogRecord logRecord = water_sensor_mesh_LogRecord_init_zero;
            ...
            uint8_t *buffer = new uint8_t[water_sensor_mesh_LogRecord_size];
            size_t size = pb_encode_to_bytes(buffer, water_sensor_mesh_LogRecord_size, water_sensor_mesh_LogRecord_fields, &logRecord);
#ifdef ARCH_ESP32
...
water_sensor_mesh_LogRecord_Level RedirectablePrint::getLogLevel(const char *logLevel)
{
    water_sensor_mesh_LogRecord_Level ll = water_sensor_mesh_LogRecord_Level_UNSET; // default to unset
    switch (logLevel[0]) {
    case 'D':
        ll = water_sensor_mesh_LogRecord_Level_DEBUG;
        break;
    case 'I':
        ll = water_sensor_mesh_LogRecord_Level_INFO;
        break;
    case 'W':
        ll = water_sensor_mesh_LogRecord_Level_WARNING;
        break;
    case 'E':
        ll = water_sensor_mesh_LogRecord_Level_ERROR;
        break;
    case 'C':
        ll = water_sensor_mesh_LogRecord_Level_CRITICAL;
        break;
    }
    return ll;
}
...
void RedirectablePrint::log(const char *logLevel, const char *format, ...)
{
    ...
    if (strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_TRACE) == 0) {
        ...
        if (settingsMap[logoutputlevel] < level_trace && strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_TRACE) == 0) {
            delete[] newFormat;
            return;
        }
    }
    if (settingsMap[logoutputlevel] < level_debug && strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_DEBUG) == 0) {
        delete[] newFormat;
        return;
    } else if (settingsMap[logoutputlevel] < level_info && strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_INFO) == 0) {
        delete[] newFormat;
        return;
    } else if (settingsMap[logoutputlevel] < level_warn && strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_WARN) == 0) {
        delete[] newFormat;
        return;
    }
#endif
    if (moduleConfig.serial.override_console_serial_port && strcmp(logLevel, WATER_SENSOR_MESH_LOG_LEVEL_DEBUG) == 0) {
        delete[] newFormat;
        return;
    }
