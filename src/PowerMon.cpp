...
void PowerMon::setState(_water_sensor_mesh_PowerMon_State state, const char *reason)
{
#ifdef USE_POWERMON
...
void PowerMon::clearState(_water_sensor_mesh_PowerMon_State state, const char *reason)
{
#ifdef USE_POWERMON
    auto oldstates = states;
    states &= ~state;
