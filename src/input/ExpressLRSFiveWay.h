...
class ExpressLRSFiveWay : public Observable<const InputEvent *>, public concurrency::OSThread
{
  private:
    ...
    // This merged an enum used by the ExpressLRS code, with water_sensor_mesh canned message values
    ...
    // Simplified Water_Sensor_Mesh actions, for easier remapping by user
    void toggleGPS();
    void sendAdhocPing();
    void shutdown();
    void click();
