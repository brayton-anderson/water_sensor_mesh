...
class Channels
{
  ...
  public:
    ...
    const water_sensor_mesh_ChannelSettings &getPrimary() { return getByIndex(getPrimaryIndex()).settings; }

    /** Return the Channel for a specified index */
    water_sensor_mesh_Channel &getByIndex(ChannelIndex chIndex);

    /** Return the Channel for a specified name, return primary if not found. */
    water_sensor_mesh_Channel &getByName(const char *chName);
     ...
     */
    void setChannel(const water_sensor_mesh_Channel &c);
  ...
  private:
     ...
     */
    water_sensor_mesh_Channel &fixupChannel(ChannelIndex chIndex);
