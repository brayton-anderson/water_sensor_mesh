...
const water_sensor_mesh_MeshPacket *MeshModule::currentRequest;
 ...
 */
water_sensor_mesh_MeshPacket *MeshModule::currentReply;
...
water_sensor_mesh_MeshPacket *MeshModule::allocAckNak(water_sensor_mesh_Routing_Error err, NodeNum to, PacketId idFrom, ChannelIndex chIndex,
                                               uint8_t hopLimit)
{
    water_sensor_mesh_Routing c = water_sensor_mesh_Routing_init_default;

    c.error_reason = err;
    c.which_variant = water_sensor_mesh_Routing_error_reason_tag;
    ...
    p->decoded.portnum = water_sensor_mesh_PortNum_ROUTING_APP;
    p->decoded.payload.size =
        pb_encode_to_bytes(p->decoded.payload.bytes, sizeof(p->decoded.payload.bytes), &water_sensor_mesh_Routing_msg, &c);

    p->priority = water_sensor_mesh_MeshPacket_Priority_ACK;

    p->hop_limit = hopLimit; // Flood ACK back to original sender
    p->to = to;
    p->decoded.request_id = idFrom;
    p->channel = chIndex;
    if (err != water_sensor_mesh_Routing_Error_NONE)
        LOG_WARN("Alloc an err=%d,to=0x%x,idFrom=0x%x,id=0x%x", err, to, idFrom, p->id);

    return p;
}

water_sensor_mesh_MeshPacket *MeshModule::allocErrorResponse(water_sensor_mesh_Routing_Error err, const water_sensor_mesh_MeshPacket *p)
{
    // If the original packet couldn't be decoded, use the primary channel
    uint8_t channelIndex =
        p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag ? p->channel : channels.getPrimaryIndex();
...
void MeshModule::callModules(water_sensor_mesh_MeshPacket &mp, RxSource src)
{
    // LOG_DEBUG("In call modules");
    bool moduleFound = false;

    // We now allow **encrypted** packets to pass through the modules
    bool isDecoded = mp.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag;
    ...
    for (auto i = modules->begin(); i != modules->end(); ++i) {
        ...
        if (wantsPacket) {
            ...
            water_sensor_mesh_Channel *ch = isDecoded ? &channels.getByIndex(mp.channel) : NULL;
            ...
            if (!rxChannelOk) {
                ...
                if (isDecoded && mp.decoded.want_response) {
                    ...
                    currentReply = pi.allocErrorResponse(water_sensor_mesh_Routing_Error_NOT_AUTHORIZED, &mp);
                } else
    ...
    if (isDecoded && mp.decoded.want_response && toUs) {
        if (currentReply) {
            ...
            // bad.
            routingModule->sendAckNak(water_sensor_mesh_Routing_Error_NO_RESPONSE, getFrom(&mp), mp.id, mp.channel,
...
water_sensor_mesh_MeshPacket *MeshModule::allocReply()
{
    auto r = myReply;
 ...
 * is optional
 */
void MeshModule::sendResponse(const water_sensor_mesh_MeshPacket &req)
{
    auto r = allocReply();
    if (r) {
        setReplyTo(r, req);
 ...
 */
void setReplyTo(water_sensor_mesh_MeshPacket *p, const water_sensor_mesh_MeshPacket &to)
{
    assert(p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag); // Should already be set by now
    ...
    if (p->priority == water_sensor_mesh_MeshPacket_Priority_UNSET)
...
AdminMessageHandleResult MeshModule::handleAdminMessageForAllModules(const water_sensor_mesh_MeshPacket &mp,
