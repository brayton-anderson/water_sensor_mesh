...
#if !defined(WATER_SENSOR_MESH_EXCLUDE_PKI)
...
extern water_sensor_mesh_DeviceState devicestate;
extern water_sensor_mesh_NodeDatabase nodeDatabase;
extern water_sensor_mesh_ChannelFile channelFile;
extern water_sensor_mesh_MyNodeInfo &myNodeInfo;
extern water_sensor_mesh_LocalConfig config;
extern water_sensor_mesh_DeviceUIConfig uiconfig;
extern water_sensor_mesh_LocalModuleConfig moduleConfig;
extern water_sensor_mesh_User &owner;
extern water_sensor_mesh_Position localPosition;
...
class NodeDB
{
  ...
  public:
    std::vector<water_sensor_mesh_NodeInfoLite> *meshNodes;
    bool updateGUI = false; // we think the gui should definitely be redrawn, screen will clear this once handled
    water_sensor_mesh_NodeInfoLite *updateGUIforNode = NULL; // if currently showing this node, we think you should update the GUI
    Observable<const water_sensor_mesh::NodeStatus *> newStatus;
    ...
    void updateFrom(const water_sensor_mesh_MeshPacket &p);

    void addFromContact(const water_sensor_mesh_SharedContact);

    /** Update position info for this node based on received position data
     */
    void updatePosition(uint32_t nodeId, const water_sensor_mesh_Position &p, RxSource src = RX_SRC_RADIO);

    /** Update telemetry info for this node based on received metrics
     */
    void updateTelemetry(uint32_t nodeId, const water_sensor_mesh_Telemetry &t, RxSource src = RX_SRC_RADIO);

    /** Update user info and channel for this node based on received user data
     */
    bool updateUser(uint32_t nodeId, water_sensor_mesh_User &p, uint8_t channelIndex = 0);
    ...
    */
    ...
    void installRoleDefaults(water_sensor_mesh_Config_DeviceConfig_Role role);

    const water_sensor_mesh_NodeInfoLite *readNextMeshNode(uint32_t &readIndex);

    water_sensor_mesh_NodeInfoLite *getMeshNodeByIndex(size_t x)
    {
        assert(x < numMeshNodes);
        return &meshNodes->at(x);
    }

    virtual water_sensor_mesh_NodeInfoLite *getMeshNode(NodeNum n);
    ...
    {
        water_sensor_mesh_NodeDatabase emptyNodeDatabase;
        emptyNodeDatabase.version = DEVICESTATE_CUR_VER;
        size_t nodeDatabaseSize;
        pb_get_encoded_size(&nodeDatabaseSize, water_sensor_mesh_NodeDatabase_fields, &emptyNodeDatabase);
        return nodeDatabaseSize + (MAX_NUM_NODES * water_sensor_mesh_NodeInfoLite_size);
    }
    ...
    void setLocalPosition(water_sensor_mesh_Position position, bool timeOnly = false)
    {
        if (timeOnly) {
    ...
    bool hasValidPosition(const water_sensor_mesh_NodeInfoLite *n);

    bool checkLowEntropyPublicKey(const water_sensor_mesh_Config_SecurityConfig_public_key_t keyToTest);

    bool backupPreferences(water_sensor_mesh_AdminMessage_BackupLocation location);
    bool restorePreferences(water_sensor_mesh_AdminMessage_BackupLocation location,
    ...
    void notifyObservers(bool forceUpdate = false)
    {
        ...
        const water_sensor_mesh::NodeStatus status = water_sensor_mesh::NodeStatus(getNumOnlineMeshNodes(), getNumMeshNodes(), forceUpdate);
  ...
  private:
    ...
    water_sensor_mesh_NodeInfoLite *getOrCreateMeshNode(NodeNum n);
...
extern water_sensor_mesh_CriticalErrorCode error_code;
