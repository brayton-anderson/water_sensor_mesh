 ...
 *
 * A module allows you to add new features to water_sensor_mesh device code, without needing to know messaging details.
 *
 * A key concept for this is that your module should use a particular "portnum" for each message type you want to receive
 * and handle.
 *
 * Internally we use modules to implement the core water_sensor_mesh text messaging and gps position sharing features.  You
...
class MeshModule
{
  ...
  public:
     ...
     */
    static void callModules(water_sensor_mesh_MeshPacket &mp, RxSource src = RX_SRC_RADIO);

    static std::vector<MeshModule *> GetMeshModulesWithUIFrames(int startIndex);
    static void observeUIEvents(Observer<const UIFrameEvent *> *observer);
    static AdminMessageHandleResult handleAdminMessageForAllModules(const water_sensor_mesh_MeshPacket &mp,
  ...
  protected:
    ...
    */
     ...
     */
    static const water_sensor_mesh_MeshPacket *currentRequest;
     ...
     */
    water_sensor_mesh_MeshPacket *myReply = NULL;
     ...
     */
    virtual bool wantPacket(const water_sensor_mesh_MeshPacket *p) = 0;
    ...
    it
    */
    virtual ProcessMessage handleReceived(const water_sensor_mesh_MeshPacket &mp) { return ProcessMessage::CONTINUE; }
    ...
    */
    virtual void alterReceived(water_sensor_mesh_MeshPacket &mp) {}
     ...
     * */
    virtual water_sensor_mesh_MeshPacket *allocReply();
    ...
    water_sensor_mesh_MeshPacket *allocAckNak(water_sensor_mesh_Routing_Error err, NodeNum to, PacketId idFrom, ChannelIndex chIndex,
                                       uint8_t hopLimit = 0);

    /// Send an error response for the specified packet.
    water_sensor_mesh_MeshPacket *allocErrorResponse(water_sensor_mesh_Routing_Error err, const water_sensor_mesh_MeshPacket *p);
  ...
  private:
     ...
     */
    static water_sensor_mesh_MeshPacket *currentReply;
     ...
     */
    void sendResponse(const water_sensor_mesh_MeshPacket &req);
};
