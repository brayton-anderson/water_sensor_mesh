...
inline uint32_t getPriority(const water_sensor_mesh_MeshPacket *p)
{
    auto pri = p->priority;
    return pri;
}
...
bool CompareMeshPacketFunc(const water_sensor_mesh_MeshPacket *p1, const water_sensor_mesh_MeshPacket *p2)
{
    assert(p1 && p2);
...
void fixPriority(water_sensor_mesh_MeshPacket *p)
{
    ...
    if (p->priority == water_sensor_mesh_MeshPacket_Priority_UNSET) {
        ...
        p->priority = (p->want_ack ? water_sensor_mesh_MeshPacket_Priority_RELIABLE : water_sensor_mesh_MeshPacket_Priority_DEFAULT);
        if (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
            // if acks/naks give very high priority
            if (p->decoded.portnum == water_sensor_mesh_PortNum_ROUTING_APP) {
                p->priority = water_sensor_mesh_MeshPacket_Priority_ACK;
                // if text or admin, give high priority
            } else if (p->decoded.portnum == water_sensor_mesh_PortNum_TEXT_MESSAGE_APP ||
                ...
                p->priority = water_sensor_mesh_MeshPacket_Priority_RESPONSE;
...
bool MeshPacketQueue::enqueue(water_sensor_mesh_MeshPacket *p)
{
...
water_sensor_mesh_MeshPacket *MeshPacketQueue::dequeue()
{
    if (empty()) {
        return NULL;
    }
...
water_sensor_mesh_MeshPacket *MeshPacketQueue::getFront()
{
    if (empty()) {
        return NULL;
    }
...
water_sensor_mesh_MeshPacket *MeshPacketQueue::remove(NodeNum from, PacketId id, bool tx_normal, bool tx_late)
{
 ...
 */
bool MeshPacketQueue::replaceLowerPriorityPacket(water_sensor_mesh_MeshPacket *p)
{
