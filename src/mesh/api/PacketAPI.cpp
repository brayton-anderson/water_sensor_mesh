...
bool PacketAPI::receivePacket(void)
{
    ...
    while (server->hasData()) {
        ...
        water_sensor_mesh_ToRadio *mr;
        auto p = server->receivePacket()->move();
        int id = p->getPacketId();
        LOG_DEBUG("Received packet id=%u", id);
        mr = (water_sensor_mesh_ToRadio *)&static_cast<DataPacket<water_sensor_mesh_ToRadio> *>(p.get())->getData();

        switch (mr->which_payload_variant) {
        case water_sensor_mesh_ToRadio_packet_tag: {
            water_sensor_mesh_MeshPacket *mp = &mr->packet;
        ...
        case water_sensor_mesh_ToRadio_heartbeat_tag:
            ...
            break;
        default:
            LOG_ERROR("Error: unhandled water_sensor_mesh_ToRadio variant: %d", mr->which_payload_variant);
            break;
        }
    }
...
bool PacketAPI::sendPacket(void)
{
    if (server->available()) {
        ...
        if (len != 0) {
            ...
            bool result = server->sendPacket(DataPacket<water_sensor_mesh_FromRadio>(id, fromRadioScratch));
            if (!result) {
...
bool PacketAPI::notifyProgrammingMode(void)
{
    ...
    return server->sendPacket(DataPacket<water_sensor_mesh_FromRadio>(0, fromRadioScratch));
}
