...
static MemoryDynamic<water_sensor_mesh_MeshPacket> staticPool;

Allocator<water_sensor_mesh_MeshPacket> &packetPool = staticPool;
...
{
    water_sensor_mesh_MeshPacket *mp;
 ...
 * freeing the packet
 */
void Router::enqueueReceivedMessage(water_sensor_mesh_MeshPacket *p)
{
    // Try enqueue until successful
    while (!fromRadioQueue.enqueue(p, 0)) {
        water_sensor_mesh_MeshPacket *old_p;
...
water_sensor_mesh_MeshPacket *Router::allocForSending()
{
    water_sensor_mesh_MeshPacket *p = packetPool.allocZeroed();

    p->which_payload_variant = water_sensor_mesh_MeshPacket_decoded_tag; // Assume payload is decoded at start.
 ...
 */
void Router::sendAckNak(water_sensor_mesh_Routing_Error err, NodeNum to, PacketId idFrom, ChannelIndex chIndex, uint8_t hopLimit)
{
    routingModule->sendAckNak(err, to, idFrom, chIndex, hopLimit);
}

void Router::abortSendAndNak(water_sensor_mesh_Routing_Error err, water_sensor_mesh_MeshPacket *p)
{
...
water_sensor_mesh_QueueStatus Router::getQueueStatus()
{
    if (!iface) {
        water_sensor_mesh_QueueStatus qs;
...
ErrorCode Router::sendLocal(water_sensor_mesh_MeshPacket *p, RxSource src)
{
    if (p->to == 0) {
    ...
    if (isToUs(p)) {
        ...
        abortSendAndNak(water_sensor_mesh_Routing_Error_NO_INTERFACE, p);
        ...
        if (!p->channel && !p->pki_encrypted && !isBroadcast(p->to)) {
            water_sensor_mesh_NodeInfoLite const *node = nodeDB->getMeshNode(p->to);
            if (node) {
 ...
 */
ErrorCode Router::rawSend(water_sensor_mesh_MeshPacket *p)
{
 ...
 */
ErrorCode Router::send(water_sensor_mesh_MeshPacket *p)
{
    if (isToUs(p)) {
        LOG_ERROR("BUG! send() called with packet destined for local node!");
        packetPool.release(p);
        return water_sensor_mesh_Routing_Error_BAD_REQUEST;
...
#endif
            water_sensor_mesh_Routing_Error err = water_sensor_mesh_Routing_Error_DUTY_CYCLE_LIMIT;
    ...
    if (!(p->which_payload_variant == water_sensor_mesh_MeshPacket_encrypted_tag ||
          p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag)) {
        return water_sensor_mesh_Routing_Error_BAD_REQUEST;
    }
    ...
    if (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
        ChannelIndex chIndex = p->channel; // keep as a local because we are about to change it
        water_sensor_mesh_MeshPacket *p_decoded = packetPool.allocCopy(*p);

        auto encodeResult = perhapsEncode(p);
        if (encodeResult != water_sensor_mesh_Routing_Error_NONE) {
    ...
    if (udpHandler && config.network.enabled_protocols & water_sensor_mesh_Config_NetworkConfig_ProtocolFlags_UDP_BROADCAST) {
        udpHandler->onSend(const_cast<water_sensor_mesh_MeshPacket *>(p));
    }
#endif
 ...
 */
void Router::sniffReceived(const water_sensor_mesh_MeshPacket *p, const water_sensor_mesh_Routing *c)
{
    // FIXME, update nodedb here for any packet that passes through us
}

DecodeState perhapsDecode(water_sensor_mesh_MeshPacket *p)
{
    concurrency::LockGuard g(cryptLock);

    if (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_REPEATER &&
        config.device.rebroadcast_mode == water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_ALL_SKIP_DECODING)
        return DecodeState::DECODE_FAILURE;

    if (config.device.rebroadcast_mode == water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_KNOWN_ONLY &&
    ...
    if (p->channel == 0 && isToUs(p) && p->to > 0 && !isBroadcast(p->to) && nodeDB->getMeshNode(p->from) != nullptr &&
        ...
        rawSize > WATER_SENSOR_MESH_PKC_OVERHEAD) {
        ...
        if (crypto->decryptCurve25519(p->from, nodeDB->getMeshNode(p->from)->user.public_key, p->id, rawSize, p->encrypted.bytes,
            ...
            water_sensor_mesh_Data decodedtmp;
            memset(&decodedtmp, 0, sizeof(decodedtmp));
            rawSize -= WATER_SENSOR_MESH_PKC_OVERHEAD;
            if (pb_decode_from_bytes(bytes, rawSize, &water_sensor_mesh_Data_msg, &decodedtmp) &&
                decodedtmp.portnum != water_sensor_mesh_PortNum_UNKNOWN_APP) {
                decrypted = true;
                ...
                p->which_payload_variant = water_sensor_mesh_MeshPacket_decoded_tag; // change type to decoded
            } else {
    ...
    if (!decrypted) {
        ...
        for (chIndex = 0; chIndex < channels.getNumChannels(); chIndex++) {
            ...
            if (channels.decryptForHash(chIndex, p->channel)) {
                ...
                } else if (decodedtmp.portnum == water_sensor_mesh_PortNum_UNKNOWN_APP) {
    ...
    if (decrypted) {
        ...
        if (p->decoded.portnum == water_sensor_mesh_PortNum_TEXT_MESSAGE_COMPRESSED_APP) {
            // Decompress the payload
            char compressed_in[water_sensor_mesh_Constants_DATA_PAYLOAD_LEN] = {};
            char decompressed_out[water_sensor_mesh_Constants_DATA_PAYLOAD_LEN] = {};
            ...
            p->decoded.portnum = water_sensor_mesh_PortNum_TEXT_MESSAGE_APP;
        } */
 ...
 */
water_sensor_mesh_Routing_Error perhapsEncode(water_sensor_mesh_MeshPacket *p)
{
    concurrency::LockGuard g(cryptLock);
    ...
    if (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
        ...
        size_t numbytes = pb_encode_to_bytes(bytes, sizeof(bytes), &water_sensor_mesh_Data_msg, &p->decoded);

        /* Not actually used, so save the cycles
        //  TODO: Allow modules to opt into compression.
        if (p->decoded.portnum == water_sensor_mesh_PortNum_TEXT_MESSAGE_APP) {

            char original_payload[water_sensor_mesh_Constants_DATA_PAYLOAD_LEN];
            memcpy(original_payload, p->decoded.payload.bytes, p->decoded.payload.size);

            char compressed_out[water_sensor_mesh_Constants_DATA_PAYLOAD_LEN] = {0};
            ...
            if (compressed_len >= p->decoded.payload.size) {
                ...
                p->decoded.portnum = water_sensor_mesh_PortNum_TEXT_MESSAGE_COMPRESSED_APP;
            }
        } */

        if (numbytes + WATER_SENSOR_MESH_HEADER_LENGTH > MAX_LORA_PAYLOAD_LEN)
            return water_sensor_mesh_Routing_Error_TOO_LARGE;
...
#if !(WATER_SENSOR_MESH_EXCLUDE_PKI)
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(p->to);
        ...
        if (isFromUs(p) &&
            ...
            p->decoded.portnum != water_sensor_mesh_PortNum_TRACEROUTE_APP && p->decoded.portnum != water_sensor_mesh_PortNum_NODEINFO_APP &&
            p->decoded.portnum != water_sensor_mesh_PortNum_ROUTING_APP && p->decoded.portnum != water_sensor_mesh_PortNum_POSITION_APP) {
            LOG_DEBUG("Use PKI!");
            if (numbytes + WATER_SENSOR_MESH_HEADER_LENGTH + WATER_SENSOR_MESH_PKC_OVERHEAD > MAX_LORA_PAYLOAD_LEN)
                return water_sensor_mesh_Routing_Error_TOO_LARGE;
            if (p->pki_encrypted && !memfll(p->public_key.bytes, 0, 32) &&
                memcmp(p->public_key.bytes, node->user.public_key.bytes, 32) != 0) {
                LOG_WARN("Client public key differs from requested: 0x%02x, stored key begins 0x%02x", *p->public_key.bytes,
                         *node->user.public_key.bytes);
                return water_sensor_mesh_Routing_Error_PKI_FAILED;
            }
            crypto->encryptCurve25519(p->to, getFrom(p), node->user.public_key, p->id, numbytes, bytes, p->encrypted.bytes);
            numbytes += WATER_SENSOR_MESH_PKC_OVERHEAD;
            p->channel = 0;
            p->pki_encrypted = true;
            ...
            if (nodeDB->keyIsLowEntropy) {
                ...
                if (!nodeDB->hasWarned) {
                    water_sensor_mesh_ClientNotification *cn = clientNotificationPool.allocZeroed();
                    cn->which_payload_variant = water_sensor_mesh_ClientNotification_low_entropy_key_tag;
                    cn->level = water_sensor_mesh_LogRecord_Level_WARNING;
                    cn->time = getValidTime(RTCQualityFromNet);
            ...
            if (hash < 0) {
                ...
                return water_sensor_mesh_Routing_Error_NO_CHANNEL;
            }
        ...
        p->which_payload_variant = water_sensor_mesh_MeshPacket_encrypted_tag;
    }

    return water_sensor_mesh_Routing_Error_NONE;
}
 ...
 */
void Router::handleReceived(water_sensor_mesh_MeshPacket *p, RxSource src)
{
    bool skipHandle = false;
    // Also, we should set the time from the ISR and it should have msec level resolution
    p->rx_time = getValidTime(RTCQualityFromNet); // store the arrival timestamp for the phone
    // Store a copy of encrypted packet for MQTT
    water_sensor_mesh_MeshPacket *p_encrypted = packetPool.allocCopy(*p);
    ...
    if (decodedState == DecodeState::DECODE_FATAL) {
        ...
        if (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag &&
            p->decoded.portnum == water_sensor_mesh_PortNum_NEIGHBORINFO_APP &&
...
#endif
        if (shouldIgnoreNonstandardPorts && p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag &&
            IS_ONE_OF(p->decoded.portnum, water_sensor_mesh_PortNum_ATAK_FORWARDER, water_sensor_mesh_PortNum_ATAK_PLUGIN,
                      water_sensor_mesh_PortNum_PAXCOUNTER_APP, water_sensor_mesh_PortNum_IP_TUNNEL_APP, water_sensor_mesh_PortNum_AUDIO_APP,
                      water_sensor_mesh_PortNum_PRIVATE_APP, water_sensor_mesh_PortNum_DETECTION_SENSOR_APP, water_sensor_mesh_PortNum_RANGE_TEST_APP,
                      water_sensor_mesh_PortNum_REMOTE_HARDWARE_APP)) {
...
void Router::perhapsHandleReceived(water_sensor_mesh_MeshPacket *p)
{
#if ENABLE_JSON_LOGGING
    ...
    water_sensor_mesh_NodeInfoLite const *node = nodeDB->getMeshNode(p->from);
