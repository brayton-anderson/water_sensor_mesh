...
// FIXME: correct logic to default to not using TCXO if no voltage is specified for LR11x0_DIO3_TCXO_VOLTAGE
    ...
    if ((power > LR1120_MAX_POWER) &&
        (config.lora.region == water_sensor_mesh_Config_LoRaConfig_RegionCode_LORA_24)) { // clamp again if wide freq range
...
template <typename T> bool LR11x0Interface<T>::reconfigure()
{
    ...
    if ((power > LR1120_MAX_POWER) && (config.lora.region == water_sensor_mesh_Config_LoRaConfig_RegionCode_LORA_24)) // 2.4G power limit
        power = LR1120_MAX_POWER;
 ...
 */
template <typename T> void LR11x0Interface<T>::addReceiveMetadata(water_sensor_mesh_MeshPacket *mp)
{
...
template <typename T> void LR11x0Interface<T>::startReceive()
{
    ...
    int err =
        lora.startReceive(RADIOLIB_LR11X0_RX_TIMEOUT_INF, WATER_SENSOR_MESH_RADIOLIB_IRQ_RX_FLAGS, RADIOLIB_IRQ_RX_DEFAULT_MASK, 0);
    assert(err == RADIOLIB_ERR_NONE);
