...
bool initEthernet()
{
    if (config.network.eth_enabled) {
        ...
        if (config.network.address_mode == water_sensor_mesh_Config_NetworkConfig_AddressMode_DHCP) {
            LOG_INFO("Start Ethernet DHCP");
            status = Ethernet.begin(mac);
        } else if (config.network.address_mode == water_sensor_mesh_Config_NetworkConfig_AddressMode_STATIC) {
