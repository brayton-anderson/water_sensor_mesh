...
static void onNetworkConnected()
{
    if (!APStartupComplete) {
        ...
        // start mdns
        if (!MDNS.begin("Water_Sensor_Mesh")) {
            LOG_ERROR("Error setting up mDNS responder!");
        } else {
            LOG_INFO("mDNS Host: Water_Sensor_Mesh.local");
            MDNS.addService("water_sensor_mesh", "tcp", SERVER_API_DEFAULT_PORT);
// ESPmDNS (ESP32) and SimpleMDNS (RP2040) have slightly different APIs for adding TXT records
#ifdef ARCH_ESP32
            MDNS.addServiceTxt("water_sensor_mesh", "tcp", "shortname", String(owner.short_name));
            MDNS.addServiceTxt("water_sensor_mesh", "tcp", "id", String(owner.id));
            // ESP32 prints obtained IP address in WiFiEvent
#elif defined(ARCH_RP2040)
            MDNS.addServiceTxt("water_sensor_mesh", "shortname", owner.short_name);
            MDNS.addServiceTxt("water_sensor_mesh", "id", owner.id);
        ...
        if (config.network.rsyslog_server[0]) {
            ...
            syslog.appName("Water_Sensor_Mesh");
...
#endif
#if !WATER_SENSOR_MESH_EXCLUDE_SOCKETAPI
        if (config.display.displaymode != water_sensor_mesh_Config_DisplayConfig_DisplayMode_COLOR) {
            initApiServer();
        }
#endif
...
bool initWifi()
{
    if (config.network.wifi_enabled && config.network.wifi_ssid[0]) {
        ...
        if (*wifiName) {
            uint8_t dmac[6];
            getMacAddr(dmac);
            snprintf(ourHost, sizeof(ourHost), "Water_Sensor_Mesh-%02x%02x", dmac[4], dmac[5]);

            WiFi.mode(WIFI_STA);
            WiFi.setHostname(ourHost);

            if (config.network.address_mode == water_sensor_mesh_Config_NetworkConfig_AddressMode_STATIC &&
