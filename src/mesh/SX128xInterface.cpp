...
template <typename T> bool SX128xInterface<T>::init()
{
    ...
    if ((config.lora.region != water_sensor_mesh_Config_LoRaConfig_RegionCode_LORA_24) && (res == RADIOLIB_ERR_INVALID_FREQUENCY)) {
        LOG_WARN("Radio only supports 2.4GHz LoRa. Adjusting Region and rebooting");
        config.lora.region = water_sensor_mesh_Config_LoRaConfig_RegionCode_LORA_24;
 ...
 */
template <typename T> void SX128xInterface<T>::addReceiveMetadata(water_sensor_mesh_MeshPacket *mp)
{
...
template <typename T> void SX128xInterface<T>::startReceive()
{
    ...
    int err = lora.startReceive(RADIOLIB_SX128X_RX_TIMEOUT_INF, WATER_SENSOR_MESH_RADIOLIB_IRQ_RX_FLAGS);
