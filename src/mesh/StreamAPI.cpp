...
void StreamAPI::emitRebooted()
{
    ...
    fromRadioScratch.which_payload_variant = water_sensor_mesh_FromRadio_rebooted_tag;
    fromRadioScratch.rebooted = true;

    // LOG_DEBUG("Emitting reboot packet for serial shell");
    emitTxBuffer(pb_encode_to_bytes(txBuf + HEADER_LEN, water_sensor_mesh_FromRadio_size, &water_sensor_mesh_FromRadio_msg, &fromRadioScratch));
}

void StreamAPI::emitLogRecord(water_sensor_mesh_LogRecord_Level level, const char *src, const char *format, va_list arg)
{
    // In case we send a FromRadio packet
    memset(&fromRadioScratch, 0, sizeof(fromRadioScratch));
    fromRadioScratch.which_payload_variant = water_sensor_mesh_FromRadio_log_record_tag;
