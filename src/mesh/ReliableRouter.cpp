 ...
 */
ErrorCode ReliableRouter::send(water_sensor_mesh_MeshPacket *p)
{
    if (p->want_ack) {
...
bool ReliableRouter::shouldFilterReceived(const water_sensor_mesh_MeshPacket *p)
{
    ...
    if (p->from == getNodeNum()) {
        ...
        if (old) {
            ...
            // marked as wantAck
            sendAckNak(water_sensor_mesh_Routing_Error_NONE, getFrom(p), p->id, old->packet->channel);
 ...
 */
void ReliableRouter::sniffReceived(const water_sensor_mesh_MeshPacket *p, const water_sensor_mesh_Routing *c)
{
    if (isToUs(p)) { // ignore ack/nak/want_ack packets that are not address to us (we only handle 0 hop reliability)
        if (p->want_ack) {
            if (MeshModule::currentReply) {
            ...
            } else if (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
                ...
                if (!p->decoded.request_id)
                    sendAckNak(water_sensor_mesh_Routing_Error_NONE, getFrom(p), p->id, p->channel,
            ...
            } else if (p->which_payload_variant == water_sensor_mesh_MeshPacket_encrypted_tag && p->channel == 0 &&
                       (nodeDB->getMeshNode(p->from) == nullptr || nodeDB->getMeshNode(p->from)->user.public_key.size == 0)) {
                LOG_INFO("PKI packet from unknown node, send PKI_UNKNOWN_PUBKEY");
                sendAckNak(water_sensor_mesh_Routing_Error_PKI_UNKNOWN_PUBKEY, getFrom(p), p->id, channels.getPrimaryIndex(),
                           routingModule->getHopLimitForResponse(p->hop_start, p->hop_limit));
            } else {
                // Send a 'NO_CHANNEL' error on the primary channel if want_ack packet destined for us cannot be decoded
                sendAckNak(water_sensor_mesh_Routing_Error_NO_CHANNEL, getFrom(p), p->id, channels.getPrimaryIndex(),
                           routingModule->getHopLimitForResponse(p->hop_start, p->hop_limit));
            }
        }
        if (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag && c &&
            c->error_reason == water_sensor_mesh_Routing_Error_PKI_UNKNOWN_PUBKEY) {
        ...
        PacketId ackId = ((c && c->error_reason == water_sensor_mesh_Routing_Error_NONE) || !c) ? p->decoded.request_id : 0;

        // A nak is a routing packt that has an  error code
        PacketId nakId = (c && c->error_reason != water_sensor_mesh_Routing_Error_NONE) ? p->decoded.request_id : 0;
