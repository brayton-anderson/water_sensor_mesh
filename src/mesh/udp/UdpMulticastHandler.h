...
class UdpMulticastHandler final
{
  public:
    ...
    void onReceive(AsyncUDPPacket packet)
    {
        ...
        bool isPacketDecoded = pb_decode_from_bytes(packet.data(), packetLength, &water_sensor_mesh_MeshPacket_msg, &mp);
        if (isPacketDecoded && router && mp.which_payload_variant == water_sensor_mesh_MeshPacket_encrypted_tag) {
    ...
    bool onSend(const water_sensor_mesh_MeshPacket *mp)
    {
        if (!mp || !udp) {
            return false;
        }
        ...
        uint8_t buffer[water_sensor_mesh_MeshPacket_size];
        size_t encodedLength = pb_encode_to_bytes(buffer, sizeof(buffer), &water_sensor_mesh_MeshPacket_msg, mp);
