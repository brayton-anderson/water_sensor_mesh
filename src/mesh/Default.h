...
#define default_mqtt_address "mqtt.water_sensor_mesh.org"
...
#define IF_ROUTER(routerVal, normalVal)                                                                                          \
    ((config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER) ? (routerVal) : (normalVal))
  ...
  private:
    static float congestionScalingCoefficient(int numOnlineNodes)
    {
        ...
        if (numOnlineNodes <= 10) {
            ...
            if (config.lora.use_preset && config.lora.modem_preset == water_sensor_mesh_Config_LoRaConfig_ModemPreset_MEDIUM_SLOW)
                throttlingFactor = 0.04;
            else if (config.lora.use_preset && config.lora.modem_preset == water_sensor_mesh_Config_LoRaConfig_ModemPreset_MEDIUM_FAST)
                throttlingFactor = 0.02;
            else if (config.lora.use_preset &&
                     IS_ONE_OF(config.lora.modem_preset, water_sensor_mesh_Config_LoRaConfig_ModemPreset_SHORT_FAST,
                               water_sensor_mesh_Config_LoRaConfig_ModemPreset_SHORT_TURBO,
                               water_sensor_mesh_Config_LoRaConfig_ModemPreset_SHORT_SLOW))
