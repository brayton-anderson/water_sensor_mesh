...
bool RadioLibInterface::canSendImmediately()
{
    ...
    if (busyTx || busyRx) {
        ...
        if (busyTx && !Throttle::isWithinTimespanMs(lastTxStart, 60000)) {
            ...
            RECORD_CRITICALERROR(water_sensor_mesh_CriticalErrorCode_TRANSMIT_FAILED);
...
ErrorCode RadioLibInterface::send(water_sensor_mesh_MeshPacket *p)
{

#ifndef DISABLE_WELCOME_UNSET

    if (config.lora.region != water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET) {
...
water_sensor_mesh_QueueStatus RadioLibInterface::getQueueStatus()
{
    ...
    case TRANSMIT_DELAY_COMPLETED:
        ...
        if (!txQueue.empty()) {
            if (!canSendImmediately()) {
                ...
                water_sensor_mesh_MeshPacket *txp = txQueue.getFront();
                assert(txp);
...
{
    water_sensor_mesh_MeshPacket *p = txQueue.getFront();
    if (!p) {
...
void RadioLibInterface::clampToLateRebroadcastWindow(NodeNum from, PacketId id)
{
    ...
    water_sensor_mesh_MeshPacket *p = txQueue.remove(from, id, true, false);
    if (p) {
...
void RadioLibInterface::handleTransmitInterrupt()
{
    ...
    if (sendingPacket)
        completeSending();
    powerMon->clearState(water_sensor_mesh_PowerMon_State_Lora_TXOn); // But our transmitter is definitely off now
}
    ...
    if (state != RADIOLIB_ERR_NONE) {
        ...
        if (payloadLen < 0) {
            ...
            mp->which_payload_variant =
                water_sensor_mesh_MeshPacket_encrypted_tag; // Mark that the payload is still encrypted at this point
...
{
    isReceiving = true;
    powerMon->setState(water_sensor_mesh_PowerMon_State_Lora_RXOn);
}

void RadioLibInterface::configHardwareForSend()
{
    powerMon->setState(water_sensor_mesh_PowerMon_State_Lora_TXOn);
}

void RadioLibInterface::setStandby()
{
    // neither sending nor receiving
    powerMon->clearState(water_sensor_mesh_PowerMon_State_Lora_RXOn);
    powerMon->clearState(water_sensor_mesh_PowerMon_State_Lora_TXOn);
}

/** start an immediate transmit */
bool RadioLibInterface::startSend(water_sensor_mesh_MeshPacket *txp)
{
    ...
    if (disabled || !config.lora.tx_enabled) {
        ...
        if (res != RADIOLIB_ERR_NONE) {
            ...
            RECORD_CRITICALERROR(water_sensor_mesh_CriticalErrorCode_RADIO_SPI_BUG);

            // This send failed, but make sure to 'complete' it properly
            completeSending();
            powerMon->clearState(water_sensor_mesh_PowerMon_State_Lora_TXOn); // Transmitter off now
