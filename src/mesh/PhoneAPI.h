...
#include "water_sensor_mesh/portnums.pb.h"
#include <iterator>
#include <string>
...
#if water_sensor_mesh_FromRadio_size > MAX_TO_FROM_RADIO_SIZE
#error "water_sensor_mesh_FromRadio_size is too large for our BLE packets"
#endif
#if water_sensor_mesh_ToRadio_size > MAX_TO_FROM_RADIO_SIZE
#error "water_sensor_mesh_ToRadio_size is too large for our BLE packets"
#endif
    ...
    : public Observer<uint32_t> // FIXME, we shouldn't be inheriting from Observer, instead use CallbackObserver as a member
{
    ...
    std::unordered_map<water_sensor_mesh_PortNum, uint32_t> lastPortNumToRadio;
    ...
    /// downloads it
    water_sensor_mesh_MeshPacket *packetForPhone = NULL;

    // file transfer packets destined for phone. Push it to the queue then free it.
    water_sensor_mesh_XModem xmodemPacketForPhone = water_sensor_mesh_XModem_init_zero;

    // Keep QueueStatus packet just as packetForPhone
    water_sensor_mesh_QueueStatus *queueStatusPacketForPhone = NULL;

    // Keep MqttClientProxyMessage packet just as packetForPhone
    water_sensor_mesh_MqttClientProxyMessage *mqttClientProxyMessageForPhone = NULL;

    // Keep ClientNotification packet just as packetForPhone
    water_sensor_mesh_ClientNotification *clientNotification = NULL;

    /// We temporarily keep the nodeInfo here between the call to available and getFromRadio
    water_sensor_mesh_NodeInfo nodeInfoForPhone = water_sensor_mesh_NodeInfo_init_default;

    water_sensor_mesh_ToRadio toRadioScratch = {
    ...
    std::vector<water_sensor_mesh_FileInfo> filesManifest = {};
  ...
  public:
     ...
     */
    virtual void sendNotification(water_sensor_mesh_LogRecord_Level level, uint32_t replyId, const char *message);
  ...
  protected:
    ...
    water_sensor_mesh_FromRadio fromRadioScratch = {};
  ...
  private:
     ...
     */
    bool handleToRadioPacket(water_sensor_mesh_MeshPacket &p);
