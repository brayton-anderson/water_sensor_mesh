...
struct GlobalPacketId {
    ...
    explicit GlobalPacketId(const water_sensor_mesh_MeshPacket *p)
    {
        node = getFrom(p);
        id = p->id;
    }
...
struct PendingPacket {
    water_sensor_mesh_MeshPacket *packet;
    ...
    PendingPacket() {}
    explicit PendingPacket(water_sensor_mesh_MeshPacket *p, uint8_t numRetransmissions);
};
...
class NextHopRouter : public FloodingRouter
{
  ...
  protected:
     ...
     */
    PendingPacket *startRetransmission(water_sensor_mesh_MeshPacket *p, uint8_t numReTx = NUM_INTERMEDIATE_RETX);
  ...
  private:
    ...
    bool perhapsRelay(const water_sensor_mesh_MeshPacket *p);
};