...
static MemoryDynamic<water_sensor_mesh_MqttClientProxyMessage> staticMqttClientProxyMessagePool;

static MemoryDynamic<water_sensor_mesh_QueueStatus> staticQueueStatusPool;

static MemoryDynamic<water_sensor_mesh_ClientNotification> staticClientNotificationPool;

Allocator<water_sensor_mesh_MqttClientProxyMessage> &mqttClientProxyMessagePool = staticMqttClientProxyMessagePool;

Allocator<water_sensor_mesh_ClientNotification> &clientNotificationPool = staticClientNotificationPool;

Allocator<water_sensor_mesh_QueueStatus> &queueStatusPool = staticQueueStatusPool;
...
int MeshService::handleFromRadio(const water_sensor_mesh_MeshPacket *mp)
{
    powerFSM.trigger(EVENT_PACKET_FOR_PHONE); // Possibly keep the node from sleeping

    nodeDB->updateFrom(*mp); // update our DB state based off sniffing every RX packet from the radio
    bool isPreferredRebroadcaster =
        IS_ONE_OF(config.device.role, water_sensor_mesh_Config_DeviceConfig_Role_ROUTER, water_sensor_mesh_Config_DeviceConfig_Role_REPEATER);
    if (mp->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag &&
        mp->decoded.portnum == water_sensor_mesh_PortNum_TELEMETRY_APP && mp->decoded.request_id > 0) {
        LOG_DEBUG("Received telemetry response. Skip sending our NodeInfo"); //  because this potentially a Repeater which will
                                                                             //  ignore our request for its NodeInfo
    } else if (mp->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag && !nodeDB->getMeshNode(mp->from)->has_user &&
...
NodeNum MeshService::getNodenumFromRequestId(uint32_t request_id)
{
    ...
    for (int i = 0; i < toPhoneQueue.numUsed(); i++) {
        water_sensor_mesh_MeshPacket *p = toPhoneQueue.dequeuePtr(0);
        if (p->id == request_id) {
            nodenum = p->to;
 ...
 * reference
 */
void MeshService::handleToRadio(water_sensor_mesh_MeshPacket &p)
{
#if defined(ARCH_PORTDUINO)
    if (SimRadio::instance && p.decoded.portnum == water_sensor_mesh_PortNum_SIMULATOR_APP) {
...
ErrorCode MeshService::sendQueueStatusToPhone(const water_sensor_mesh_QueueStatus &qs, ErrorCode res, uint32_t mesh_packet_id)
{
    water_sensor_mesh_QueueStatus *copied = queueStatusPool.allocCopy(qs);
    ...
    if (toPhoneQueueStatusQueue.numFree() == 0) {
        ...
        water_sensor_mesh_QueueStatus *d = toPhoneQueueStatusQueue.dequeuePtr(0);
        if (d)
...
void MeshService::sendToMesh(water_sensor_mesh_MeshPacket *p, RxSource src, bool ccToPhone)
{
...
#if HAS_GPS && !WATER_SENSOR_MESH_EXCLUDE_GPS
        if (positionModule) {
...
void MeshService::sendToPhone(water_sensor_mesh_MeshPacket *p)
{
    perhapsDecode(p);
    ...
    if (toPhoneQueue.numFree() == 0) {
        if (p->decoded.portnum == water_sensor_mesh_PortNum_TEXT_MESSAGE_APP ||
            ...
            water_sensor_mesh_MeshPacket *d = toPhoneQueue.dequeuePtr(0);
            if (d)
                releaseToPool(d);
        } else {
...
void MeshService::sendMqttMessageToClientProxy(water_sensor_mesh_MqttClientProxyMessage *m)
{
    ...
    if (toPhoneMqttProxyQueue.numFree() == 0) {
        ...
        water_sensor_mesh_MqttClientProxyMessage *d = toPhoneMqttProxyQueue.dequeuePtr(0);
        if (d)
...
void MeshService::sendClientNotification(water_sensor_mesh_ClientNotification *n)
{
    ...
    if (toPhoneClientNotificationQueue.numFree() == 0) {
        ...
        water_sensor_mesh_ClientNotification *d = toPhoneClientNotificationQueue.dequeuePtr(0);
        if (d)
...
water_sensor_mesh_NodeInfoLite *MeshService::refreshLocalMeshNode()
{
    water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(nodeDB->getNodeNum());
    assert(node);
    ...
    water_sensor_mesh_PositionLite &position = node->position;
...
#if HAS_GPS
int MeshService::onGPSChanged(const water_sensor_mesh::GPSStatus *newStatus)
{
    // Update our local node info with our position (even if we don't decide to update anyone else)
    const water_sensor_mesh_NodeInfoLite *node = refreshLocalMeshNode();
...
uint32_t MeshService::GetTimeSinceMeshPacket(const water_sensor_mesh_MeshPacket *mp)
{
