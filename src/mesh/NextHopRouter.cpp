...
PendingPacket::PendingPacket(water_sensor_mesh_MeshPacket *p, uint8_t numRetransmissions)
{
    packet = p;
 ...
 * Send a packet
 */
ErrorCode NextHopRouter::send(water_sensor_mesh_MeshPacket *p)
{
...
bool NextHopRouter::shouldFilterReceived(const water_sensor_mesh_MeshPacket *p)
{
        ...
        if (wasFallback) {
            ...
            if (isRepeated) {
                if (!findInTxQueue(p->from, p->id) && !perhapsRelay(p) && isToUs(p) && p->want_ack)
                    sendAckNak(water_sensor_mesh_Routing_Error_NONE, getFrom(p), p->id, p->channel, 0);
...
void NextHopRouter::sniffReceived(const water_sensor_mesh_MeshPacket *p, const water_sensor_mesh_Routing *c)
{
    NodeNum ourNodeNum = getNodeNum();
    uint8_t ourRelayID = nodeDB->getLastByteOfNodeNum(ourNodeNum);
    bool isAckorReply = (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) &&
    ...
    if (isAckorReply) {
        ...
        if (p->from != 0) {
            water_sensor_mesh_NodeInfoLite *origTx = nodeDB->getMeshNode(p->from);
            if (origTx) {
...
bool NextHopRouter::perhapsRelay(const water_sensor_mesh_MeshPacket *p)
{
    if (!isToUs(p) && !isFromUs(p) && p->hop_limit > 0) {
        if (p->next_hop == NO_NEXT_HOP_PREFERENCE || p->next_hop == nodeDB->getLastByteOfNodeNum(getNodeNum())) {
            if (isRebroadcaster()) {
                water_sensor_mesh_MeshPacket *tosend = packetPool.allocCopy(*p); // keep a copy because we will be sending it
...
uint8_t NextHopRouter::getNextHop(NodeNum to, uint8_t relay_node)
{
    ...
    water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(to);
...
bool NextHopRouter::stopRetransmission(GlobalPacketId key)
{
    ...
    if (old) {
        ...
        if (old->numRetransmissions < NUM_RELIABLE_RETX - 1) {
            ...
            if (isFromUs(p) || (config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_ROUTER &&
 ...
 */
PendingPacket *NextHopRouter::startRetransmission(water_sensor_mesh_MeshPacket *p, uint8_t numReTx)
{
            ...
            if (p.numRetransmissions == 0) {
                if (isFromUs(p.packet)) {
                    LOG_DEBUG("Reliable send failed, returning a nak for fr=0x%x,to=0x%x,id=0x%x", p.packet->from, p.packet->to,
                              p.packet->id);
                    sendAckNak(water_sensor_mesh_Routing_Error_MAX_RETRANSMIT, getFrom(p.packet), p.packet->id, p.packet->channel);
                }
                ...
                if (!isBroadcast(p.packet->to)) {
                    if (p.numRetransmissions == 1) {
                        ...
                        water_sensor_mesh_NodeInfoLite *sentTo = nodeDB->getMeshNode(p.packet->to);
                        if (sentTo) {
