 ...
 */
template <typename T> void SX126xInterface<T>::addReceiveMetadata(water_sensor_mesh_MeshPacket *mp)
{
...
template <typename T> void SX126xInterface<T>::startReceive()
{
    ...
    int err = lora.startReceiveDutyCycleAuto(preambleLength, 8, WATER_SENSOR_MESH_RADIOLIB_IRQ_RX_FLAGS);
    if (err != RADIOLIB_ERR_NONE)
