...
EXT_RAM_BSS_ATTR water_sensor_mesh_DeviceState devicestate;
water_sensor_mesh_MyNodeInfo &myNodeInfo = devicestate.my_node;
water_sensor_mesh_NodeDatabase nodeDatabase;
water_sensor_mesh_LocalConfig config;
water_sensor_mesh_DeviceUIConfig uiconfig{.screen_brightness = 153, .screen_timeout = 30};
water_sensor_mesh_LocalModuleConfig moduleConfig;
water_sensor_mesh_ChannelFile channelFile;
...
bool water_sensor_mesh_NodeDatabase_callback(pb_istream_t *istream, pb_ostream_t *ostream, const pb_field_iter_t *field)
{
    if (ostream) {
        std::vector<water_sensor_mesh_NodeInfoLite> const *vec = (std::vector<water_sensor_mesh_NodeInfoLite> *)field->pData;
        for (auto item : *vec) {
            if (!pb_encode_tag_for_field(ostream, field))
                return false;
            pb_encode_submessage(ostream, water_sensor_mesh_NodeInfoLite_fields, &item);
        }
    }
    if (istream) {
        water_sensor_mesh_NodeInfoLite node; // this gets good data
        std::vector<water_sensor_mesh_NodeInfoLite> *vec = (std::vector<water_sensor_mesh_NodeInfoLite> *)field->pData;

        if (istream->bytes_left && pb_decode(istream, water_sensor_mesh_NodeInfoLite_fields, &node))
            vec->push_back(node);
    }
    return true;
}
 ...
 */
water_sensor_mesh_User &owner = devicestate.owner;
water_sensor_mesh_Position localPosition = water_sensor_mesh_Position_init_default;
water_sensor_mesh_CriticalErrorCode error_code =
    water_sensor_mesh_CriticalErrorCode_NONE; // For the error code, only show values from this boot (discard value from flash)
...
NodeDB::NodeDB()
{
    ...
    if (!config.has_security) {
        ...
        config.security = water_sensor_mesh_Config_SecurityConfig_init_default;
...
#elif !(WATER_SENSOR_MESH_EXCLUDE_PKI)
    ...
    water_sensor_mesh_NodeInfoLite *info = getOrCreateMeshNode(getNodeNum());
    ...
    preferences.begin("water_sensor_mesh", false);
    ...
    // config.network.enabled_protocols = water_sensor_mesh_Config_NetworkConfig_ProtocolFlags_UDP_BROADCAST;
    ...
    if (config.display.displaymode == water_sensor_mesh_Config_DisplayConfig_DisplayMode_COLOR) {
        ...
        config.display.displaymode = water_sensor_mesh_Config_DisplayConfig_DisplayMode_DEFAULT;
    ...
    if (config.position.gps_enabled) {
        config.position.gps_mode = water_sensor_mesh_Config_PositionConfig_GpsMode_ENABLED;
        config.position.gps_enabled = 0;
    }
#ifdef USERPREFS_FIXED_GPS
    if (myNodeInfo.reboot_count == 1) { // Check if First boot ever or after Factory Reset.
        water_sensor_mesh_Position fixedGPS = water_sensor_mesh_Position_init_default;
        ...
        fixedGPS.location_source = water_sensor_mesh_Position_LocSource_LOC_MANUAL;
...
void NodeDB::installDefaultNodeDatabase()
{
    ...
    nodeDatabase.nodes = std::vector<water_sensor_mesh_NodeInfoLite>(MAX_NUM_NODES);
    numMeshNodes = 0;
...
void NodeDB::installDefaultConfig(bool preserveKey = false)
{
    ...
    memset(&config, 0, sizeof(water_sensor_mesh_LocalConfig));
    ...
    config.display.displaymode = water_sensor_mesh_Config_DisplayConfig_DisplayMode_COLOR;
#endif

#ifdef USERPREFS_CONFIG_DEVICE_ROLE
    // Restrict ROUTER*, LOST AND FOUND, and REPEATER roles for security reasons
    if (IS_ONE_OF(USERPREFS_CONFIG_DEVICE_ROLE, water_sensor_mesh_Config_DeviceConfig_Role_ROUTER,
                  water_sensor_mesh_Config_DeviceConfig_Role_ROUTER_LATE, water_sensor_mesh_Config_DeviceConfig_Role_REPEATER,
                  water_sensor_mesh_Config_DeviceConfig_Role_LOST_AND_FOUND)) {
...
#else
    config.device.role = water_sensor_mesh_Config_DeviceConfig_Role_CLIENT; // Default to client.
#endif

#ifdef USERPREFS_CONFIG_LORA_REGION
    config.lora.region = USERPREFS_CONFIG_LORA_REGION;
#else
    config.lora.region = water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET;
#endif
#ifdef USERPREFS_LORACONFIG_MODEM_PRESET
    config.lora.modem_preset = USERPREFS_LORACONFIG_MODEM_PRESET;
#else
    config.lora.modem_preset = water_sensor_mesh_Config_LoRaConfig_ModemPreset_LONG_FAST;
#endif
    ...
    config.position.gps_mode = water_sensor_mesh_Config_PositionConfig_GpsMode_NOT_PRESENT;
#elif !defined(GPS_RX_PIN)
    if (config.position.rx_gpio == 0)
        config.position.gps_mode = water_sensor_mesh_Config_PositionConfig_GpsMode_NOT_PRESENT;
    else
        config.position.gps_mode = water_sensor_mesh_Config_PositionConfig_GpsMode_DISABLED;
#else
    config.position.gps_mode = water_sensor_mesh_Config_PositionConfig_GpsMode_ENABLED;
#endif
    ...
    strncpy(config.network.ntp_server, "water_sensor_mesh.pool.ntp.org", 32);
...
#elif WATER_SENSOR_MESH_INCLUDE_NICHE_GRAPHICS // See "src/graphics/niche"
    ...
    config.bluetooth.mode = water_sensor_mesh_Config_BluetoothConfig_PairingMode_FIXED_PIN;
#else
    config.bluetooth.mode = hasScreen ? water_sensor_mesh_Config_BluetoothConfig_PairingMode_RANDOM_PIN
                                      : water_sensor_mesh_Config_BluetoothConfig_PairingMode_FIXED_PIN;
#endif
    // for backward compat, default position flags are ALT+MSL
    config.position.position_flags =
        (water_sensor_mesh_Config_PositionConfig_PositionFlags_ALTITUDE | water_sensor_mesh_Config_PositionConfig_PositionFlags_ALTITUDE_MSL |
         water_sensor_mesh_Config_PositionConfig_PositionFlags_SPEED | water_sensor_mesh_Config_PositionConfig_PositionFlags_HEADING |
         water_sensor_mesh_Config_PositionConfig_PositionFlags_DOP | water_sensor_mesh_Config_PositionConfig_PositionFlags_SATINVIEW);
...
void NodeDB::installDefaultModuleConfig()
{
    ...
    memset(&moduleConfig, 0, sizeof(water_sensor_mesh_ModuleConfig));
...
#if USERPREFS_MQTT_ENABLED && !WATER_SENSOR_MESH_EXCLUDE_MQTT
    ...
    moduleConfig.detection_sensor.detection_trigger_type = water_sensor_mesh_ModuleConfig_DetectionSensorConfig_TriggerType_LOGIC_HIGH;
...
void NodeDB::installRoleDefaults(water_sensor_mesh_Config_DeviceConfig_Role role)
{
    if (role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER) {
        initConfigIntervals();
        initModuleConfigIntervals();
        moduleConfig.telemetry.device_update_interval = default_telemetry_broadcast_interval_secs;
        config.device.rebroadcast_mode = water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_CORE_PORTNUMS_ONLY;
        owner.has_is_unmessagable = true;
        owner.is_unmessagable = true;
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER_LATE) {
        moduleConfig.telemetry.device_update_interval = ONE_DAY;
        owner.has_is_unmessagable = true;
        owner.is_unmessagable = true;
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_REPEATER) {
        owner.has_is_unmessagable = true;
        owner.is_unmessagable = true;
        config.display.screen_on_secs = 1;
        config.device.rebroadcast_mode = water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_CORE_PORTNUMS_ONLY;
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_SENSOR) {
        owner.has_is_unmessagable = true;
        owner.is_unmessagable = true;
        moduleConfig.telemetry.device_update_interval = default_telemetry_broadcast_interval_secs;
        moduleConfig.telemetry.environment_measurement_enabled = true;
        moduleConfig.telemetry.environment_update_interval = 300;
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_LOST_AND_FOUND) {
        config.position.position_broadcast_smart_enabled = false;
        config.position.position_broadcast_secs = 300; // Every 5 minutes
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_TAK) {
        config.device.node_info_broadcast_secs = ONE_DAY;
        config.position.position_broadcast_smart_enabled = false;
        config.position.position_broadcast_secs = ONE_DAY;
        // Remove Altitude MSL from flags since CoTs use HAE (height above ellipsoid)
        config.position.position_flags =
            (water_sensor_mesh_Config_PositionConfig_PositionFlags_ALTITUDE | water_sensor_mesh_Config_PositionConfig_PositionFlags_SPEED |
             water_sensor_mesh_Config_PositionConfig_PositionFlags_HEADING | water_sensor_mesh_Config_PositionConfig_PositionFlags_DOP);
        moduleConfig.telemetry.device_update_interval = ONE_DAY;
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_TRACKER) {
        owner.has_is_unmessagable = true;
        owner.is_unmessagable = true;
        moduleConfig.telemetry.device_update_interval = default_telemetry_broadcast_interval_secs;
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_TAK_TRACKER) {
        owner.has_is_unmessagable = true;
        owner.is_unmessagable = true;
        config.device.node_info_broadcast_secs = ONE_DAY;
        config.position.position_broadcast_smart_enabled = true;
        ...
        config.position.position_flags =
            (water_sensor_mesh_Config_PositionConfig_PositionFlags_ALTITUDE | water_sensor_mesh_Config_PositionConfig_PositionFlags_SPEED |
             water_sensor_mesh_Config_PositionConfig_PositionFlags_HEADING | water_sensor_mesh_Config_PositionConfig_PositionFlags_DOP);
        moduleConfig.telemetry.device_update_interval = ONE_DAY;
    } else if (role == water_sensor_mesh_Config_DeviceConfig_Role_CLIENT_HIDDEN) {
        config.device.rebroadcast_mode = water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_LOCAL_ONLY;
        config.device.node_info_broadcast_secs = MAX_INTERVAL;
        config.position.position_broadcast_smart_enabled = false;
        config.position.position_broadcast_secs = MAX_INTERVAL;
...
void NodeDB::installDefaultChannels()
{
    ...
    memset(&channelFile, 0, sizeof(water_sensor_mesh_ChannelFile));
...
void NodeDB::resetNodes()
{
    if (!config.position.fixed_position)
        clearLocalPosition();
    numMeshNodes = 1;
    std::fill(nodeDatabase.nodes.begin() + 1, nodeDatabase.nodes.end(), water_sensor_mesh_NodeInfoLite());
...
{
    water_sensor_mesh_NodeInfoLite *node = getMeshNode(nodeDB->getNodeNum());
    node->position.latitude_i = 0;
    node->position.longitude_i = 0;
    node->position.altitude = 0;
    node->position.time = 0;
    setLocalPosition(water_sensor_mesh_Position_init_default);
}
...
void NodeDB::installDefaultDeviceState()
{
    ...
    // memset(&devicestate, 0, sizeof(water_sensor_mesh_DeviceState));
...
#else
    snprintf(owner.long_name, sizeof(owner.long_name), "Water_Sensor_Mesh %04x", getNodeNum() & 0x0ffff);
#endif
...
void NodeDB::pickNewNodeNum()
{
    ...
    water_sensor_mesh_NodeInfoLite *found;
                                 ...
                                 void *dest_struct)
{
    ...
    if (f) {
        ...
        if (fields != &water_sensor_mesh_NodeDatabase_msg) // contains a vector object
...
void NodeDB::loadFromDisk()
{
    ...
    water_sensor_mesh_Config_SecurityConfig backupSecurity = water_sensor_mesh_Config_SecurityConfig_init_zero;
    ...
    if (FSCom.exists(legacyPrefFileName)) {
        ...
        if (loadProto(configFileName, water_sensor_mesh_LocalConfig_size, sizeof(water_sensor_mesh_LocalConfig), &water_sensor_mesh_LocalConfig_msg,
...
#endif
    auto state = loadProto(nodeDatabaseFileName, getMaxNodesAllocatedSize(), sizeof(water_sensor_mesh_NodeDatabase),
                           &water_sensor_mesh_NodeDatabase_msg, &nodeDatabase);
    ...
    state = loadProto(deviceStateFileName, water_sensor_mesh_DeviceState_size, sizeof(water_sensor_mesh_DeviceState),
                      &water_sensor_mesh_DeviceState_msg, &devicestate);

    // See https://github.com/water_sensor_mesh/firmware/issues/4184#issuecomment-2269390786
    // It is very important to try and use the saved prefs even if we fail to read water_sensor_mesh_DeviceState.  Because most of our
    ...
    state = loadProto(configFileName, water_sensor_mesh_LocalConfig_size, sizeof(water_sensor_mesh_LocalConfig), &water_sensor_mesh_LocalConfig_msg,
                      &config);
    if (state != LoadFileResult::LOAD_SUCCESS) {
    ...
    state = loadProto(moduleConfigFileName, water_sensor_mesh_LocalModuleConfig_size, sizeof(water_sensor_mesh_LocalModuleConfig),
                      &water_sensor_mesh_LocalModuleConfig_msg, &moduleConfig);
    if (state != LoadFileResult::LOAD_SUCCESS) {
    ...
    state = loadProto(channelFileName, water_sensor_mesh_ChannelFile_size, sizeof(water_sensor_mesh_ChannelFile), &water_sensor_mesh_ChannelFile_msg,
                      &channelFile);
    if (state != LoadFileResult::LOAD_SUCCESS) {
    ...
    state = loadProto(uiconfigFileName, water_sensor_mesh_DeviceUIConfig_size, sizeof(water_sensor_mesh_DeviceUIConfig),
                      &water_sensor_mesh_DeviceUIConfig_msg, &uiconfig);
    if (state == LoadFileResult::LOAD_SUCCESS) {
...
bool NodeDB::saveChannelsToDisk()
{
    ...
    spiLock->unlock();
#endif
    return saveProto(channelFileName, water_sensor_mesh_ChannelFile_size, &water_sensor_mesh_ChannelFile_msg, &channelFile);
}
...
bool NodeDB::saveDeviceStateToDisk()
{
    ...
    spiLock->unlock();
#endif
    // Note: if MAX_NUM_NODES=100 and water_sensor_mesh_NodeInfoLite_size=166, so will be approximately 17KB
    // Because so huge we _must_ not use fullAtomic, because the filesystem is probably too small to hold two copies of this
    return saveProto(deviceStateFileName, water_sensor_mesh_DeviceState_size, &water_sensor_mesh_DeviceState_msg, &devicestate, true);
}
...
bool NodeDB::saveNodeDatabaseToDisk()
{
    ...
    pb_get_encoded_size(&nodeDatabaseSize, water_sensor_mesh_NodeDatabase_fields, &nodeDatabase);
    return saveProto(nodeDatabaseFileName, nodeDatabaseSize, &water_sensor_mesh_NodeDatabase_msg, &nodeDatabase, false);
}
...
bool NodeDB::saveToDiskNoRetry(int saveWhat)
{
    ...
    if (saveWhat & SEGMENT_CONFIG) {
        ...
        success &= saveProto(configFileName, water_sensor_mesh_LocalConfig_size, &water_sensor_mesh_LocalConfig_msg, &config);
    }
    ...
    if (saveWhat & SEGMENT_MODULECONFIG) {
        ...
        success &=
            saveProto(moduleConfigFileName, water_sensor_mesh_LocalModuleConfig_size, &water_sensor_mesh_LocalModuleConfig_msg, &moduleConfig);
    }
...
bool NodeDB::saveToDisk(int saveWhat)
{
    ...
    if (!success) {
        ...
        RECORD_CRITICALERROR(success ? water_sensor_mesh_CriticalErrorCode_FLASH_CORRUPTION_RECOVERABLE
                                     : water_sensor_mesh_CriticalErrorCode_FLASH_CORRUPTION_UNRECOVERABLE);
    }

    return success;
}

const water_sensor_mesh_NodeInfoLite *NodeDB::readNextMeshNode(uint32_t &readIndex)
{
...
uint32_t sinceLastSeen(const water_sensor_mesh_NodeInfoLite *n)
{
...
uint32_t sinceReceived(const water_sensor_mesh_MeshPacket *p)
{
    uint32_t now = getTime();
 ...
 */
void NodeDB::updatePosition(uint32_t nodeId, const water_sensor_mesh_Position &p, RxSource src)
{
    water_sensor_mesh_NodeInfoLite *info = getOrCreateMeshNode(nodeId);
    if (!info) {
        return;
    }
 ...
 */
void NodeDB::updateTelemetry(uint32_t nodeId, const water_sensor_mesh_Telemetry &t, RxSource src)
{
    water_sensor_mesh_NodeInfoLite *info = getOrCreateMeshNode(nodeId);
    // Environment metrics should never go to NodeDb but we'll safegaurd anyway
    if (!info || t.which_variant != water_sensor_mesh_Telemetry_device_metrics_tag) {
        return;
    }
 ...
 */
void NodeDB::addFromContact(water_sensor_mesh_SharedContact contact)
{
    water_sensor_mesh_NodeInfoLite *info = getOrCreateMeshNode(contact.node_num);
    if (!info) {
        return;
    }
 ...
 */
bool NodeDB::updateUser(uint32_t nodeId, water_sensor_mesh_User &p, uint8_t channelIndex)
{
    water_sensor_mesh_NodeInfoLite *info = getOrCreateMeshNode(nodeId);
    if (!info) {
        return false;
    ...
    if (p.public_key.size == 32 && nodeId != nodeDB->getNodeNum()) {
        ...
        if (owner.public_key.size == 32 && memcmp(p.public_key.bytes, owner.public_key.bytes, 32) == 0 && !duplicateWarned) {
            ...
            cn->which_payload_variant = water_sensor_mesh_ClientNotification_duplicated_public_key_tag;
...
void NodeDB::updateFrom(const water_sensor_mesh_MeshPacket &mp)
{
    ...
    if (mp.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag && mp.from) {
        ...
        water_sensor_mesh_NodeInfoLite *info = getOrCreateMeshNode(getFrom(&mp));
        if (!info) {
            return;
        }
...
{
    water_sensor_mesh_NodeInfoLite *lite = getMeshNode(nodeId);
...
{
    const water_sensor_mesh_NodeInfoLite *info = getMeshNode(n);
    if (!info) {
...
water_sensor_mesh_NodeInfoLite *NodeDB::getMeshNode(NodeNum n)
{
    for (int i = 0; i < numMeshNodes; i++)
...
water_sensor_mesh_NodeInfoLite *NodeDB::getOrCreateMeshNode(NodeNum n)
{
    water_sensor_mesh_NodeInfoLite *lite = getMeshNode(n);
...
/// valid lat/lon
bool NodeDB::hasValidPosition(const water_sensor_mesh_NodeInfoLite *n)
{
...
{
    water_sensor_mesh_NodeInfoLite *info = getMeshNode(nodeNum);
...
bool NodeDB::checkLowEntropyPublicKey(const water_sensor_mesh_Config_SecurityConfig_public_key_t keyToTest)
{
...
bool NodeDB::backupPreferences(water_sensor_mesh_AdminMessage_BackupLocation location)
{
    bool success = false;
    lastBackupAttempt = millis();
#ifdef FSCom
    if (location == water_sensor_mesh_AdminMessage_BackupLocation_FLASH) {
        water_sensor_mesh_BackupPreferences backup = water_sensor_mesh_BackupPreferences_init_zero;
        ...
        size_t backupSize;
        pb_get_encoded_size(&backupSize, water_sensor_mesh_BackupPreferences_fields, &backup);

        spiLock->lock();
        FSCom.mkdir("/backups");
        spiLock->unlock();
        success = saveProto(backupFileName, backupSize, &water_sensor_mesh_BackupPreferences_msg, &backup);
    ...
    } else if (location == water_sensor_mesh_AdminMessage_BackupLocation_SD) {
...
bool NodeDB::restorePreferences(water_sensor_mesh_AdminMessage_BackupLocation location, int restoreWhat)
{
    bool success = false;
#ifdef FSCom
    if (location == water_sensor_mesh_AdminMessage_BackupLocation_FLASH) {
        ...
        water_sensor_mesh_BackupPreferences backup = water_sensor_mesh_BackupPreferences_init_zero;
        success = loadProto(backupFileName, water_sensor_mesh_BackupPreferences_size, sizeof(water_sensor_mesh_BackupPreferences),
                            &water_sensor_mesh_BackupPreferences_msg, &backup);
        if (success) {
...
void recordCriticalError(water_sensor_mesh_CriticalErrorCode code, uint32_t address, const char *filename)
{
    if (filename) {
