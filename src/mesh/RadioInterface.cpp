...
#define RDEF(name, freq_start, freq_end, duty_cycle, spacing, power_limit, audio_permitted, frequency_switching, wide_lora)      \
    {                                                                                                                            \
        water_sensor_mesh_Config_LoRaConfig_RegionCode_##name, freq_start, freq_end, duty_cycle, spacing, power_limit, audio_permitted, \
    ...
    */
    ...
    /*
        Philippines
                ...
                https://github.com/water_sensor_mesh/firmware/issues/4948#issuecomment-2394926135
    */
    ...
    /*
        Kazakhstan
                ...
                863 - 868 MHz <25 mW EIRP, 500kHz channels allowed, must not be used at airfields
                                https://github.com/water_sensor_mesh/firmware/issues/7204
    */
...
void initRegion()
{
    ...
    for (; r->code != water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET && r->code != REGULATORY_LORA_REGIONCODE; r++)
        ;
    LOG_INFO("Wanted region %d, regulatory override to %s", config.lora.region, r->name);
#else
    for (; r->code != water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET && r->code != config.lora.region; r++)
        ;
...
uint32_t RadioInterface::getPacketTime(const water_sensor_mesh_MeshPacket *p)
{
    uint32_t pl = 0;
...
uint32_t RadioInterface::getRetransmissionMsec(const water_sensor_mesh_MeshPacket *p)
{
    size_t numbytes = pb_encode_to_bytes(bytes, sizeof(bytes), &water_sensor_mesh_Data_msg, &p->decoded);
...
void printPacket(const char *prefix, const water_sensor_mesh_MeshPacket *p)
{
...
{
    assert(sizeof(PacketHeader) == WATER_SENSOR_MESH_HEADER_LENGTH); // make sure the compiler did what we expected
}
...
void RadioInterface::applyModemConfig()
{
    ...
    while (!validConfig) {
        ...
        if ((myRegion->freqEnd - myRegion->freqStart) < bw / 1000) {
            ...
            RECORD_CRITICALERROR(water_sensor_mesh_CriticalErrorCode_INVALID_RADIO_SETTING);

            water_sensor_mesh_ClientNotification *cn = clientNotificationPool.allocZeroed();
            cn->level = water_sensor_mesh_LogRecord_Level_ERROR;
            sprintf(cn->message, err_string);
            service->sendClientNotification(cn);

            // Set to default modem preset
            loraConfig.use_preset = true;
            loraConfig.modem_preset = water_sensor_mesh_Config_LoRaConfig_ModemPreset_LONG_FAST;
        } else {
            validConfig = true;
        }
    }
    ...
    maxPacketTimeMsec = getPacketTime(water_sensor_mesh_Constants_DATA_PAYLOAD_LEN + sizeof(PacketHeader));
...
void RadioInterface::deliverToReceiver(water_sensor_mesh_MeshPacket *p)
{
    if (router)
 ...
 */
size_t RadioInterface::beginSending(water_sensor_mesh_MeshPacket *p)
{
    assert(!sendingPacket);

    // LOG_DEBUG("Send queued packet on mesh (txGood=%d,rxGood=%d,rxBad=%d)", rf95.txGood(), rf95.rxGood(), rf95.rxBad());
    assert(p->which_payload_variant == water_sensor_mesh_MeshPacket_encrypted_tag); // It should have already been encoded by now
