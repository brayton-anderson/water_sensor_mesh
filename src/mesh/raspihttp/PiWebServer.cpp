/*
Adds a WebServer and WebService callbacks to water_sensor_mesh as Linux Version. The WebServer & Webservices
...
3.) Checkout the web project
    https://github.com/water_sensor_mesh/web.git
...
int handleAPIv1ToRadio(const struct _u_request *req, struct _u_response *res, void *user_data)
{
    ...
    ulfius_add_header_to_response(res, "X-Protobuf-Schema",
                                  "https://raw.githubusercontent.com/water_sensor_mesh/protobufs/master/water_sensor_mesh/mesh.proto");
 ...
 * Trigger : WebGui(POLL)->handleAPIv1FromRadio->phoneapi->Water_Sensor_Mesh(Radio) events
 */
...
int generate_self_signed_x509(EVP_PKEY *pkey, X509 **x509)
{
    *x509 = X509_new();
    ...
    X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC, (unsigned char *)"Water_Sensor_Mesh", -1, -1, 0);
    X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, (unsigned char *)"water_sensor_mesh.local", -1, -1, 0);
