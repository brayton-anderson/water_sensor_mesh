...
#define WATER_SENSOR_MESH_HEADER_LENGTH 16
#define WATER_SENSOR_MESH_PKC_OVERHEAD 12
...
class RadioInterface
{
  ...
  protected:
    ...
    water_sensor_mesh_MeshPacket *sendingPacket = NULL; // The packet we are currently sending
     ...
     */
    void deliverToReceiver(water_sensor_mesh_MeshPacket *p);
  ...
  public:
     ...
     */
    virtual ErrorCode send(water_sensor_mesh_MeshPacket *p) = 0;

    /** Return TX queue status */
    virtual water_sensor_mesh_QueueStatus getQueueStatus()
    {
    ...
    uint32_t getRetransmissionMsec(const water_sensor_mesh_MeshPacket *p);
     ...
     */
    uint32_t getPacketTime(const water_sensor_mesh_MeshPacket *p);
  ...
  protected:
     ...
     */
    size_t beginSending(water_sensor_mesh_MeshPacket *p);
