...
#if HAS_WIFI && !defined(ARCH_PORTDUINO) || defined(WATER_SENSOR_MESH_EXCLUDE_WIFI)
...
// Circumvent the battery sensing logic and assumes constant power if no battery pin or power mgmt IC
    ...
    bool isRouter = (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER ? 1 : 0);
...
static void lsIdle()
{
    ...
    if (secsSlept < config.power.ls_secs) {
        ...
        if (doPreflightSleep()) {
            ...
            powerMon->setState(water_sensor_mesh_PowerMon_State_CPU_LightSleep);
            ledBlink.set(false); // Never leave led on while in light sleep
            esp_sleep_source_t wakeCause2 = doLightSleep(sleepTime * 1000LL);
            powerMon->clearState(water_sensor_mesh_PowerMon_State_CPU_LightSleep);
...
// We never enter light-sleep or NB states on NRF52 (because the CPU uses so little power normally)
#ifdef ARCH_ESP32
    // See: https://github.com/water_sensor_mesh/firmware/issues/1071
...
#if HAS_WIFI || !defined(WATER_SENSOR_MESH_EXCLUDE_WIFI)
    bool isTrackerOrSensor = config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_TRACKER ||
                             config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_TAK_TRACKER ||
                             config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_SENSOR;
    ...
    if ((isRouter || config.power.is_power_saving) && !isWifiAvailable() && !isTrackerOrSensor) {
        ...
        // Also serves purpose of the old DARK to DARK transition(?) See https://github.com/water_sensor_mesh/firmware/issues/3517
...
#endif // HAS_WIFI || !defined(WATER_SENSOR_MESH_EXCLUDE_WIFI)
