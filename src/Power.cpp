 ...
 *
 * This file is part of the Water_Sensor_Mesh project.
 * For more information, see: https://water_sensor_mesh.org/
 */
#include "power.h"
#include "NodeDB.h"
#include "PowerFSM.h"
...
#if defined(DEBUG_HEAP_MQTT) && !WATER_SENSOR_MESH_EXCLUDE_MQTT
#include "mqtt/MQTT.h"
...
#if HAS_TELEMETRY && (!WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR || !WATER_SENSOR_MESH_EXCLUDE_POWER_TELEMETRY)
...
#if HAS_TELEMETRY && !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR && HAS_RAKPROT
...
using namespace water_sensor_mesh;
...
#if HAS_TELEMETRY && defined(HAS_RAKPROT) && !defined(HAS_PMU) && !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR
        if (hasRAK()) {
            return getRAKVoltage();
        }
#endif

#if HAS_TELEMETRY && !defined(HAS_PMU) && !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR
        if (hasINA()) {
...
#define BATTERY_SENSE_SAMPLES                                                                                                    \
    ...
    {
#if HAS_TELEMETRY && !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR && defined(HAS_RAKPROT) && !defined(HAS_PMU)
        if (hasRAK()) {
...
#else
#if HAS_TELEMETRY && !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR && !defined(DISABLE_INA_CHARGING_DETECTION)
        if (hasINA()) {
  ...
  private:
...
#if HAS_TELEMETRY && !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR && defined(HAS_RAKPROT)
    ...
    {
        if (nodeTelemetrySensorsMap[water_sensor_mesh_TelemetrySensorType_INA219].first == config.power.device_battery_ina_address) {
            return ina219Sensor.getBusVoltageMv();
        } else if (nodeTelemetrySensorsMap[water_sensor_mesh_TelemetrySensorType_INA226].first ==
                   config.power.device_battery_ina_address) {
            return ina226Sensor.getBusVoltageMv();
        } else if (nodeTelemetrySensorsMap[water_sensor_mesh_TelemetrySensorType_INA260].first ==
                   config.power.device_battery_ina_address) {
            return ina260Sensor.getBusVoltageMv();
        } else if (nodeTelemetrySensorsMap[water_sensor_mesh_TelemetrySensorType_INA3221].first ==
                   config.power.device_battery_ina_address) {
...
bool Power::axpChipInit()
{
    ...
    if (PMU->getChipModel() == XPOWERS_AXP192) {
        ...
        if (HW_VENDOR == water_sensor_mesh_HardwareModel_TBEAM) {
            // Unuse power channel
            PMU->disablePowerOutput(XPOWERS_DCDC2);
        ...
        } else if (HW_VENDOR == water_sensor_mesh_HardwareModel_LILYGO_TBEAM_S3_CORE ||
                   HW_VENDOR == water_sensor_mesh_HardwareModel_T_WATCH_S3) {
            // t-beam s3 core
            /**
...
#if !WATER_SENSOR_MESH_EXCLUDE_I2C && __has_include(<Adafruit_MAX1704X.h>)
...
class LipoBatteryLevel : public HasBatteryLevel
{
  ...
  public:
    ...
    bool runOnce()
    {
        ...
        if (nodeTelemetrySensorsMap[water_sensor_mesh_TelemetrySensorType_MAX17048].first != 0) {
            return max17048->runOnce(nodeTelemetrySensorsMap[water_sensor_mesh_TelemetrySensorType_MAX17048].second);
        }
        return false;
    }
