...
using namespace water_sensor_mesh; /** @todo remove */
...
static bool shouldDrawMessage(const water_sensor_mesh_MeshPacket *packet)
{
...
Screen::Screen(ScanI2C::DeviceAddress address, water_sensor_mesh_Config_DisplayConfig_OledType screenType, OLEDDISPLAY_GEOMETRY geometry)
...
void Screen::handleSetOn(bool on, FrameCallback einkScreensaver)
{
    ...
    if (on != screenOn) {
        if (on) {
            ...
            powerMon->setState(water_sensor_mesh_PowerMon_State_Screen_On);
#ifdef T_WATCH_S3
    ...
    !defined(M5STACK) // set display brightness when turning on screens. Just moved function from TFTDisplay to here.
            ...
            powerMon->clearState(water_sensor_mesh_PowerMon_State_Screen_On);
#ifdef USE_EINK
...
int32_t Screen::runOnce()
{
    ...
    if (!NotificationRenderer::isOverlayBannerShowing() && config.lora.region == water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET) {
...
// Show detailed node views only on E-Ink builds
    ...
    for (size_t i = 0; i < nodeDB->getNumMeshNodes(); i++) {
        const water_sensor_mesh_NodeInfoLite *n = nodeDB->getMeshNodeByIndex(i);
...
int Screen::handleStatusUpdate(const water_sensor_mesh::Status *arg)
{
...
int Screen::handleTextMessage(const water_sensor_mesh_MeshPacket *packet)
{
    if (showingNormalScreen) {
        if (packet->from == 0) {
            ...
            const water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(packet->from);
...
#else
graphics::Screen::Screen(ScanI2C::DeviceAddress, water_sensor_mesh_Config_DisplayConfig_OledType, OLEDDISPLAY_GEOMETRY) {}
#endif // HAS_SCREEN
