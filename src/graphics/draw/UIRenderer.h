...
class UIRenderer
{
  public:
    // Common UI elements
    static void drawNodes(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::NodeStatus *nodeStatus,
                          int node_offset = 0, bool show_total = true, String additional_words = "");

    // GPS status functions
    static void drawGps(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gpsStatus);
    static void drawGpsCoordinates(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gpsStatus);
    static void drawGpsAltitude(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gpsStatus);
    static void drawGpsPowerStatus(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gpsStatus);
