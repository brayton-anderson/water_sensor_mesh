...
void menuHandler::LoraRegionPicker(uint32_t duration)
{
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected != 0 && config.lora.region != _water_sensor_mesh_Config_LoRaConfig_RegionCode(selected)) {
            config.lora.region = _water_sensor_mesh_Config_LoRaConfig_RegionCode(selected);
...
void menuHandler::ClockFacePicker()
{
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == Back) {
            ...
            nodeDB->saveProto("/prefs/uiconfig.proto", water_sensor_mesh_DeviceUIConfig_size, &water_sensor_mesh_DeviceUIConfig_msg, &uiconfig);
...
void menuHandler::messageResponseMenu()
{
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == Dismiss) {
            ...
            const water_sensor_mesh_MeshPacket &mp = devicestate.rx_text_message;
...
void menuHandler::compassNorthMenu()
{
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == 1) {
            if (uiconfig.compass_mode != water_sensor_mesh_CompassMode_DYNAMIC) {
                uiconfig.compass_mode = water_sensor_mesh_CompassMode_DYNAMIC;
                nodeDB->saveProto("/prefs/uiconfig.proto", water_sensor_mesh_DeviceUIConfig_size, &water_sensor_mesh_DeviceUIConfig_msg,
                                  &uiconfig);
                screen->setFrames(graphics::Screen::FOCUS_PRESERVE);
            }
        } else if (selected == 2) {
            if (uiconfig.compass_mode != water_sensor_mesh_CompassMode_FIXED_RING) {
                uiconfig.compass_mode = water_sensor_mesh_CompassMode_FIXED_RING;
                nodeDB->saveProto("/prefs/uiconfig.proto", water_sensor_mesh_DeviceUIConfig_size, &water_sensor_mesh_DeviceUIConfig_msg,
                                  &uiconfig);
                screen->setFrames(graphics::Screen::FOCUS_PRESERVE);
            }
            ...
            if (uiconfig.compass_mode != water_sensor_mesh_CompassMode_FREEZE_HEADING) {
                uiconfig.compass_mode = water_sensor_mesh_CompassMode_FREEZE_HEADING;
                nodeDB->saveProto("/prefs/uiconfig.proto", water_sensor_mesh_DeviceUIConfig_size, &water_sensor_mesh_DeviceUIConfig_msg,
                                  &uiconfig);
                screen->setFrames(graphics::Screen::FOCUS_PRESERVE);
            }
...
void menuHandler::GPSToggleMenu()
{
    ...
    bannerOptions.InitialSelected = config.position.gps_mode == water_sensor_mesh_Config_PositionConfig_GpsMode_ENABLED ? 1 : 2;
    screen->showOverlayBanner(bannerOptions);
}
#endif
...
void menuHandler::BuzzerModeMenu()
{
    ...
    bannerOptions.optionsCount = 4;
    bannerOptions.bannerCallback = [](int selected) -> void {
        config.device.buzzer_mode = (water_sensor_mesh_Config_DeviceConfig_BuzzerMode)selected;
        service->reloadConfig(SEGMENT_CONFIG);
    };
...
{
    static const char *optionsArray[] = {"Back", "Default", "Water_Sensor_Mesh Green", "Yellow", "Red", "Orange", "Purple", "Teal",
                                         "Pink", "White"};
    BannerOverlayOptions bannerOptions;
    ...
    bannerOptions.bannerCallback = [display](int selected) -> void {
        ...
        if (selected == 1) {
            ...
            LOG_INFO("Setting color to Water_Sensor_Mesh Green");
            r = 103;
            g = 234;
            b = 148;
        } else if (selected == 3) {
