...
void drawFrameWiFi(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    */
    ...
    display->drawString(x, getTextPositions(display)[line++], "URL: http://water_sensor_mesh.local");
...
void drawFrameSettings(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    if (config.position.gps_mode == water_sensor_mesh_Config_PositionConfig_GpsMode_ENABLED) {
        ...
        if (config.display.gps_format !=
            water_sensor_mesh_Config_DisplayConfig_GpsCoordinateFormat_DMS) // if DMS then don't draw altitude
