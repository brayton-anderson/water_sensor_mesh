...
void UIRenderer::drawGps(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gps)
{
...
void UIRenderer::drawGpsPowerStatus(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gps)
{
    const char *displayLine;
    int pos;
    if (y < FONT_HEIGHT_SMALL) { // Line 1: use short string
        displayLine = config.position.gps_mode == water_sensor_mesh_Config_PositionConfig_GpsMode_NOT_PRESENT ? "No GPS" : "GPS off";
        pos = display->getWidth() - display->getStringWidth(displayLine);
    } else {
        displayLine = config.position.gps_mode == water_sensor_mesh_Config_PositionConfig_GpsMode_NOT_PRESENT ? "GPS not present"
                                                                                                       : "GPS is disabled";
...
void UIRenderer::drawGpsAltitude(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gps)
{
    char displayLine[32];
    if (!gps->getIsConnected() && !config.position.fixed_position) {
        ...
        if (config.display.units == water_sensor_mesh_Config_DisplayConfig_DisplayUnits_IMPERIAL)
...
void UIRenderer::drawGpsCoordinates(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::GPSStatus *gps)
{
    ...
    if (!gps->getIsConnected() && !config.position.fixed_position) {
        ...
        if (gpsFormat != water_sensor_mesh_Config_DisplayConfig_GpsCoordinateFormat_DMS) {
            char coordinateLine[22];
            if (gpsFormat == water_sensor_mesh_Config_DisplayConfig_GpsCoordinateFormat_DEC) { // Decimal Degrees
                snprintf(coordinateLine, sizeof(coordinateLine), "%f %f", geoCoord.getLatitude() * 1e-7,
                         geoCoord.getLongitude() * 1e-7);
            } else if (gpsFormat == water_sensor_mesh_Config_DisplayConfig_GpsCoordinateFormat_UTM) { // Universal Transverse Mercator
                snprintf(coordinateLine, sizeof(coordinateLine), "%2i%1c %06u %07u", geoCoord.getUTMZone(), geoCoord.getUTMBand(),
                         geoCoord.getUTMEasting(), geoCoord.getUTMNorthing());
            } else if (gpsFormat == water_sensor_mesh_Config_DisplayConfig_GpsCoordinateFormat_MGRS) { // Military Grid Reference System
            ...
            } else if (gpsFormat == water_sensor_mesh_Config_DisplayConfig_GpsCoordinateFormat_OLC) { // Open Location Code
                geoCoord.getOLCCode(coordinateLine);
            } else if (gpsFormat == water_sensor_mesh_Config_DisplayConfig_GpsCoordinateFormat_OSGR) { // Ordnance Survey Grid Reference
...
#endif // !WATER_SENSOR_MESH_EXCLUDE_GPS

// Draw nodes status
void UIRenderer::drawNodes(OLEDDisplay *display, int16_t x, int16_t y, const water_sensor_mesh::NodeStatus *nodeStatus, int node_offset,
...
void UIRenderer::drawNodeInfo(OLEDDisplay *display, const OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    static std::vector<water_sensor_mesh_NodeInfoLite *> favoritedNodes;
    ...
    if (state->currentFrame != prevFrame) {
        ...
        for (size_t i = 0; i < total; i++) {
            water_sensor_mesh_NodeInfoLite *n = nodeDB->getMeshNodeByIndex(i);
        ...
        std::sort(favoritedNodes.begin(), favoritedNodes.end(),
                  [](const water_sensor_mesh_NodeInfoLite *a, const water_sensor_mesh_NodeInfoLite *b) { return a->num < b->num; });
    }
    ...
    water_sensor_mesh_NodeInfoLite *node = favoritedNodes[nodeIndex];
    ...
    water_sensor_mesh_NodeInfoLite *ourNode = nodeDB->getMeshNode(nodeDB->getNodeNum());
    ...
    if (SCREEN_WIDTH > SCREEN_HEIGHT) {
        ...
        if (showCompass) {
            ...
            */
            ...
            if (uiconfig.compass_mode == water_sensor_mesh_CompassMode_FREEZE_HEADING) {
                myHeading = 0;
            } else {
                bearing -= myHeading;
            }
...
#if HAS_GPS
    if (config.position.gps_mode != water_sensor_mesh_Config_PositionConfig_GpsMode_ENABLED) {
        const char *displayLine;
        if (config.position.fixed_position) {
...
void UIRenderer::drawIconScreen(const char *upperMsg, OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    const char *title = "water_sensor_mesh.org";
...
void UIRenderer::drawCompassAndLocationScreen(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    if (validHeading) {
        ...
        if (SCREEN_WIDTH > SCREEN_HEIGHT) {
            ...
            // "N" label
            float northAngle = 0;
            if (uiconfig.compass_mode != water_sensor_mesh_CompassMode_FIXED_RING)
                northAngle = -heading;
