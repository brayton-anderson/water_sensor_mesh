...
class Applet : public GFX
{
  ...
  protected:
    ...
    // Water_Sensor_Mesh Logo

    static constexpr float LOGO_ASPECT_RATIO = 1.9;                    // Width:Height for drawing the Water_Sensor_Mesh logo
    uint16_t getLogoWidth(uint16_t limitWidth, uint16_t limitHeight);  // Size Water_Sensor_Mesh logo to fit within region
    uint16_t getLogoHeight(uint16_t limitWidth, uint16_t limitHeight); // Size Water_Sensor_Mesh logo to fit within region
    void drawLogo(int16_t centerX, int16_t centerY, uint16_t width, uint16_t height,
                  Color color = BLACK); // Draw the Water_Sensor_Mesh logo
    ...
    std::string parseShortName(water_sensor_mesh_NodeInfoLite *node); // Get the shortname, or a substitute if has unprintable chars
