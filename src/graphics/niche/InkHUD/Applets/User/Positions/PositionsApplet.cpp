...
void InkHUD::PositionsApplet::onRender()
{
    ...
    water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(lastFrom);
...
ProcessMessage InkHUD::PositionsApplet::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
    ...
    float lat;
    float lng;
    if (mp.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag && mp.decoded.portnum == water_sensor_mesh_PortNum_POSITION_APP) {
        water_sensor_mesh_Position position = water_sensor_mesh_Position_init_default;
        if (pb_decode_from_bytes(mp.decoded.payload.bytes, mp.decoded.payload.size, &water_sensor_mesh_Position_msg, &position)) {
            ...
            {
                hasPosition = true;
                lat = position.latitude_i * 1e-7; // Convert from Water_Sensor_Mesh's internal  int32_t format
