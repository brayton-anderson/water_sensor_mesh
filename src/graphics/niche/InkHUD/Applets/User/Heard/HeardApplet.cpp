...
void InkHUD::HeardApplet::populateFromNodeDB()
{
    ...
    std::vector<water_sensor_mesh_NodeInfoLite *> ordered;
    ...
    std::sort(ordered.begin(), ordered.end(), [](water_sensor_mesh_NodeInfoLite *top, water_sensor_mesh_NodeInfoLite *bottom) -> bool {
    ...
    for (water_sensor_mesh_NodeInfoLite *node : ordered) {
        CardInfo c;
        c.nodeNum = node->num;
