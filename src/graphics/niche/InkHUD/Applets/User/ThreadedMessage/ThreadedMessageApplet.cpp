...
InkHUD::ThreadedMessageApplet::ThreadedMessageApplet(uint8_t channelIndex)
    : SinglePortModule("ThreadedMessageApplet", water_sensor_mesh_PortNum_TEXT_MESSAGE_APP), channelIndex(channelIndex)
{
...
void InkHUD::ThreadedMessageApplet::onRender()
{
    ...
    while (msgB >= (0 - fontSmall.lineHeight()) && i < store->messages.size()) {
        ...
        if (outgoing)
            ...
            water_sensor_mesh_NodeInfoLite *sender = nodeDB->getMeshNode(m.sender);
            if (sender)
...
ProcessMessage InkHUD::ThreadedMessageApplet::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
