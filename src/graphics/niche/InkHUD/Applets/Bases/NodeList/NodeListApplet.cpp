...
bool InkHUD::NodeListApplet::wantPacket(const water_sensor_mesh_MeshPacket *p)
{
    // Only interested if:
    return isActive()                                                  // Applet is active
           ...
           && (isToUs(p) || isBroadcast(p->to) ||                      // Either: intended for us,
               p->decoded.portnum == water_sensor_mesh_PortNum_NODEINFO_APP); // or nodeinfo
...
ProcessMessage InkHUD::NodeListApplet::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
    ...
    water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(c.nodeNum);
    ...
    if (node) {
        ...
        if (nodeDB->hasValidPosition(node) && nodeDB->hasValidPosition(ourNode)) {
            ...
            // Water_Sensor_Mesh stores these as integers internally
...
void InkHUD::NodeListApplet::onRender()
{
    ...
    for (auto card = cards.begin(); card != cards.end(); ++card) {
        ...
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(nodeNum);
