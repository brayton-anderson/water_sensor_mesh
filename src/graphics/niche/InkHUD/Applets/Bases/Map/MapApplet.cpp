...
void InkHUD::MapApplet::drawLabeledMarker(water_sensor_mesh_NodeInfoLite *node)
{
    // Find x and y position based on node's position in nodeDB
    assert(nodeDB->hasValidPosition(node));
    Marker m = calculateMarker(node->position.latitude_i * 1e-7,  // Lat, converted from Water_Sensor_Mesh's internal int32 style
                               node->position.longitude_i * 1e-7, // Long, converted from Water_Sensor_Mesh's internal int32 style
...
void InkHUD::MapApplet::calculateAllMarkers()
{
    ...
    for (uint32_t i = 0; i < nodeDB->getNumMeshNodes(); i++) {
        ...
        markers.push_back(
            calculateMarker(node->position.latitude_i * 1e-7,  // Lat, converted from Water_Sensor_Mesh's internal int32 style
                            node->position.longitude_i * 1e-7, // Long, converted from Water_Sensor_Mesh's internal int32 style
                            node->has_hops_away,               // Is the hopsAway number valid
                            node->hops_away                    // Hops away
                            ));
