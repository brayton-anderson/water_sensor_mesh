...
class MapApplet : public Applet
{
  ...
  protected:
    virtual bool shouldDrawNode(water_sensor_mesh_NodeInfoLite *node) { return true; } // Allow derived applets to filter the nodes
    ...
    void drawLabeledMarker(water_sensor_mesh_NodeInfoLite *node); // Highlight a specific marker
