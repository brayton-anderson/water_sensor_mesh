...
void InkHUD::MenuApplet::execute(MenuItem item)
{
    ...
    case TOGGLE_INVERT_COLOR:
        if (config.display.displaymode == water_sensor_mesh_Config_DisplayConfig_DisplayMode_INVERTED)
            config.display.displaymode = water_sensor_mesh_Config_DisplayConfig_DisplayMode_DEFAULT;
        else
            config.display.displaymode = water_sensor_mesh_Config_DisplayConfig_DisplayMode_INVERTED;
...
void InkHUD::MenuApplet::showPage(MenuPage page)
{
    ...
    case OPTIONS:
        ...
        // Optional: GPS
        if (config.position.gps_mode == water_sensor_mesh_Config_PositionConfig_GpsMode_DISABLED)
        ...
        invertedColors = (config.display.displaymode == water_sensor_mesh_Config_DisplayConfig_DisplayMode_INVERTED);
...
void InkHUD::MenuApplet::populateRecipientPage()
{
    ...
    for (uint8_t i = 0; i < MAX_NUM_CHANNELS; i++) {
        ...
        water_sensor_mesh_Channel &channel = channels.getByIndex(i);
        if (!channel.has_settings || channel.role == water_sensor_mesh_Channel_Role_DISABLED)
            continue;
        ...
        if (channel.role == water_sensor_mesh_Channel_Role_PRIMARY)
            r.label += "Primary";
        else
