...
class CannedMessageStore
{
  ...
  private:
    ...
    void handleSet(const water_sensor_mesh_AdminMessage *request); // Client API changing the canned messages
    void handleGet(water_sensor_mesh_AdminMessage *response);      // Client API wants to know current canned messages
