...
ProcessMessage RangeTestModuleRadio::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
#if defined(ARCH_ESP32) || defined(ARCH_NRF52) || defined(ARCH_PORTDUINO)
...
bool RangeTestModuleRadio::appendFile(const water_sensor_mesh_MeshPacket &mp)
{
#ifdef ARCH_ESP32
    auto &p = mp.decoded;

    water_sensor_mesh_NodeInfoLite *n = nodeDB->getMeshNode(getFrom(&mp));
    /*
        LOG_DEBUG("-----------------------------------------");
        LOG_DEBUG("p.payload.bytes  \"%s\"", p.payload.bytes);
        LOG_DEBUG("p.payload.size   %d", p.payload.size);
    ...
    */
    ...
    if (gpsStatus->getIsConnected() || config.position.fixed_position) {
        ...
        water_sensor_mesh_NodeInfoLite *us = nodeDB->getMeshNode(nodeDB->getNodeNum());
