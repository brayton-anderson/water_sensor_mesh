...
struct NodeEntry {
    water_sensor_mesh_NodeInfoLite *node;
    uint32_t lastHeard;
};
...
class CannedMessageModule : public SinglePortModule, public Observable<const UIFrameEvent *>, private concurrency::OSThread
{
  public:
    ...
    // === Admin Handlers ===
    void handleGetCannedMessageModuleMessages(const water_sensor_mesh_MeshPacket &req, water_sensor_mesh_AdminMessage *response);
    ...
    virtual bool wantPacket(const water_sensor_mesh_MeshPacket *p) override
    {
        ...
        if (p->rx_snr > 0)
            lastRxSnr = p->rx_snr;
        return (p->decoded.portnum == water_sensor_mesh_PortNum_ROUTING_APP) ? waitingForAck : false;
    }
