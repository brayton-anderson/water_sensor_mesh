...
bool RoutingModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Routing *r)
{
    bool maybePKI = mp.which_payload_variant == water_sensor_mesh_MeshPacket_encrypted_tag && mp.channel == 0 && !isBroadcast(mp.to);
    // Beginning of logic whether to drop the packet based on Rebroadcast mode
    if (mp.which_payload_variant == water_sensor_mesh_MeshPacket_encrypted_tag &&
        (config.device.rebroadcast_mode == water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_LOCAL_ONLY ||
         config.device.rebroadcast_mode == water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_KNOWN_ONLY)) {
        if (!maybePKI)
            return false;
...
water_sensor_mesh_MeshPacket *RoutingModule::allocReply()
{
...
void RoutingModule::sendAckNak(water_sensor_mesh_Routing_Error err, NodeNum to, PacketId idFrom, ChannelIndex chIndex, uint8_t hopLimit)
{
...
RoutingModule::RoutingModule() : ProtobufModule("routing", water_sensor_mesh_PortNum_ROUTING_APP, &water_sensor_mesh_Routing_msg)
{
    isPromiscuous = true;
