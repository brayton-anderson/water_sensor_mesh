 ...
 * functionality of the Water_Sensor_Mesh device. The class provides methods for sending and receiving messages, as well as managing the
 * message history queue. It also initializes and manages the data structures used for storing the message history.
 *
 * The StoreForwardModule class is used by the MeshService class to provide store and forward functionality to the Water_Sensor_Mesh
 * device.
 *
 * @author Jm Casler
 * @date [Insert Date]
...
#include "mesh/generated/water_sensor_mesh/storeforward.pb.h"
...
int32_t StoreForwardModule::runOnce()
{
    ...
    if (moduleConfig.store_forward.enabled && is_server) {
        ...
        if (this->busy) {
            ...
            water_sensor_mesh_StoreAndForward sf = water_sensor_mesh_StoreAndForward_init_zero;
            sf.rr = water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_HEARTBEAT;
            sf.which_variant = water_sensor_mesh_StoreAndForward_heartbeat_tag;
...
void StoreForwardModule::historySend(uint32_t secAgo, uint32_t to)
{
    ...
    water_sensor_mesh_StoreAndForward sf = water_sensor_mesh_StoreAndForward_init_zero;
    sf.rr = water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_HISTORY;
    sf.which_variant = water_sensor_mesh_StoreAndForward_history_tag;
 ...
 */
water_sensor_mesh_MeshPacket *StoreForwardModule::getForPhone()
{
    if (moduleConfig.store_forward.enabled && is_server) {
        ...
        if (this->busy && this->busyTo == to) {
            water_sensor_mesh_MeshPacket *p = preparePayload(to, 0, true); // No time limit
 ...
 */
void StoreForwardModule::historyAdd(const water_sensor_mesh_MeshPacket &mp)
{
    ...
    memcpy(this->packetHistory[this->packetHistoryTotalCount].payload, p.payload.bytes, water_sensor_mesh_Constants_DATA_PAYLOAD_LEN);
...
{
    water_sensor_mesh_MeshPacket *p = preparePayload(dest, last_time);
    if (p) {
 ...
 */
water_sensor_mesh_MeshPacket *StoreForwardModule::preparePayload(NodeNum dest, uint32_t last_time, bool local)
{
    for (uint32_t i = lastRequest[dest]; i < this->packetHistoryTotalCount; i++) {
        if (this->packetHistory[i].time && (this->packetHistory[i].time > last_time)) {
            ...
            if (this->packetHistory[i].from != dest &&
                ...
                water_sensor_mesh_MeshPacket *p = allocDataPacket();
                ...
                if (local) { // PhoneAPI gets normal TEXT_MESSAGE_APP
                    ...
                    water_sensor_mesh_StoreAndForward sf = water_sensor_mesh_StoreAndForward_init_zero;
                    sf.which_variant = water_sensor_mesh_StoreAndForward_text_tag;
                    ...
                    if (this->packetHistory[i].to == NODENUM_BROADCAST) {
                        sf.rr = water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_TEXT_BROADCAST;
                    } else {
                        sf.rr = water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_TEXT_DIRECT;
                    }

                    p->decoded.payload.size = pb_encode_to_bytes(p->decoded.payload.bytes, sizeof(p->decoded.payload.bytes),
                                                                 &water_sensor_mesh_StoreAndForward_msg, &sf);
                }
 ...
 */
void StoreForwardModule::sendMessage(NodeNum dest, const water_sensor_mesh_StoreAndForward &payload)
{
    water_sensor_mesh_MeshPacket *p = allocDataProtobuf(payload);
 ...
 */
void StoreForwardModule::sendMessage(NodeNum dest, water_sensor_mesh_StoreAndForward_RequestResponse rr)
{
...
{
    water_sensor_mesh_MeshPacket *pr = allocDataPacket();
    pr->to = dest;
    pr->priority = water_sensor_mesh_MeshPacket_Priority_BACKGROUND;
    pr->want_ack = false;
    pr->decoded.want_response = false;
    pr->decoded.portnum = water_sensor_mesh_PortNum_TEXT_MESSAGE_APP;
    const char *str;
    if (this->busy) {
...
void StoreForwardModule::statsSend(uint32_t to)
{
    ...
    sf.rr = water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_STATS;
    sf.which_variant = water_sensor_mesh_StoreAndForward_stats_tag;
 ...
 */
ProcessMessage StoreForwardModule::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
#if defined(ARCH_ESP32) || defined(ARCH_PORTDUINO)
    if (moduleConfig.store_forward.enabled) {

        if ((mp.decoded.portnum == water_sensor_mesh_PortNum_TEXT_MESSAGE_APP) && is_server) {
            auto &p = mp.decoded;
        ...
        } else if (!isFromUs(&mp) && mp.decoded.portnum == water_sensor_mesh_PortNum_STORE_FORWARD_APP) {
            auto &p = mp.decoded;
            water_sensor_mesh_StoreAndForward scratch;
            water_sensor_mesh_StoreAndForward *decoded = NULL;
            if (mp.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
                if (pb_decode_from_bytes(p.payload.bytes, p.payload.size, &water_sensor_mesh_StoreAndForward_msg, &scratch)) {
                    decoded = &scratch;
                } else {
 ...
 */
bool StoreForwardModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_StoreAndForward *p)
{
    if (!moduleConfig.store_forward.enabled) {
    ...
    switch (p->rr) {
    case water_sensor_mesh_StoreAndForward_RequestResponse_CLIENT_ERROR:
    case water_sensor_mesh_StoreAndForward_RequestResponse_CLIENT_ABORT:
        if (is_server) {
    ...
    case water_sensor_mesh_StoreAndForward_RequestResponse_CLIENT_HISTORY:
        if (is_server) {
            requests_history++;
            ...
            if (this->busy || channels.isDefaultChannel(mp.channel)) {
                ...
                if ((p->which_variant == water_sensor_mesh_StoreAndForward_history_tag) && (p->variant.history.window > 0)) {
    ...
    case water_sensor_mesh_StoreAndForward_RequestResponse_CLIENT_PING:
        if (is_server) {
            // respond with a ROUTER PONG
            storeForwardModule->sendMessage(getFrom(&mp), water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_PONG);
        }
        break;

    case water_sensor_mesh_StoreAndForward_RequestResponse_CLIENT_PONG:
        if (is_server) {
            // NodeDB is already updated
        }
        break;

    case water_sensor_mesh_StoreAndForward_RequestResponse_CLIENT_STATS:
        if (is_server) {
            LOG_INFO("Client Request to send STATS");
            if (this->busy) {
                storeForwardModule->sendMessage(getFrom(&mp), water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_BUSY);
    ...
    case water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_ERROR:
    case water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_BUSY:
        if (is_client) {
            ...
            retry_delay = millis() + getNumAvailablePackets(this->busyTo, this->last_time) * packetTimeMax *
                                         (water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_ERROR ? 2 : 1);
        }
        break;

    case water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_PONG:
    // A router responded, this is equal to receiving a heartbeat
    case water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_HEARTBEAT:
        if (is_client) {
            // register heartbeat and interval
            if (p->which_variant == water_sensor_mesh_StoreAndForward_heartbeat_tag) {
    ...
    case water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_PING:
        if (is_client) {
            // respond with a CLIENT PONG
            storeForwardModule->sendMessage(getFrom(&mp), water_sensor_mesh_StoreAndForward_RequestResponse_CLIENT_PONG);
        }
        break;

    case water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_STATS:
        if (is_client) {
            LOG_DEBUG("Router Response STATS");
            // These fields only have informational purpose on a client. Fill them to consume later.
            if (p->which_variant == water_sensor_mesh_StoreAndForward_stats_tag) {
    ...
    case water_sensor_mesh_StoreAndForward_RequestResponse_ROUTER_HISTORY:
        if (is_client) {
            // These fields only have informational purpose on a client. Fill them to consume later.
            if (p->which_variant == water_sensor_mesh_StoreAndForward_history_tag) {
...
StoreForwardModule::StoreForwardModule()
    : concurrency::OSThread("StoreForward"),
      ProtobufModule("StoreForward", water_sensor_mesh_PortNum_STORE_FORWARD_APP, &water_sensor_mesh_StoreAndForward_msg)
{
    ...
    if (moduleConfig.store_forward.enabled) {
        ...
        // Router
        if ((config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER || moduleConfig.store_forward.is_server)) {
