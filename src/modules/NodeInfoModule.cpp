...
bool NodeInfoModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_User *pptr)
{
    auto p = *pptr;
...
void NodeInfoModule::sendOurNodeInfo(NodeNum dest, bool wantReplies, uint8_t channel, bool _shorterTimeout)
{
    ...
    if (p) { // Check whether we didn't ignore it
        p->to = dest;
        p->decoded.want_response = (config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_TRACKER &&
                                    config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_SENSOR) &&
                                   wantReplies;
        if (_shorterTimeout)
            p->priority = water_sensor_mesh_MeshPacket_Priority_DEFAULT;
        else
...
water_sensor_mesh_MeshPacket *NodeInfoModule::allocReply()
{
    ...
    if (!shorterTimeout && lastSentToMesh && Throttle::isWithinTimespanMs(lastSentToMesh, 5 * 60 * 1000)) {
        ...
        water_sensor_mesh_User &u = owner;
        ...
        if (u.role == water_sensor_mesh_Config_DeviceConfig_Role_REPEATER) {
...
NodeInfoModule::NodeInfoModule()
    : ProtobufModule("nodeinfo", water_sensor_mesh_PortNum_NODEINFO_APP, &water_sensor_mesh_User_msg), concurrency::OSThread("NodeInfo")
{
...
int32_t NodeInfoModule::runOnce()
{
    ...
    if (airTime->isTxAllowedAirUtil() && config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_CLIENT_HIDDEN) {
