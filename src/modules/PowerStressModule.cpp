...
PowerStressModule::PowerStressModule()
    : ProtobufModule("powerstress", water_sensor_mesh_PortNum_POWERSTRESS_APP, &water_sensor_mesh_PowerStressMessage_msg),
      concurrency::OSThread("PowerStress")
{
}

bool PowerStressModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &req, water_sensor_mesh_PowerStressMessage *pptr)
{
    ...
    if (config.power.powermon_enables) {
        ...
        switch (p.cmd) {
        case water_sensor_mesh_PowerStressMessage_Opcode_UNSET:
            LOG_ERROR("PowerStress operation unset");
            break;

        case water_sensor_mesh_PowerStressMessage_Opcode_PRINT_INFO:
            printInfo();
        ...
        default:
            if (currentMessage.cmd != water_sensor_mesh_PowerStressMessage_Opcode_UNSET)
...
int32_t PowerStressModule::runOnce()
{
    ...
    if (isRunningCommand) {
        ...
        p.cmd = water_sensor_mesh_PowerStressMessage_Opcode_UNSET;
        p.num_seconds = 0;
        isRunningCommand = false;
        LOG_INFO("S:PS:%u", p.cmd);
    } else {
        if (p.cmd != water_sensor_mesh_PowerStressMessage_Opcode_UNSET) {
            ...
            switch (p.cmd) {
            case water_sensor_mesh_PowerStressMessage_Opcode_LED_ON:
                ledForceOn.set(true);
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_LED_OFF:
                ledForceOn.set(false);
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_GPS_ON:
                // FIXME - implement
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_GPS_OFF:
                // FIXME - implement
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_LORA_OFF:
                // FIXME - implement
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_LORA_RX:
                // FIXME - implement
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_LORA_TX:
                // FIXME - implement
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_SCREEN_OFF:
                // FIXME - implement
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_SCREEN_ON:
                // FIXME - implement
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_BT_OFF:
                setBluetoothEnable(false);
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_BT_ON:
                setBluetoothEnable(true);
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_CPU_DEEPSLEEP:
                doDeepSleep(sleep_msec, true, true);
                break;
            case water_sensor_mesh_PowerStressMessage_Opcode_CPU_FULLON: {
            ...
            case water_sensor_mesh_PowerStressMessage_Opcode_CPU_IDLE:
                // FIXME - implement
                break;
            default:
