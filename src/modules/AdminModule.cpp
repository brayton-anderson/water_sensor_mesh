...
#if defined(ARCH_ESP32) && !WATER_SENSOR_MESH_EXCLUDE_BLUETOOTH
#include "BleOta.h"
#endif
#if defined(ARCH_ESP32) && !WATER_SENSOR_MESH_EXCLUDE_WIFI
#include "WiFiOTA.h"
#endif
#include "Router.h"
 ...
 * @return bool
 */
bool AdminModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_AdminMessage *r)
{
    // if handled == false, then let others look at this message also if they want
    bool handled = false;
    assert(r);
    bool fromOthers = !isFromUs(&mp);
    if (mp.which_payload_variant != water_sensor_mesh_MeshPacket_decoded_tag) {
        return handled;
    }
    water_sensor_mesh_Channel *ch = &channels.getByIndex(mp.channel);
    ...
    if (messageIsResponse(r)) {
        ...
        if (!config.security.admin_channel_enabled) {
            ...
            myReply = allocErrorResponse(water_sensor_mesh_Routing_Error_NOT_AUTHORIZED, &mp);
            return handled;
        }
        ...
        if ((config.security.admin_key[0].size == 32 &&
            ...
            myReply = allocErrorResponse(water_sensor_mesh_Routing_Error_ADMIN_PUBLIC_KEY_UNAUTHORIZED, &mp);
    ...
    if (mp.from != 0 && !messageIsRequest(r) && !messageIsResponse(r)) {
        if (!checkPassKey(r)) {
            ...
            myReply = allocErrorResponse(water_sensor_mesh_Routing_Error_ADMIN_BAD_SESSION_KEY, &mp);
            return handled;
        }
    }
     ...
     * Getters
     */
    case water_sensor_mesh_AdminMessage_get_owner_request_tag:
        LOG_DEBUG("Client got owner");
        handleGetOwner(mp);
        break;

    case water_sensor_mesh_AdminMessage_get_config_request_tag:
        LOG_DEBUG("Client got config");
        handleGetConfig(mp, r->get_config_request);
        break;

    case water_sensor_mesh_AdminMessage_get_module_config_request_tag:
        LOG_DEBUG("Client got module config");
        handleGetModuleConfig(mp, r->get_module_config_request);
        break;

    case water_sensor_mesh_AdminMessage_get_channel_request_tag: {
        uint32_t i = r->get_channel_request - 1;
        LOG_DEBUG("Client got channel %u", i);
        if (i >= MAX_NUM_CHANNELS)
            myReply = allocErrorResponse(water_sensor_mesh_Routing_Error_BAD_REQUEST, &mp);
        else
     ...
     * Setters
     */
    case water_sensor_mesh_AdminMessage_set_owner_tag:
    ...
    case water_sensor_mesh_AdminMessage_set_config_tag:
        LOG_DEBUG("Client set config");
        handleSetConfig(r->set_config);
        break;

    case water_sensor_mesh_AdminMessage_set_module_config_tag:
    ...
    case water_sensor_mesh_AdminMessage_set_channel_tag:
        ...
        break;
    case water_sensor_mesh_AdminMessage_set_ham_mode_tag:
        LOG_DEBUG("Client set ham mode");
        handleSetHamMode(r->set_ham_mode);
        break;
    case water_sensor_mesh_AdminMessage_get_ui_config_request_tag: {
     ...
     * Other
     */
    case water_sensor_mesh_AdminMessage_reboot_seconds_tag: {
        reboot(r->reboot_seconds);
        break;
    }
    case water_sensor_mesh_AdminMessage_reboot_ota_seconds_tag: {
...
#endif
#if !WATER_SENSOR_MESH_EXCLUDE_WIFI
    ...
    case water_sensor_mesh_AdminMessage_shutdown_seconds_tag: {
        int32_t s = r->shutdown_seconds;
        LOG_INFO("Shutdown in %d seconds", s);
        shutdownAtMsec = (s < 0) ? 0 : (millis() + s * 1000);
        break;
    }
    case water_sensor_mesh_AdminMessage_get_device_metadata_request_tag: {
        LOG_INFO("Client got device metadata");
        handleGetDeviceMetadata(mp);
        break;
    }
    case water_sensor_mesh_AdminMessage_factory_reset_config_tag: {
        disableBluetooth();
    ...
    case water_sensor_mesh_AdminMessage_factory_reset_device_tag: {
        disableBluetooth();
    ...
    case water_sensor_mesh_AdminMessage_nodedb_reset_tag: {
        disableBluetooth();
    ...
    case water_sensor_mesh_AdminMessage_store_ui_config_tag: {
        LOG_INFO("Storing device-ui config");
        handleStoreDeviceUIConfig(r->store_ui_config);
        handled = true;
        break;
    }
    case water_sensor_mesh_AdminMessage_begin_edit_settings_tag: {
        LOG_INFO("Begin transaction for editing settings");
        hasOpenEditTransaction = true;
        break;
    }
    case water_sensor_mesh_AdminMessage_commit_edit_settings_tag: {
        disableBluetooth();
    ...
    case water_sensor_mesh_AdminMessage_get_device_connection_status_request_tag: {
        LOG_INFO("Client got device connection status");
        handleGetDeviceConnectionStatus(mp);
        break;
    }
    case water_sensor_mesh_AdminMessage_get_module_config_response_tag: {
        LOG_INFO("Client received a get_module_config response");
        if (fromOthers && r->get_module_config_response.which_payload_variant ==
                              water_sensor_mesh_AdminMessage_ModuleConfigType_REMOTEHARDWARE_CONFIG) {
            handleGetModuleConfigResponse(mp, r);
        }
        break;
    }
    case water_sensor_mesh_AdminMessage_remove_by_nodenum_tag: {
        LOG_INFO("Client received remove_nodenum command");
        nodeDB->removeNodeByNum(r->remove_by_nodenum);
        break;
    }
    case water_sensor_mesh_AdminMessage_add_contact_tag: {
        LOG_INFO("Client received add_contact command");
        nodeDB->addFromContact(r->add_contact);
        break;
    }
    case water_sensor_mesh_AdminMessage_set_favorite_node_tag: {
        LOG_INFO("Client received set_favorite_node command");
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(r->set_favorite_node);
        if (node != NULL) {
    ...
    case water_sensor_mesh_AdminMessage_remove_favorite_node_tag: {
        LOG_INFO("Client received remove_favorite_node command");
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(r->remove_favorite_node);
        if (node != NULL) {
    ...
    case water_sensor_mesh_AdminMessage_set_ignored_node_tag: {
        LOG_INFO("Client received set_ignored_node command");
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(r->set_ignored_node);
        if (node != NULL) {
    ...
    case water_sensor_mesh_AdminMessage_remove_ignored_node_tag: {
        LOG_INFO("Client received remove_ignored_node command");
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(r->remove_ignored_node);
        if (node != NULL) {
    ...
    case water_sensor_mesh_AdminMessage_set_fixed_position_tag: {
        LOG_INFO("Client received set_fixed_position command");
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(nodeDB->getNodeNum());
    ...
    case water_sensor_mesh_AdminMessage_remove_fixed_position_tag: {
    ...
    case water_sensor_mesh_AdminMessage_set_time_only_tag: {
    ...
    case water_sensor_mesh_AdminMessage_enter_dfu_mode_request_tag: {
    ...
    case water_sensor_mesh_AdminMessage_delete_file_request_tag: {
    ...
    case water_sensor_mesh_AdminMessage_backup_preferences_tag: {
        LOG_INFO("Client requesting to backup preferences");
        if (nodeDB->backupPreferences(r->backup_preferences)) {
            myReply = allocErrorResponse(water_sensor_mesh_Routing_Error_NONE, &mp);
        } else {
            myReply = allocErrorResponse(water_sensor_mesh_Routing_Error_BAD_REQUEST, &mp);
        }
        break;
    }
    case water_sensor_mesh_AdminMessage_restore_preferences_tag: {
    ...
    case water_sensor_mesh_AdminMessage_remove_backup_preferences_tag: {
        LOG_INFO("Client requesting to remove backup preferences");
#ifdef FSCom
        if (r->remove_backup_preferences == water_sensor_mesh_AdminMessage_BackupLocation_FLASH) {
            spiLock->lock();
            FSCom.remove(backupFileName);
            spiLock->unlock();
        } else if (r->remove_backup_preferences == water_sensor_mesh_AdminMessage_BackupLocation_SD) {
    ...
    case water_sensor_mesh_AdminMessage_send_input_event_tag: {
        LOG_INFO("Client requesting to send input event");
        handleSendInputEvent(r->send_input_event);
        break;
    }
#ifdef ARCH_PORTDUINO
    case water_sensor_mesh_AdminMessage_exit_simulator_tag:
    ...
    default:
        water_sensor_mesh_AdminMessage res = water_sensor_mesh_AdminMessage_init_default;
    ...
    water_sensor_mesh_AdminMessage observerResponse = water_sensor_mesh_AdminMessage_init_default;
...
void AdminModule::handleGetModuleConfigResponse(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_AdminMessage *r)
{
...
void AdminModule::handleSetOwner(const water_sensor_mesh_User &o)
{
    int changed = 0;
...
void AdminModule::handleSetConfig(const water_sensor_mesh_Config &c)
{
    auto changes = SEGMENT_CONFIG;
    auto existingRole = config.device.role;
    bool isRegionUnset = (config.lora.region == water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET);
    bool requiresReboot = true;

    switch (c.which_payload_variant) {
    case water_sensor_mesh_Config_device_tag:
        LOG_INFO("Set config: Device");
        config.has_device = true;
#if !defined(ARCH_PORTDUINO) && !defined(ARCH_STM32WL) && !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR
        ...
        if (config.device.rebroadcast_mode == water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_NONE &&
            IS_ONE_OF(config.device.role, water_sensor_mesh_Config_DeviceConfig_Role_ROUTER,
                      water_sensor_mesh_Config_DeviceConfig_Role_REPEATER)) {
            config.device.rebroadcast_mode = water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_ALL;
        ...
        if (c.payload_variant.device.role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER_CLIENT) {
            config.device.role = water_sensor_mesh_Config_DeviceConfig_Role_CLIENT;
        ...
        if (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER ||
            config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_ROUTER_LATE ||
            config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_REPEATER) {
            config.device.role = water_sensor_mesh_Config_DeviceConfig_Role_CLIENT;
        }
#endif
        break;
    case water_sensor_mesh_Config_position_tag:
        LOG_INFO("Set config: Position");
        config.has_position = true;
        config.position = c.payload_variant.position;
        // Save nodedb as well in case we got a fixed position packet
        break;
    case water_sensor_mesh_Config_power_tag:
        ...
        break;
    case water_sensor_mesh_Config_network_tag:
        LOG_INFO("Set config: WiFi");
        config.has_network = true;
        config.network = c.payload_variant.network;
        break;
    case water_sensor_mesh_Config_display_tag:
        ...
        if (config.display.screen_on_secs == c.payload_variant.display.screen_on_secs &&
            ...
            requiresReboot = false;
        } else if (config.display.displaymode != water_sensor_mesh_Config_DisplayConfig_DisplayMode_COLOR &&
                   c.payload_variant.display.displaymode == water_sensor_mesh_Config_DisplayConfig_DisplayMode_COLOR) {
        ...
        break;
    case water_sensor_mesh_Config_lora_tag:
        ...
        if (isRegionUnset && config.lora.region > water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET) {
        ...
        break;
    case water_sensor_mesh_Config_bluetooth_tag:
        LOG_INFO("Set config: Bluetooth");
        config.has_bluetooth = true;
        config.bluetooth = c.payload_variant.bluetooth;
        break;
    case water_sensor_mesh_Config_security_tag:
        LOG_INFO("Set config: Security");
        config.security = c.payload_variant.security;
#if !(WATER_SENSOR_MESH_EXCLUDE_PKI_KEYGEN) && !(WATER_SENSOR_MESH_EXCLUDE_PKI)
        // If the client set the key to blank, go ahead and regenerate so long as we're not in ham mode
        if (!owner.is_licensed && config.lora.region != water_sensor_mesh_Config_LoRaConfig_RegionCode_UNSET) {
            if (config.security.private_key.size != 32) {
                crypto->generateKeyPair(config.security.public_key.bytes, config.security.private_key.bytes);
        ...
        break;
    case water_sensor_mesh_Config_device_ui_tag:
...
bool AdminModule::handleSetModuleConfig(const water_sensor_mesh_ModuleConfig &c)
{
    if (!hasOpenEditTransaction)
        disableBluetooth();
    switch (c.which_payload_variant) {
    case water_sensor_mesh_ModuleConfig_mqtt_tag:
#if WATER_SENSOR_MESH_EXCLUDE_MQTT
        LOG_WARN("Set module config: WATER_SENSOR_MESH_EXCLUDE_MQTT is defined. Not setting MQTT config");
        return false;
#else
...
#endif
        break;
    case water_sensor_mesh_ModuleConfig_serial_tag:
        LOG_INFO("Set module config: Serial");
        moduleConfig.has_serial = true;
        moduleConfig.serial = c.payload_variant.serial;
        break;
    case water_sensor_mesh_ModuleConfig_external_notification_tag:
    ...
    case water_sensor_mesh_ModuleConfig_remote_hardware_tag:
        ...
        break;
    case water_sensor_mesh_ModuleConfig_neighbor_info_tag:
...
void AdminModule::handleSetChannel(const water_sensor_mesh_Channel &cc)
{
...
void AdminModule::handleGetOwner(const water_sensor_mesh_MeshPacket &req)
{
    if (req.decoded.want_response) {
        ...
        res.which_payload_variant = water_sensor_mesh_AdminMessage_get_owner_response_tag;
        setPassKey(&res);
        myReply = allocDataProtobuf(res);
    }
}

void AdminModule::handleGetConfig(const water_sensor_mesh_MeshPacket &req, const uint32_t configType)
{
    water_sensor_mesh_AdminMessage res = water_sensor_mesh_AdminMessage_init_default;

    if (req.decoded.want_response) {
        switch (configType) {
        case water_sensor_mesh_AdminMessage_ConfigType_DEVICE_CONFIG:
            LOG_INFO("Get config: Device");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_device_tag;
            res.get_config_response.payload_variant.device = config.device;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_POSITION_CONFIG:
            LOG_INFO("Get config: Position");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_position_tag;
            res.get_config_response.payload_variant.position = config.position;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_POWER_CONFIG:
            LOG_INFO("Get config: Power");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_power_tag;
            res.get_config_response.payload_variant.power = config.power;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_NETWORK_CONFIG:
            LOG_INFO("Get config: Network");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_network_tag;
            ...
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_DISPLAY_CONFIG:
            LOG_INFO("Get config: Display");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_display_tag;
            res.get_config_response.payload_variant.display = config.display;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_LORA_CONFIG:
            LOG_INFO("Get config: LoRa");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_lora_tag;
            res.get_config_response.payload_variant.lora = config.lora;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_BLUETOOTH_CONFIG:
            LOG_INFO("Get config: Bluetooth");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_bluetooth_tag;
            res.get_config_response.payload_variant.bluetooth = config.bluetooth;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_SECURITY_CONFIG:
            LOG_INFO("Get config: Security");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_security_tag;
            res.get_config_response.payload_variant.security = config.security;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_SESSIONKEY_CONFIG:
            LOG_INFO("Get config: Sessionkey");
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_sessionkey_tag;
            break;
        case water_sensor_mesh_AdminMessage_ConfigType_DEVICEUI_CONFIG:
            // NOOP! This is handled by handleGetDeviceUIConfig
            res.get_config_response.which_payload_variant = water_sensor_mesh_Config_device_ui_tag;
            break;
        }
        ...
        res.which_payload_variant = water_sensor_mesh_AdminMessage_get_config_response_tag;
        setPassKey(&res);
        myReply = allocDataProtobuf(res);
    }
}

void AdminModule::handleGetModuleConfig(const water_sensor_mesh_MeshPacket &req, const uint32_t configType)
{
    water_sensor_mesh_AdminMessage res = water_sensor_mesh_AdminMessage_init_default;

    if (req.decoded.want_response) {
        switch (configType) {
        case water_sensor_mesh_AdminMessage_ModuleConfigType_MQTT_CONFIG:
            LOG_INFO("Get module config: MQTT");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_mqtt_tag;
            res.get_module_config_response.payload_variant.mqtt = moduleConfig.mqtt;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_SERIAL_CONFIG:
            LOG_INFO("Get module config: Serial");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_serial_tag;
            res.get_module_config_response.payload_variant.serial = moduleConfig.serial;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_EXTNOTIF_CONFIG:
            LOG_INFO("Get module config: External Notification");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_external_notification_tag;
            res.get_module_config_response.payload_variant.external_notification = moduleConfig.external_notification;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_STOREFORWARD_CONFIG:
            LOG_INFO("Get module config: Store & Forward");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_store_forward_tag;
            res.get_module_config_response.payload_variant.store_forward = moduleConfig.store_forward;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_RANGETEST_CONFIG:
            LOG_INFO("Get module config: Range Test");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_range_test_tag;
            res.get_module_config_response.payload_variant.range_test = moduleConfig.range_test;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_TELEMETRY_CONFIG:
            LOG_INFO("Get module config: Telemetry");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_telemetry_tag;
            res.get_module_config_response.payload_variant.telemetry = moduleConfig.telemetry;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_CANNEDMSG_CONFIG:
            LOG_INFO("Get module config: Canned Message");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_canned_message_tag;
            res.get_module_config_response.payload_variant.canned_message = moduleConfig.canned_message;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_AUDIO_CONFIG:
            LOG_INFO("Get module config: Audio");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_audio_tag;
            res.get_module_config_response.payload_variant.audio = moduleConfig.audio;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_REMOTEHARDWARE_CONFIG:
            LOG_INFO("Get module config: Remote Hardware");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_remote_hardware_tag;
            res.get_module_config_response.payload_variant.remote_hardware = moduleConfig.remote_hardware;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_NEIGHBORINFO_CONFIG:
            LOG_INFO("Get module config: Neighbor Info");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_neighbor_info_tag;
            res.get_module_config_response.payload_variant.neighbor_info = moduleConfig.neighbor_info;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_DETECTIONSENSOR_CONFIG:
            LOG_INFO("Get module config: Detection Sensor");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_detection_sensor_tag;
            res.get_module_config_response.payload_variant.detection_sensor = moduleConfig.detection_sensor;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_AMBIENTLIGHTING_CONFIG:
            LOG_INFO("Get module config: Ambient Lighting");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_ambient_lighting_tag;
            res.get_module_config_response.payload_variant.ambient_lighting = moduleConfig.ambient_lighting;
            break;
        case water_sensor_mesh_AdminMessage_ModuleConfigType_PAXCOUNTER_CONFIG:
            LOG_INFO("Get module config: Paxcounter");
            res.get_module_config_response.which_payload_variant = water_sensor_mesh_ModuleConfig_paxcounter_tag;
        ...
        // and useful for users to know current provisioning) hideSecret(r.get_radio_response.preferences.wifi_password);
        // r.get_config_response.which_payloadVariant = Config_ModuleConfig_telemetry_tag;
        res.which_payload_variant = water_sensor_mesh_AdminMessage_get_module_config_response_tag;
        setPassKey(&res);
        myReply = allocDataProtobuf(res);
    }
}

void AdminModule::handleGetNodeRemoteHardwarePins(const water_sensor_mesh_MeshPacket &req)
{
    water_sensor_mesh_AdminMessage r = water_sensor_mesh_AdminMessage_init_default;
    r.which_payload_variant = water_sensor_mesh_AdminMessage_get_node_remote_hardware_pins_response_tag;
    for (uint8_t i = 0; i < devicestate.node_remote_hardware_pins_count; i++) {
        if (devicestate.node_remote_hardware_pins[i].node_num == 0 || !devicestate.node_remote_hardware_pins[i].has_pin) {
            continue;
        }
    ...
    for (uint8_t i = 0; i < moduleConfig.remote_hardware.available_pins_count; i++) {
        ...
        water_sensor_mesh_NodeRemoteHardwarePin nodePin = water_sensor_mesh_NodeRemoteHardwarePin_init_default;
...
void AdminModule::handleGetDeviceMetadata(const water_sensor_mesh_MeshPacket &req)
{
    water_sensor_mesh_AdminMessage r = water_sensor_mesh_AdminMessage_init_default;
    r.get_device_metadata_response = getDeviceMetadata();
    r.which_payload_variant = water_sensor_mesh_AdminMessage_get_device_metadata_response_tag;
    setPassKey(&r);
    myReply = allocDataProtobuf(r);
}

void AdminModule::handleGetDeviceConnectionStatus(const water_sensor_mesh_MeshPacket &req)
{
    water_sensor_mesh_AdminMessage r = water_sensor_mesh_AdminMessage_init_default;

    water_sensor_mesh_DeviceConnectionStatus conn = water_sensor_mesh_DeviceConnectionStatus_init_zero;
...
#ifndef WATER_SENSOR_MESH_EXCLUDE_MQTT
...
#if !WATER_SENSOR_MESH_EXCLUDE_POWER_FSM
    ...
    r.which_payload_variant = water_sensor_mesh_AdminMessage_get_device_connection_status_response_tag;
    setPassKey(&r);
    myReply = allocDataProtobuf(r);
}

void AdminModule::handleGetChannel(const water_sensor_mesh_MeshPacket &req, uint32_t channelIndex)
{
    if (req.decoded.want_response) {
        // We create the reply here
        water_sensor_mesh_AdminMessage r = water_sensor_mesh_AdminMessage_init_default;
        r.get_channel_response = channels.getByIndex(channelIndex);
        r.which_payload_variant = water_sensor_mesh_AdminMessage_get_channel_response_tag;
        setPassKey(&r);
        myReply = allocDataProtobuf(r);
    }
}

void AdminModule::handleGetDeviceUIConfig(const water_sensor_mesh_MeshPacket &req)
{
    water_sensor_mesh_AdminMessage r = water_sensor_mesh_AdminMessage_init_default;
    r.which_payload_variant = water_sensor_mesh_AdminMessage_get_ui_config_response_tag;
...
void AdminModule::handleStoreDeviceUIConfig(const water_sensor_mesh_DeviceUIConfig &uicfg)
{
    nodeDB->saveProto("/prefs/uiconfig.proto", water_sensor_mesh_DeviceUIConfig_size, &water_sensor_mesh_DeviceUIConfig_msg, &uicfg);
}

void AdminModule::handleSetHamMode(const water_sensor_mesh_HamParameters &p)
{
    ...
    config.device.rebroadcast_mode = water_sensor_mesh_Config_DeviceConfig_RebroadcastMode_LOCAL_ONLY;
...
AdminModule::AdminModule() : ProtobufModule("Admin", water_sensor_mesh_PortNum_ADMIN_APP, &water_sensor_mesh_AdminMessage_msg)
{
    // restrict to the admin channel for rx
    // boundChannel = Channels::adminChannel;
}

void AdminModule::setPassKey(water_sensor_mesh_AdminMessage *res)
{
...
bool AdminModule::checkPassKey(water_sensor_mesh_AdminMessage *res)
...
bool AdminModule::messageIsResponse(const water_sensor_mesh_AdminMessage *r)
{
    if (r->which_payload_variant == water_sensor_mesh_AdminMessage_get_channel_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_owner_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_config_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_module_config_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_canned_message_module_messages_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_device_metadata_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_ringtone_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_device_connection_status_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_node_remote_hardware_pins_response_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_ui_config_response_tag)
        return true;
...
bool AdminModule::messageIsRequest(const water_sensor_mesh_AdminMessage *r)
{
    if (r->which_payload_variant == water_sensor_mesh_AdminMessage_get_channel_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_owner_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_config_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_module_config_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_canned_message_module_messages_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_device_metadata_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_ringtone_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_device_connection_status_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_node_remote_hardware_pins_request_tag ||
        r->which_payload_variant == water_sensor_mesh_AdminMessage_get_ui_config_request_tag)
        return true;
    else
        return false;
}

void AdminModule::handleSendInputEvent(const water_sensor_mesh_AdminMessage_InputEvent &inputEvent)
{
...
#if !defined(WATER_SENSOR_MESH_EXCLUDE_INPUTBROKER)
...
{
    water_sensor_mesh_ClientNotification *cn = clientNotificationPool.allocZeroed();
    cn->level = water_sensor_mesh_LogRecord_Level_WARNING;
