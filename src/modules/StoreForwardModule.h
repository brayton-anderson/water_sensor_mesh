...
struct PacketHistoryStruct {
    ...
    uint32_t reply_id;
    bool emoji;
    uint8_t payload[water_sensor_mesh_Constants_DATA_PAYLOAD_LEN];
    pb_size_t payload_size;
};

class StoreForwardModule : private concurrency::OSThread, public ProtobufModule<water_sensor_mesh_StoreAndForward>
{
    bool busy = 0;
    uint32_t busyTo = 0;
    char routerMessage[water_sensor_mesh_Constants_DATA_PAYLOAD_LEN] = {0};
  ...
  public:
     ...
     */
    void historyAdd(const water_sensor_mesh_MeshPacket &mp);
    ...
    water_sensor_mesh_MeshPacket *preparePayload(NodeNum dest, uint32_t packetHistory_index, bool local = false);
    void sendMessage(NodeNum dest, const water_sensor_mesh_StoreAndForward &payload);
    void sendMessage(NodeNum dest, water_sensor_mesh_StoreAndForward_RequestResponse rr);
    void sendErrorTextMessage(NodeNum dest, bool want_response);
    water_sensor_mesh_MeshPacket *getForPhone();
    ...
    */
    virtual bool wantPacket(const water_sensor_mesh_MeshPacket *p) override
    {
        ...
        case water_sensor_mesh_PortNum_STORE_FORWARD_APP:
            return true;
        default:
            return false;
        }
  ...
  protected:
    ...
    */
    ...
    virtual bool handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_StoreAndForward *p);
};
