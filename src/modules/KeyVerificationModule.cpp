...
KeyVerificationModule::KeyVerificationModule()
    : ProtobufModule("KeyVerification", water_sensor_mesh_PortNum_KEY_VERIFICATION_APP, &water_sensor_mesh_KeyVerification_msg)
{
    ourPortNum = water_sensor_mesh_PortNum_KEY_VERIFICATION_APP;
}

AdminMessageHandleResult KeyVerificationModule::handleAdminMessageForModule(const water_sensor_mesh_MeshPacket &mp,
                                                                            water_sensor_mesh_AdminMessage *request,
                                                                            water_sensor_mesh_AdminMessage *response)
{
    updateState();
    if (request->which_payload_variant == water_sensor_mesh_AdminMessage_key_verification_tag && mp.from == 0) {
        LOG_WARN("Handling Key Verification Admin Message type %u", request->key_verification.message_type);

        if (request->key_verification.message_type == water_sensor_mesh_KeyVerificationAdmin_MessageType_INITIATE_VERIFICATION &&
            ...
            sendInitialRequest(request->key_verification.remote_nodenum);
                       ...
                       water_sensor_mesh_KeyVerificationAdmin_MessageType_PROVIDE_SECURITY_NUMBER &&
        ...
        } else if (request->key_verification.message_type == water_sensor_mesh_KeyVerificationAdmin_MessageType_DO_VERIFY &&
                   request->key_verification.nonce == currentNonce) {
            auto remoteNodePtr = nodeDB->getMeshNode(currentRemoteNode);
            remoteNodePtr->bitfield |= NODEINFO_BITFIELD_IS_KEY_MANUALLY_VERIFIED_MASK;
            resetToIdle();
        } else if (request->key_verification.message_type == water_sensor_mesh_KeyVerificationAdmin_MessageType_DO_NOT_VERIFY) {
            resetToIdle();
        }
...
bool KeyVerificationModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_KeyVerification *r)
{
    updateState();
    ...
    if (currentState == KEY_VERIFICATION_IDLE) {
        ...
        cn->which_payload_variant = water_sensor_mesh_ClientNotification_key_verification_number_request_tag;
        ...
        if (memcmp(hash1, r->hash1.bytes, 32) == 0) {
            ...
            cn->which_payload_variant = water_sensor_mesh_ClientNotification_key_verification_final_tag;
...
bool KeyVerificationModule::sendInitialRequest(NodeNum remoteNode)
{
    ...
    water_sensor_mesh_KeyVerification KeyVerification = water_sensor_mesh_KeyVerification_init_zero;
    KeyVerification.nonce = currentNonce;
    KeyVerification.hash2.size = 0;
    KeyVerification.hash1.size = 0;
    water_sensor_mesh_MeshPacket *p = allocDataProtobuf(KeyVerification);
    p->to = remoteNode;
    p->channel = 0;
...
water_sensor_mesh_MeshPacket *KeyVerificationModule::allocReply()
{
    SHA256 hash;
    ...
    water_sensor_mesh_KeyVerification scratch;
    water_sensor_mesh_KeyVerification response;
    water_sensor_mesh_MeshPacket *responsePacket = nullptr;
    pb_decode_from_bytes(p.payload.bytes, p.payload.size, &water_sensor_mesh_KeyVerification_msg, &scratch);
    ...
    cn->which_payload_variant = water_sensor_mesh_ClientNotification_key_verification_number_inform_tag;
...
void KeyVerificationModule::processSecurityNumber(uint32_t incomingNumber)
{
    ...
    water_sensor_mesh_NodeInfoLite *remoteNodePtr = nullptr;
