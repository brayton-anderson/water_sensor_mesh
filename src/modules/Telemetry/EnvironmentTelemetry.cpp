...
#if !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR_EXTERNAL
    ...
    */
    ...
    if (firstTime) {
...
#elif !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR_EXTERNAL
        ...
        if (((lastSentToMesh == 0) ||
            ...
            airTime->isTxAllowedChannelUtil(config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_SENSOR) &&
...
void EnvironmentTelemetryModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    const water_sensor_mesh_Data &p = lastMeasurementPacket->decoded;
    water_sensor_mesh_Telemetry telemetry;
    if (!pb_decode_from_bytes(p.payload.bytes, p.payload.size, &water_sensor_mesh_Telemetry_msg, &telemetry)) {
        display->drawString(x, currentY, "No Telemetry");
        return;
    }
...
bool EnvironmentTelemetryModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Telemetry *t)
{
    if (t->which_variant == water_sensor_mesh_Telemetry_environment_metrics_tag) {
#ifdef DEBUG_PORT
...
bool EnvironmentTelemetryModule::getEnvironmentTelemetry(water_sensor_mesh_Telemetry *m)
{
    bool valid = true;
    bool hasSensor = false;
    m->time = getTime();
    m->which_variant = water_sensor_mesh_Telemetry_environment_metrics_tag;
    m->variant.environment_metrics = water_sensor_mesh_EnvironmentMetrics_init_zero;
    ...
    if (aht10Sensor.hasSensor()) {
        if (!bmp280Sensor.hasSensor() && !bmp3xxSensor.hasSensor()) {
            ...
            water_sensor_mesh_Telemetry m_ahtx = water_sensor_mesh_Telemetry_init_zero;
...
water_sensor_mesh_MeshPacket *EnvironmentTelemetryModule::allocReply()
{
    if (currentRequest) {
        ...
        if (pb_decode_from_bytes(p.payload.bytes, p.payload.size, &water_sensor_mesh_Telemetry_msg, &scratch)) {
            decoded = &scratch;
        } else {
        ...
        if (decoded->which_variant == water_sensor_mesh_Telemetry_environment_metrics_tag) {
...
bool EnvironmentTelemetryModule::sendTelemetry(NodeNum dest, bool phoneOnly)
{
    ...
    if (getEnvironmentTelemetry(&m)) {
        ...
        water_sensor_mesh_MeshPacket *p = allocDataProtobuf(m);
        p->to = dest;
        ...
        if (phoneOnly) {
            ...
            if (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_SENSOR && config.power.is_power_saving) {
...
AdminMessageHandleResult EnvironmentTelemetryModule::handleAdminMessageForModule(const water_sensor_mesh_MeshPacket &mp,
