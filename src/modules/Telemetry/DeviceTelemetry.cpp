...
int32_t DeviceTelemetryModule::runOnce()
{
    refreshUptime();
    bool isImpoliteRole =
        IS_ONE_OF(config.device.role, water_sensor_mesh_Config_DeviceConfig_Role_SENSOR, water_sensor_mesh_Config_DeviceConfig_Role_ROUTER);
    if (((lastSentToMesh == 0) ||
        ...
        config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_CLIENT_HIDDEN) {
        sendTelemetry();
...
bool DeviceTelemetryModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Telemetry *t)
{
    ...
    if (t->which_variant == water_sensor_mesh_Telemetry_device_metrics_tag) {
#ifdef DEBUG_PORT
...
water_sensor_mesh_MeshPacket *DeviceTelemetryModule::allocReply()
{
    if (currentRequest) {
        ...
        } else if (decoded->which_variant == water_sensor_mesh_Telemetry_local_stats_tag) {
...
water_sensor_mesh_Telemetry DeviceTelemetryModule::getDeviceTelemetry()
{
    water_sensor_mesh_Telemetry t = water_sensor_mesh_Telemetry_init_zero;
    t.which_variant = water_sensor_mesh_Telemetry_device_metrics_tag;
    t.time = getTime();
    t.variant.device_metrics = water_sensor_mesh_DeviceMetrics_init_zero;
...
water_sensor_mesh_Telemetry DeviceTelemetryModule::getLocalStatsTelemetry()
{
    water_sensor_mesh_Telemetry telemetry = water_sensor_mesh_Telemetry_init_zero;
    telemetry.which_variant = water_sensor_mesh_Telemetry_local_stats_tag;
    telemetry.variant.local_stats = water_sensor_mesh_LocalStats_init_zero;
...
{
    water_sensor_mesh_MeshPacket *p = allocDataProtobuf(getLocalStatsTelemetry());
...
{
    water_sensor_mesh_Telemetry telemetry = getDeviceTelemetry();
    ...
    water_sensor_mesh_MeshPacket *p = allocDataProtobuf(telemetry);
    p->to = dest;
    p->decoded.want_response = false;
    p->priority = water_sensor_mesh_MeshPacket_Priority_BACKGROUND;
