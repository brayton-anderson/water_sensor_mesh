...
bool HostMetricsModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Telemetry *t)
{
    ...
    if (t->which_variant == water_sensor_mesh_Telemetry_host_metrics_tag) {
#ifdef DEBUG_PORT
...
/*
water_sensor_mesh_MeshPacket *HostMetricsModule::allocReply()
{
    if (currentRequest) {
        ...
        if (pb_decode_from_bytes(p.payload.bytes, p.payload.size, &water_sensor_mesh_HostMetrics_msg, &scratch)) {
            decoded = &scratch;
        } else {
        ...
        if (decoded->which_variant == water_sensor_mesh_Telemetry_host_metrics_tag) {
...
#if ARCH_PORTDUINO
water_sensor_mesh_Telemetry HostMetricsModule::getHostMetrics()
{
    std::string file_line;
    water_sensor_mesh_Telemetry t = water_sensor_mesh_Telemetry_init_zero;
    t.which_variant = water_sensor_mesh_Telemetry_host_metrics_tag;
    t.variant.host_metrics = water_sensor_mesh_HostMetrics_init_zero;
...
{
    water_sensor_mesh_Telemetry telemetry = getHostMetrics();
