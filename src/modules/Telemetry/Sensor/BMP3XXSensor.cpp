...
#if !WATER_SENSOR_MESH_EXCLUDE_ENVIRONMENTAL_SENSOR && __has_include(<Adafruit_BMP3XX.h>)

#include "BMP3XXSensor.h"

BMP3XXSensor::BMP3XXSensor() : TelemetrySensor(water_sensor_mesh_TelemetrySensorType_BMP3XX, "BMP3XX") {}
...
bool BMP3XXSensor::getMetrics(water_sensor_mesh_Telemetry *measurement)
{
    if (bmp3xx == nullptr) {
        bmp3xx = BMP3XXSingleton::GetInstance();
    }
    if ((int)measurement->which_variant == water_sensor_mesh_Telemetry_environment_metrics_tag) {
        bmp3xx->performReading();
