...
void HealthTelemetryModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    water_sensor_mesh_Telemetry lastMeasurement;
    uint32_t agoSecs = service->GetTimeSinceMeshPacket(lastMeasurementPacket);
    const char *lastSender = getSenderShortName(*lastMeasurementPacket);

    const water_sensor_mesh_Data &p = lastMeasurementPacket->decoded;
    if (!pb_decode_from_bytes(p.payload.bytes, p.payload.size, &water_sensor_mesh_Telemetry_msg, &lastMeasurement)) {
...
bool HealthTelemetryModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Telemetry *t)
{
    if (t->which_variant == water_sensor_mesh_Telemetry_health_metrics_tag) {
#ifdef DEBUG_PORT
...
bool HealthTelemetryModule::getHealthTelemetry(water_sensor_mesh_Telemetry *m)
{
    bool valid = true;
    bool hasSensor = false;
    m->time = getTime();
    m->which_variant = water_sensor_mesh_Telemetry_health_metrics_tag;
    m->variant.health_metrics = water_sensor_mesh_HealthMetrics_init_zero;
...
water_sensor_mesh_MeshPacket *HealthTelemetryModule::allocReply()
{
    if (currentRequest) {
        ...
        if (decoded->which_variant == water_sensor_mesh_Telemetry_health_metrics_tag) {
