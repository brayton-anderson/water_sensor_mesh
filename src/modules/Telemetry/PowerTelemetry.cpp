...
bool PowerTelemetryModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Telemetry *t)
{
    if (t->which_variant == water_sensor_mesh_Telemetry_power_metrics_tag) {
#ifdef DEBUG_PORT
...
bool PowerTelemetryModule::getPowerTelemetry(water_sensor_mesh_Telemetry *m)
{
    bool valid = false;
    m->time = getTime();
    m->which_variant = water_sensor_mesh_Telemetry_power_metrics_tag;

    m->variant.power_metrics = water_sensor_mesh_PowerMetrics_init_zero;
#if HAS_TELEMETRY
    if (ina219Sensor.hasSensor())
...
water_sensor_mesh_MeshPacket *PowerTelemetryModule::allocReply()
{
    if (currentRequest) {
        ...
        if (decoded->which_variant == water_sensor_mesh_Telemetry_power_metrics_tag) {
