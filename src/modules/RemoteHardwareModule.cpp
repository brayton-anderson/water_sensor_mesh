...
RemoteHardwareModule::RemoteHardwareModule()
    : ProtobufModule("remotehardware", water_sensor_mesh_PortNum_REMOTE_HARDWARE_APP, &water_sensor_mesh_HardwareMessage_msg),
...
bool RemoteHardwareModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &req, water_sensor_mesh_HardwareMessage *pptr)
{
    if (moduleConfig.remote_hardware.enabled) {
        ...
        switch (p.type) {
        case water_sensor_mesh_HardwareMessage_Type_WRITE_GPIOS: {
        ...
        case water_sensor_mesh_HardwareMessage_Type_READ_GPIOS: {
            uint64_t res = digitalReads(p.gpio_mask, availablePins);

            // Send the reply
            water_sensor_mesh_HardwareMessage r = water_sensor_mesh_HardwareMessage_init_default;
            r.type = water_sensor_mesh_HardwareMessage_Type_READ_GPIOS_REPLY;
            r.gpio_value = res;
            r.gpio_mask = p.gpio_mask;
            water_sensor_mesh_MeshPacket *p2 = allocDataProtobuf(r);
            setReplyTo(p2, req);
            myReply = p2;
            break;
        }

        case water_sensor_mesh_HardwareMessage_Type_WATCH_GPIOS: {
        ...
        case water_sensor_mesh_HardwareMessage_Type_READ_GPIOS_REPLY:
        case water_sensor_mesh_HardwareMessage_Type_GPIOS_CHANGED:
    ...
    if (moduleConfig.remote_hardware.enabled && watchGpios) {
        ...
        if (!Throttle::isWithinTimespanMs(lastWatchMsec, WATCH_INTERVAL_MSEC)) {
            ...
            if (curVal != previousWatch) {
                ...
                water_sensor_mesh_HardwareMessage r = water_sensor_mesh_HardwareMessage_init_default;
                r.type = water_sensor_mesh_HardwareMessage_Type_GPIOS_CHANGED;
                r.gpio_value = curVal;
                water_sensor_mesh_MeshPacket *p = allocDataProtobuf(r);
