 ...
 *
 * Documentation:
 * https://water_sensor_mesh.org/docs/configuration/module/external-notification
 *
 * @author Jm Casler & Water_Sensor_Mesh Team
 * @date [Insert Date]
 */
...
#include "main.h"
#include "mesh/generated/water_sensor_mesh/rtttl.pb.h"
#include <Arduino.h>
...
water_sensor_mesh_RTTTLConfig rtttlConfig;
...
{
    if (config.device.buzzer_mode != water_sensor_mesh_Config_DeviceConfig_BuzzerMode_DISABLED &&
        config.device.buzzer_mode != water_sensor_mesh_Config_DeviceConfig_BuzzerMode_SYSTEM_ONLY) {
        return true;
    }
    return false;
}

bool ExternalNotificationModule::wantPacket(const water_sensor_mesh_MeshPacket *p)
{
...
ExternalNotificationModule::ExternalNotificationModule()
    : SinglePortModule("ExternalNotificationModule", water_sensor_mesh_PortNum_TEXT_MESSAGE_APP),
      concurrency::OSThread("ExternalNotification")
{
    ...
    */
    ...
    if (moduleConfig.external_notification.enabled) {
        ...
        if (nodeDB->loadProto(rtttlConfigFile, water_sensor_mesh_RTTTLConfig_size, sizeof(water_sensor_mesh_RTTTLConfig),
                              &water_sensor_mesh_RTTTLConfig_msg, &rtttlConfig) != LoadFileResult::LOAD_SUCCESS) {
...
ProcessMessage ExternalNotificationModule::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
 ...
 */
AdminMessageHandleResult ExternalNotificationModule::handleAdminMessageForModule(const water_sensor_mesh_MeshPacket &mp,
                                                                                 ...
                                                                                 water_sensor_mesh_AdminMessage *response)
{
    ...
    case water_sensor_mesh_AdminMessage_get_ringtone_request_tag:
        LOG_INFO("Client getting ringtone");
        this->handleGetRingtone(mp, response);
        result = AdminMessageHandleResult::HANDLED_WITH_RESPONSE;
        break;

    case water_sensor_mesh_AdminMessage_set_ringtone_message_tag:
...
void ExternalNotificationModule::handleGetRingtone(const water_sensor_mesh_MeshPacket &req, water_sensor_mesh_AdminMessage *response)
{
    LOG_INFO("*** handleGetRingtone");
    if (req.decoded.want_response) {
        response->which_payload_variant = water_sensor_mesh_AdminMessage_get_ringtone_response_tag;
...
void ExternalNotificationModule::handleSetRingtone(const char *from_msg)
{
    ...
    if (changed) {
        nodeDB->saveProto(rtttlConfigFile, water_sensor_mesh_RTTTLConfig_size, &water_sensor_mesh_RTTTLConfig_msg, &rtttlConfig);
    }
}
