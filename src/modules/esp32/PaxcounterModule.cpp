...
#if defined(ARCH_ESP32) && !WATER_SENSOR_MESH_EXCLUDE_PAXCOUNTER
#include "Default.h"
...
PaxcounterModule::PaxcounterModule()
    : concurrency::OSThread("Paxcounter"),
      ProtobufModule("paxcounter", water_sensor_mesh_PortNum_PAXCOUNTER_APP, &water_sensor_mesh_Paxcount_msg)
{
}
...
bool PaxcounterModule::sendInfo(NodeNum dest)
{
    ...
    water_sensor_mesh_Paxcount pl = water_sensor_mesh_Paxcount_init_default;
    pl.wifi = count_from_libpax.wifi_count;
    pl.ble = count_from_libpax.ble_count;
    pl.uptime = millis() / 1000;

    water_sensor_mesh_MeshPacket *p = allocDataProtobuf(pl);
    p->to = dest;
...
bool PaxcounterModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Paxcount *p)
{
    return false; // Let others look at this message also if they want. We don't do anything with received packets.
}

water_sensor_mesh_MeshPacket *PaxcounterModule::allocReply()
{
    water_sensor_mesh_Paxcount pl = water_sensor_mesh_Paxcount_init_default;
    pl.wifi = count_from_libpax.wifi_count;
    pl.ble = count_from_libpax.ble_count;
