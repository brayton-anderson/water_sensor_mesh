...
AudioModule::AudioModule() : SinglePortModule("Audio", water_sensor_mesh_PortNum_AUDIO_APP), concurrency::OSThread("Audio")
{
    ...
    if ((moduleConfig.audio.codec2_enabled) && (myRegion->audioPermitted)) {
        ...
        encode_frame_num = (water_sensor_mesh_Constants_DATA_PAYLOAD_LEN - sizeof(tx_header)) / encode_codec_size;
...
water_sensor_mesh_MeshPacket *AudioModule::allocReply()
{
...
void AudioModule::sendPayload(NodeNum dest, bool wantReplies)
{
    ...
    p->priority = water_sensor_mesh_MeshPacket_Priority_MAX; // Audio is important, because realtime
...
ProcessMessage AudioModule::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
    if ((moduleConfig.audio.codec2_enabled) && (myRegion->audioPermitted)) {
        auto &p = mp.decoded;
        if (!isFromUs(&mp)) {
