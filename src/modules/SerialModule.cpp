...
/*
    ...
    Need help with this module? Post your question on the Water_Sensor_Mesh Discourse:
       https://water_sensor_mesh.discourse.group
...
SerialModuleRadio::SerialModuleRadio() : MeshModule("SerialModuleRadio")
{
    ...
    case water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_TEXTMSG:
        ourPortNum = water_sensor_mesh_PortNum_TEXT_MESSAGE_APP;
        break;
    case water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_NMEA:
    case water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_CALTOPO:
        ourPortNum = water_sensor_mesh_PortNum_POSITION_APP;
        break;
    default:
        ourPortNum = water_sensor_mesh_PortNum_SERIAL_APP;
...
int32_t SerialModule::runOnce()
{
    ...
    */
    ...
    // moduleConfig.serial.mode = water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_CALTOPO;
    ...
    if (moduleConfig.serial.override_console_serial_port || (moduleConfig.serial.rxd && moduleConfig.serial.txd)) {
        if (firstTime) {
            ...
            if (moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_PROTO) {
                emitRebooted();
            }
        } else {
            if (moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_PROTO) {
                return runOncePart();
            } else if ((moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_NMEA) && HAS_GPS) {
            ...
            } else if ((moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_CALTOPO) && HAS_GPS) {
                if (!Throttle::isWithinTimespanMs(lastNmeaTime, 10000)) {
                    lastNmeaTime = millis();
                    uint32_t readIndex = 0;
                    const water_sensor_mesh_NodeInfoLite *tempNodeInfo = nodeDB->readNextMeshNode(readIndex);
...
#if !defined(TTGO_T_ECHO) && !defined(CANARYONE) && !defined(MESHLINK) && !defined(ELECROW_ThinkNode_M1)
            else if ((moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_WS85)) {
                processWXSerial();

            } else {
#if defined(CONFIG_IDF_TARGET_ESP32C6)
                while (Serial1.available()) {
                    serialPayloadSize = Serial1.readBytes(serialBytes, water_sensor_mesh_Constants_DATA_PAYLOAD_LEN);
#else
                while (Serial2.available()) {
                    serialPayloadSize = Serial2.readBytes(serialBytes, water_sensor_mesh_Constants_DATA_PAYLOAD_LEN);
#endif
 ...
 * @throws None
 */
void SerialModule::sendTelemetry(water_sensor_mesh_Telemetry m)
{
    water_sensor_mesh_MeshPacket *p = router->allocForSending();
    p->decoded.portnum = water_sensor_mesh_PortNum_TELEMETRY_APP;
    p->decoded.payload.size =
        pb_encode_to_bytes(p->decoded.payload.bytes, sizeof(p->decoded.payload.bytes), &water_sensor_mesh_Telemetry_msg, &m);
    p->to = NODENUM_BROADCAST;
    p->decoded.want_response = false;
    if (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_SENSOR) {
        p->want_ack = true;
        p->priority = water_sensor_mesh_MeshPacket_Priority_HIGH;
    } else {
        p->priority = water_sensor_mesh_MeshPacket_Priority_RELIABLE;
    }
 ...
 */
water_sensor_mesh_MeshPacket *SerialModuleRadio::allocReply()
{
...
{
    const water_sensor_mesh_Channel *ch = (boundChannel != NULL) ? &channels.getByName(boundChannel) : NULL;
    water_sensor_mesh_MeshPacket *p = allocReply();
    p->to = dest;
    if (ch != NULL) {
        p->channel = ch->index;
    }
 ...
 */
ProcessMessage SerialModuleRadio::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
    if (moduleConfig.serial.enabled) {
        if (moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_PROTO) {
        ...
        if (isFromUs(&mp)) {
            ...
            if (moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_DEFAULT ||
                moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_SIMPLE) {
                serialPrint->write(p.payload.bytes, p.payload.size);
            } else if (moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_TEXTMSG) {
                water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(getFrom(&mp));
                const char *sender = (node && node->has_user) ? node->user.short_name : "???";
                serialPrint->println();
                serialPrint->printf("%s: %s", sender, p.payload.bytes);
                serialPrint->println();
            } else if ((moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_NMEA ||
                        moduleConfig.serial.mode == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Mode_CALTOPO) &&
                       HAS_GPS) {
                // Decode the Payload some more
                water_sensor_mesh_Position scratch;
                water_sensor_mesh_Position *decoded = NULL;
                if (mp.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag && mp.decoded.portnum == ourPortNum) {
                    memset(&scratch, 0, sizeof(scratch));
                    if (pb_decode_from_bytes(p.payload.bytes, p.payload.size, &water_sensor_mesh_Position_msg, &scratch)) {
                        decoded = &scratch;
                    }
...
{
    if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_110) {
        return 110;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_300) {
        return 300;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_600) {
        return 600;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_1200) {
        return 1200;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_2400) {
        return 2400;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_4800) {
        return 4800;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_9600) {
        return 9600;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_19200) {
        return 19200;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_38400) {
        return 38400;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_57600) {
        return 57600;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_115200) {
        return 115200;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_230400) {
        return 230400;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_460800) {
        return 460800;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_576000) {
        return 576000;
    } else if (moduleConfig.serial.baud == water_sensor_mesh_ModuleConfig_SerialConfig_Serial_Baud_BAUD_921600) {
        return 921600;
    }
    return BAUD;
}
...
void SerialModule::processWXSerial()
{
        ...
        if (serialPayloadSize > 0) {
            ...
            for (size_t i = 0; i < serialPayloadSize; i++) {
                ...
                if (serialBytes[i] == '\n') {
                    ...
                    char line[water_sensor_mesh_Constants_DATA_PAYLOAD_LEN];
    ...
    if (gotwind && !Throttle::isWithinTimespanMs(lastAveraged, averageIntervalMillis)) {
        ...
        water_sensor_mesh_Telemetry m = water_sensor_mesh_Telemetry_init_zero;
        m.which_variant = water_sensor_mesh_Telemetry_environment_metrics_tag;
