...
ProcessMessage WaypointModule::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
#ifdef DEBUG_PORT
    auto &p = mp.decoded;
...
void WaypointModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    // Decode the waypoint
    const water_sensor_mesh_MeshPacket &mp = devicestate.rx_waypoint;
    water_sensor_mesh_Waypoint wp;
    memset(&wp, 0, sizeof(wp));
    if (!pb_decode_from_bytes(mp.decoded.payload.bytes, mp.decoded.payload.size, &water_sensor_mesh_Waypoint_msg, &wp)) {
    ...
    if (config.display.displaymode == water_sensor_mesh_Config_DisplayConfig_DisplayMode_DEFAULT) {
    ...
    if (ourNode && (nodeDB->hasValidPosition(ourNode) || screen->hasHeading())) {
        const water_sensor_mesh_PositionLite &op = ourNode->position;
        float myHeading;
