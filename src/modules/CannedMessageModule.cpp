...
#include "mesh/generated/water_sensor_mesh/cannedmessages.pb.h"
...
water_sensor_mesh_CannedMessageModuleConfig cannedMessageModuleConfig;

CannedMessageModule *cannedMessageModule;

CannedMessageModule::CannedMessageModule()
    : SinglePortModule("canned", water_sensor_mesh_PortNum_TEXT_MESSAGE_APP), concurrency::OSThread("CannedMessage")
{
...
bool hasKeyForNode(const water_sensor_mesh_NodeInfoLite *node)
{
...
void CannedMessageModule::updateDestinationSelectionList()
{
    ...
    for (size_t i = 0; i < numMeshNodes; ++i) {
        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNodeByIndex(i);
    ...
    if (isSelect) {
        if (destIndex < static_cast<int>(activeChannelIndices.size())) {
            ...
            if (nodeIndex >= 0 && nodeIndex < static_cast<int>(filteredNodes.size())) {
                const water_sensor_mesh_NodeInfoLite *selectedNode = filteredNodes[nodeIndex].node;
                if (selectedNode) {
...
void CannedMessageModule::sendText(NodeNum dest, ChannelIndex channel, const char *message, bool wantReplies)
{
    ...
    if (moduleConfig.canned_message.send_bell && p->decoded.payload.size < water_sensor_mesh_Constants_DATA_PAYLOAD_LEN) {
    ...
    if (screen) {
        water_sensor_mesh_MeshPacket simulatedPacket = {};
        ...
        if (this->runState == CANNED_MESSAGE_RUN_STATE_FREETEXT) {
            ...
            default:
                ...
                if (this->payload >= 32 && this->payload <= 126) {
                    ...
                    this->cursor++;
                    uint16_t maxChars = water_sensor_mesh_Constants_DATA_PAYLOAD_LEN - (moduleConfig.canned_message.send_bell ? 1 : 0);
...
const char *CannedMessageModule::getNodeName(NodeNum node)
{
    ...
    water_sensor_mesh_NodeInfoLite *info = nodeDB->getMeshNode(node);
    ...
    for (int row = 0; row < visibleRows; row++) {
        ...
        // Then Draw Nodes
            ...
            if (nodeIndex >= 0 && nodeIndex < static_cast<int>(this->filteredNodes.size())) {
                water_sensor_mesh_NodeInfoLite *node = this->filteredNodes[nodeIndex].node;
                if (node) {
                    if (node->is_favorite) {
        ...
        if (itemIndex >= numActiveChannels) {
            int nodeIndex = itemIndex - numActiveChannels;
            if (nodeIndex >= 0 && nodeIndex < static_cast<int>(this->filteredNodes.size())) {
                const water_sensor_mesh_NodeInfoLite *node = this->filteredNodes[nodeIndex].node;
...
void CannedMessageModule::drawFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    if (this->runState == CANNED_MESSAGE_RUN_STATE_FREETEXT) {
        ...
        if (runState != CANNED_MESSAGE_RUN_STATE_DESTINATION_SELECTION) {
            uint16_t charsLeft =
                water_sensor_mesh_Constants_DATA_PAYLOAD_LEN - this->freetext.length() - (moduleConfig.canned_message.send_bell ? 1 : 0);
...
ProcessMessage CannedMessageModule::handleReceived(const water_sensor_mesh_MeshPacket &mp)
{
    if (mp.decoded.portnum == water_sensor_mesh_PortNum_ROUTING_APP && waitingForAck) {
        if (mp.decoded.request_id != 0) {
            ...
            water_sensor_mesh_Routing decoded = water_sensor_mesh_Routing_init_default;
            pb_decode_from_bytes(mp.decoded.payload.bytes, mp.decoded.payload.size, water_sensor_mesh_Routing_fields, &decoded);
            ...
            bool isAck = (decoded.error_reason == water_sensor_mesh_Routing_Error_NONE);
...
{
    if (nodeDB->loadProto(cannedMessagesConfigFile, water_sensor_mesh_CannedMessageModuleConfig_size,
                          sizeof(water_sensor_mesh_CannedMessageModuleConfig), &water_sensor_mesh_CannedMessageModuleConfig_msg,
...
bool CannedMessageModule::saveProtoForModule()
{
    ...
    okay &= nodeDB->saveProto(cannedMessagesConfigFile, water_sensor_mesh_CannedMessageModuleConfig_size,
                              &water_sensor_mesh_CannedMessageModuleConfig_msg, &cannedMessageModuleConfig);
 ...
 */
AdminMessageHandleResult CannedMessageModule::handleAdminMessageForModule(const water_sensor_mesh_MeshPacket &mp,
                                                                          ...
                                                                          water_sensor_mesh_AdminMessage *response)
{
    ...
    case water_sensor_mesh_AdminMessage_get_canned_message_module_messages_request_tag:
        LOG_DEBUG("Client getting radio canned messages");
        this->handleGetCannedMessageModuleMessages(mp, response);
        result = AdminMessageHandleResult::HANDLED_WITH_RESPONSE;
        break;

    case water_sensor_mesh_AdminMessage_set_canned_message_module_messages_tag:
        LOG_DEBUG("Client getting radio canned messages");
...
void CannedMessageModule::handleGetCannedMessageModuleMessages(const water_sensor_mesh_MeshPacket &req,
                                                               water_sensor_mesh_AdminMessage *response)
{
    LOG_DEBUG("*** handleGetCannedMessageModuleMessages");
    if (req.decoded.want_response) {
        response->which_payload_variant = water_sensor_mesh_AdminMessage_get_canned_message_module_messages_response_tag;
