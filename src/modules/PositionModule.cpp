...
PositionModule::PositionModule()
    : ProtobufModule("position", water_sensor_mesh_PortNum_POSITION_APP, &water_sensor_mesh_Position_msg), concurrency::OSThread("Position")
{
    precision = 0;        // safe starting value
    isPromiscuous = true; // We always want to update our nodedb, even if we are sniffing on others
    nodeStatusObserver.observe(&nodeStatus->onNewStatus);

    if (config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_TRACKER &&
        config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_TAK_TRACKER) {
...
bool PositionModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Position *pptr)
{
    auto p = *pptr;
    ...
    if (isFromUs(&mp)) {
        ...
        if (config.position.fixed_position) {
            ...
            // client device here
            if (p.time && channels.getByIndex(mp.channel).role == water_sensor_mesh_Channel_Role_PRIMARY) {
    ...
    } else if (channels.getByIndex(mp.channel).role == water_sensor_mesh_Channel_Role_PRIMARY) {
        precision = 32;
    } else {
        precision = 0;
    }
...
void PositionModule::alterReceivedProtobuf(water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_Position *p)
{
    ...
    if (isFromUs(&mp) && (precision < 32 && precision > 0)) {
        ...
        mp.decoded.payload.size =
            pb_encode_to_bytes(mp.decoded.payload.bytes, sizeof(mp.decoded.payload.bytes), &water_sensor_mesh_Position_msg, p);
    }
}

void PositionModule::trySetRtc(water_sensor_mesh_Position p, bool isLocal, bool forceUpdate)
{
    ...
    if (!isLocal && p.location_source < water_sensor_mesh_Position_LocSource_LOC_INTERNAL) {
...
water_sensor_mesh_MeshPacket *PositionModule::allocPositionPacket()
{
    if (precision == 0) {
    ...
    water_sensor_mesh_NodeInfoLite *node = service->refreshLocalMeshNode(); // should guarantee there is now a position
    ...
    water_sensor_mesh_Position p = water_sensor_mesh_Position_init_default; //   Start with an empty structure
    ...
    if (config.position.fixed_position) {
        p.location_source = water_sensor_mesh_Position_LocSource_LOC_MANUAL;
    } else {
        p.location_source = localPosition.location_source;
    }

    if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_ALTITUDE) {
        if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_ALTITUDE_MSL) {
        ...
        if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_GEOIDAL_SEPARATION) {
            p.altitude_geoidal_separation = localPosition.altitude_geoidal_separation;
            p.has_altitude_geoidal_separation = true;
        }
    }

    if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_DOP) {
        if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_HVDOP) {
    ...
    if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_SATINVIEW)
        p.sats_in_view = localPosition.sats_in_view;

    if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_TIMESTAMP)
        p.timestamp = localPosition.timestamp;

    if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_SEQ_NO)
        p.seq_number = localPosition.seq_number;

    if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_HEADING) {
        p.ground_track = localPosition.ground_track;
        p.has_ground_track = true;
    }

    if (pos_flags & water_sensor_mesh_Config_PositionConfig_PositionFlags_SPEED) {
    ...
    if (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_TAK_TRACKER)
        return allocAtakPli();

    return allocDataProtobuf(p);
}

water_sensor_mesh_MeshPacket *PositionModule::allocReply()
{
    if (config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_LOST_AND_FOUND && lastSentReply &&
    ...
    water_sensor_mesh_MeshPacket *reply = allocPositionPacket();
    if (reply) {
...
water_sensor_mesh_MeshPacket *PositionModule::allocAtakPli()
{
    LOG_INFO("Send TAK PLI packet");
    water_sensor_mesh_MeshPacket *mp = allocDataPacket();
    mp->decoded.portnum = water_sensor_mesh_PortNum_ATAK_PLUGIN;

    water_sensor_mesh_TAKPacket takPacket = {.is_compressed = true,
                                      .has_contact = true,
                                      .contact = water_sensor_mesh_Contact_init_default,
                                      .has_group = true,
                                      .group = {water_sensor_mesh_MemberRole_TeamMember, water_sensor_mesh_Team_Cyan},
                                      .has_status = true,
                                      .status =
                                          {
                                              .battery = powerStatus->getBatteryChargePercent(),
                                          },
                                      .which_payload_variant = water_sensor_mesh_TAKPacket_pli_tag,
                                      .payload_variant = {.pli = {
    ...
    mp->decoded.payload.size =
        pb_encode_to_bytes(mp->decoded.payload.bytes, sizeof(mp->decoded.payload.bytes), &water_sensor_mesh_TAKPacket_msg, &takPacket);
    return mp;
}
...
void PositionModule::sendOurPosition(NodeNum dest, bool wantReplies, uint8_t channel)
{
    ...
    water_sensor_mesh_MeshPacket *p = allocPositionPacket();
    if (p == nullptr) {
    ...
    p->to = dest;
    p->decoded.want_response = config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_TRACKER ? false : wantReplies;
    if (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_TRACKER ||
        config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_TAK_TRACKER)
    ...
    if (IS_ONE_OF(config.device.role, water_sensor_mesh_Config_DeviceConfig_Role_TRACKER,
                  water_sensor_mesh_Config_DeviceConfig_Role_TAK_TRACKER) &&
        config.power.is_power_saving) {
        water_sensor_mesh_ClientNotification *notification = clientNotificationPool.allocZeroed();
        notification->level = water_sensor_mesh_LogRecord_Level_INFO;
...
int32_t PositionModule::runOnce()
{
    ...
    if (!airTime->isTxAllowedChannelUtil(config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_TRACKER &&
                                         config.device.role != water_sensor_mesh_Config_DeviceConfig_Role_TAK_TRACKER)) {
        return RUNONCE_INTERVAL;
    }
    ...
    if (lastGpsSend == 0 || msSinceLastSend >= intervalMs) {
        if (nodeDB->hasValidPosition(node)) {
            ...
            sendOurPosition();
            if (config.device.role == water_sensor_mesh_Config_DeviceConfig_Role_LOST_AND_FOUND) {
                sendLostAndFoundText();
            }
        }
    } else if (config.position.position_broadcast_smart_enabled) {
        const water_sensor_mesh_NodeInfoLite *node2 = service->refreshLocalMeshNode(); // should guarantee there is now a position
...
struct SmartPosition PositionModule::getDistanceTraveledSinceLastSend(water_sensor_mesh_PositionLite currentPosition)
{
