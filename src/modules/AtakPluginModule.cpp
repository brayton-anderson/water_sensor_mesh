...
AtakPluginModule::AtakPluginModule()
    : ProtobufModule("atak", water_sensor_mesh_PortNum_ATAK_PLUGIN, &water_sensor_mesh_TAKPacket_msg), concurrency::OSThread("AtakPlugin")
{
    ourPortNum = water_sensor_mesh_PortNum_ATAK_PLUGIN;
}
...
bool AtakPluginModule::handleReceivedProtobuf(const water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_TAKPacket *r)
{
    return false;
}

water_sensor_mesh_TAKPacket AtakPluginModule::cloneTAKPacketData(water_sensor_mesh_TAKPacket *t)
{
    water_sensor_mesh_TAKPacket clone = water_sensor_mesh_TAKPacket_init_zero;
    if (t->has_group) {
    ...
    if (t->which_payload_variant == water_sensor_mesh_TAKPacket_pli_tag) {
        clone.which_payload_variant = water_sensor_mesh_TAKPacket_pli_tag;
        clone.payload_variant.pli = t->payload_variant.pli;
    } else if (t->which_payload_variant == water_sensor_mesh_TAKPacket_chat_tag) {
        clone.which_payload_variant = water_sensor_mesh_TAKPacket_chat_tag;
        clone.payload_variant.chat = {0};
    } else if (t->which_payload_variant == water_sensor_mesh_TAKPacket_detail_tag) {
        clone.which_payload_variant = water_sensor_mesh_TAKPacket_detail_tag;
...
void AtakPluginModule::alterReceivedProtobuf(water_sensor_mesh_MeshPacket &mp, water_sensor_mesh_TAKPacket *t)
{
    // From Phone (EUD)
    if (mp.from == 0) {
        ...
        if (t->which_payload_variant == water_sensor_mesh_TAKPacket_chat_tag) {
        ...
        mp.decoded.payload.size = pb_encode_to_bytes(mp.decoded.payload.bytes, sizeof(mp.decoded.payload.bytes),
                                                     water_sensor_mesh_TAKPacket_fields, &compressed);
        ...
        if (uncompressed.which_payload_variant == water_sensor_mesh_TAKPacket_chat_tag) {
        ...
        decompressedCopy->decoded.payload.size =
            pb_encode_to_bytes(decompressedCopy->decoded.payload.bytes, sizeof(decompressedCopy->decoded.payload),
                               water_sensor_mesh_TAKPacket_fields, &uncompressed);
