...
ErrorCode SimRadio::send(water_sensor_mesh_MeshPacket *p)
{
        ...
        if (!txQueue.empty()) {
            if (!canSendImmediately()) {
                ...
                if (isChannelActive()) { // check if there is currently a LoRa packet on the channel
                    ...
                    water_sensor_mesh_MeshPacket *txp = txQueue.dequeue();
                    assert(txp);
                    startSend(txp);
...
void SimRadio::startSend(water_sensor_mesh_MeshPacket *txp)
{
    printPacket("Start low level send", txp);
    isReceiving = false;
    size_t numbytes = beginSending(txp);
    water_sensor_mesh_MeshPacket *p = packetPool.allocCopy(*txp);
    perhapsDecode(p);
    water_sensor_mesh_Compressed c = water_sensor_mesh_Compressed_init_default;
    ...
    p->decoded.payload.size =
        pb_encode_to_bytes(p->decoded.payload.bytes, sizeof(p->decoded.payload.bytes), &water_sensor_mesh_Compressed_msg, &c);
    p->decoded.portnum = water_sensor_mesh_PortNum_SIMULATOR_APP;
...
void SimRadio::unpackAndReceive(water_sensor_mesh_MeshPacket &p)
{
    // Simulator packet (=Compressed packet) is encapsulated in a MeshPacket, so need to unwrap first
    water_sensor_mesh_Compressed scratch;
    water_sensor_mesh_Compressed *decoded = NULL;
    if (p.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
        memset(&scratch, 0, sizeof(scratch));
        p.decoded.payload.size =
            pb_decode_from_bytes(p.decoded.payload.bytes, p.decoded.payload.size, &water_sensor_mesh_Compressed_msg, &scratch);
...
void SimRadio::startReceive(water_sensor_mesh_MeshPacket *p)
{
...
water_sensor_mesh_QueueStatus SimRadio::getQueueStatus()
{
...
void SimRadio::handleReceiveInterrupt()
{
    ...
    water_sensor_mesh_MeshPacket *mp = packetPool.allocCopy(*receivingPacket); // keep a copy in packetPool
...
size_t SimRadio::getPacketLength(water_sensor_mesh_MeshPacket *mp)
{
    auto &p = mp->decoded;
