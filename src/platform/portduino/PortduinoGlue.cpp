...
void portduinoCustomInit()
{
    ...
    static char doc[] = "Water_Sensor_Mesh native build.";
...
void portduinoSetup()
{
    printf("Set up Water_Sensor_Mesh on Portduino...\n");
    int max_GPIO = 0;
    ...
    } else if (access("/etc/water_sensor_meshd/config.yaml", R_OK) == 0) {
        if (loadConfig("/etc/water_sensor_meshd/config.yaml")) {
            std::cout << "Using /etc/water_sensor_meshd/config.yaml as config file" << std::endl;
        } else {
            std::cout << "Unable to use /etc/water_sensor_meshd/config.yaml as config file" << std::endl;
            exit(EXIT_FAILURE);
        }
    } else {
...
bool loadConfig(const char *configPath)
{
    ...
    try {
        ...
        if (yamlConfig["Webserver"]) {
            ...
            settingsStrings[webserverrootpath] =
                (yamlConfig["Webserver"]["RootPath"]).as<std::string>("/usr/share/water_sensor_meshd/web");
            settingsStrings[websslkeypath] =
                (yamlConfig["Webserver"]["SSLKey"]).as<std::string>("/etc/water_sensor_meshd/ssl/private_key.pem");
            settingsStrings[websslcertpath] =
                (yamlConfig["Webserver"]["SSLCert"]).as<std::string>("/etc/water_sensor_meshd/ssl/certificate.pem");
        }
        ...
        if (yamlConfig["General"]) {
            ...
            settingsStrings[available_directory] =
                (yamlConfig["General"]["AvailableDirectory"]).as<std::string>("/etc/water_sensor_meshd/available.d/");
