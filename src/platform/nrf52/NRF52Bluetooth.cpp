...
static uint8_t fromRadioBytes[water_sensor_mesh_FromRadio_size];
static uint8_t toRadioBytes[water_sensor_mesh_ToRadio_size];
...
void onConnect(uint16_t conn_handle)
{
    ...
    bluetoothStatus->updateStatus(new water_sensor_mesh::BluetoothStatus(water_sensor_mesh::BluetoothStatus::ConnectionState::CONNECTED));
}
/**
...
void onDisconnect(uint16_t conn_handle, uint8_t reason)
{
    ...
    // Notify UI (or any other interested firmware components)
    bluetoothStatus->updateStatus(new water_sensor_mesh::BluetoothStatus(water_sensor_mesh::BluetoothStatus::ConnectionState::DISCONNECTED));
}
...
void setupMeshService(void)
{
    ...
    // was 'begin()'ed!
    auto secMode =
        config.bluetooth.mode == water_sensor_mesh_Config_BluetoothConfig_PairingMode_NO_PIN ? SECMODE_OPEN : SECMODE_ENC_NO_MITM;
...
void NRF52Bluetooth::setup()
{
    ...
    if (config.bluetooth.mode != water_sensor_mesh_Config_BluetoothConfig_PairingMode_NO_PIN) {
        configuredPasskey = config.bluetooth.mode == water_sensor_mesh_Config_BluetoothConfig_PairingMode_FIXED_PIN
...
bool NRF52Bluetooth::onPairingPasskey(uint16_t conn_handle, uint8_t const passkey[6], bool match_request)
{
    ...
    bluetoothStatus->updateStatus(new water_sensor_mesh::BluetoothStatus(textkey));

#if !defined(WATER_SENSOR_MESH_EXCLUDE_SCREEN) // Todo: migrate this display code back into Screen class, and observe bluetoothStatus
    if (screen) {
...
void NRF52Bluetooth::onPairingCompleted(uint16_t conn_handle, uint8_t auth_status)
{
    if (auth_status == BLE_GAP_SEC_STATUS_SUCCESS) {
        ...
        bluetoothStatus->updateStatus(
            new water_sensor_mesh::BluetoothStatus(water_sensor_mesh::BluetoothStatus::ConnectionState::DISCONNECTED));
    }
