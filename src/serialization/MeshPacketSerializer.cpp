...
std::string MeshPacketSerializer::JsonSerialize(const water_sensor_mesh_MeshPacket *mp, bool shouldLog)
{
    ...
    if (mp->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
        ...
        case water_sensor_mesh_PortNum_TEXT_MESSAGE_APP: {
            msgType = "text";
        ...
        case water_sensor_mesh_PortNum_TELEMETRY_APP: {
            msgType = "telemetry";
            water_sensor_mesh_Telemetry scratch;
            water_sensor_mesh_Telemetry *decoded = NULL;
            memset(&scratch, 0, sizeof(scratch));
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_Telemetry_msg, &scratch)) {
                decoded = &scratch;
                if (decoded->which_variant == water_sensor_mesh_Telemetry_device_metrics_tag) {
                ...
                } else if (decoded->which_variant == water_sensor_mesh_Telemetry_environment_metrics_tag) {
                ...
                } else if (decoded->which_variant == water_sensor_mesh_Telemetry_air_quality_metrics_tag) {
                ...
                } else if (decoded->which_variant == water_sensor_mesh_Telemetry_power_metrics_tag) {
        ...
        case water_sensor_mesh_PortNum_NODEINFO_APP: {
            msgType = "nodeinfo";
            water_sensor_mesh_User scratch;
            water_sensor_mesh_User *decoded = NULL;
            memset(&scratch, 0, sizeof(scratch));
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_User_msg, &scratch)) {
                decoded = &scratch;
        ...
        case water_sensor_mesh_PortNum_POSITION_APP: {
            msgType = "position";
            water_sensor_mesh_Position scratch;
            water_sensor_mesh_Position *decoded = NULL;
            memset(&scratch, 0, sizeof(scratch));
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_Position_msg, &scratch)) {
                decoded = &scratch;
                if ((int)decoded->time) {
        ...
        case water_sensor_mesh_PortNum_WAYPOINT_APP: {
            msgType = "waypoint";
            water_sensor_mesh_Waypoint scratch;
            water_sensor_mesh_Waypoint *decoded = NULL;
            memset(&scratch, 0, sizeof(scratch));
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_Waypoint_msg, &scratch)) {
                decoded = &scratch;
        ...
        case water_sensor_mesh_PortNum_NEIGHBORINFO_APP: {
            msgType = "neighborinfo";
            water_sensor_mesh_NeighborInfo scratch;
            water_sensor_mesh_NeighborInfo *decoded = NULL;
            memset(&scratch, 0, sizeof(scratch));
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_NeighborInfo_msg,
                                     &scratch)) {
                decoded = &scratch;
        ...
        case water_sensor_mesh_PortNum_TRACEROUTE_APP: {
            if (mp->decoded.request_id) { // Only report the traceroute response
                msgType = "traceroute";
                water_sensor_mesh_RouteDiscovery scratch;
                water_sensor_mesh_RouteDiscovery *decoded = NULL;
                memset(&scratch, 0, sizeof(scratch));
                if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_RouteDiscovery_msg,
                                         &scratch)) {
                    decoded = &scratch;
                    ...
                    auto addToRoute = [](JSONArray *route, NodeNum num) {
                        ...
                        water_sensor_mesh_NodeInfoLite *node = nodeDB->getMeshNode(num);
        ...
        case water_sensor_mesh_PortNum_DETECTION_SENSOR_APP: {
            msgType = "detection";
            char payloadStr[(mp->decoded.payload.size) + 1];
            memcpy(payloadStr, mp->decoded.payload.bytes, mp->decoded.payload.size);
            payloadStr[mp->decoded.payload.size] = 0; // null terminated string
...
#ifdef ARCH_ESP32
        case water_sensor_mesh_PortNum_PAXCOUNTER_APP: {
            msgType = "paxcounter";
            water_sensor_mesh_Paxcount scratch;
            water_sensor_mesh_Paxcount *decoded = NULL;
            memset(&scratch, 0, sizeof(scratch));
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_Paxcount_msg, &scratch)) {
                decoded = &scratch;
...
#endif
        case water_sensor_mesh_PortNum_REMOTE_HARDWARE_APP: {
            water_sensor_mesh_HardwareMessage scratch;
            water_sensor_mesh_HardwareMessage *decoded = NULL;
            memset(&scratch, 0, sizeof(scratch));
            if (pb_decode_from_bytes(mp->decoded.payload.bytes, mp->decoded.payload.size, &water_sensor_mesh_HardwareMessage_msg,
                                     &scratch)) {
                decoded = &scratch;
                if (decoded->type == water_sensor_mesh_HardwareMessage_Type_GPIOS_CHANGED) {
                    msgType = "gpios_changed";
                    msgPayload["gpio_value"] = new JSONValue((unsigned int)decoded->gpio_value);
                    jsonObj["payload"] = new JSONValue(msgPayload);
                } else if (decoded->type == water_sensor_mesh_HardwareMessage_Type_READ_GPIOS_REPLY) {
...
std::string MeshPacketSerializer::JsonSerializeEncrypted(const water_sensor_mesh_MeshPacket *mp)
{
    JSONObject jsonObj;
