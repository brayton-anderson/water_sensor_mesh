...
class MQTT : private concurrency::OSThread
{
  public:
     ...
     */
    void onSend(const water_sensor_mesh_MeshPacket &mp_encrypted, const water_sensor_mesh_MeshPacket &mp_decoded, ChannelIndex chIndex);
    ...
    void onClientProxyReceive(water_sensor_mesh_MqttClientProxyMessage msg);
    ...
    /// Validate the water_sensor_mesh_ModuleConfig_MQTTConfig.
    static bool isValidConfig(const water_sensor_mesh_ModuleConfig_MQTTConfig &config) { return isValidConfig(config, nullptr); }
...
#define MQTT_SUPPORTS_TLS 1
    ...
    static bool isValidConfig(const water_sensor_mesh_ModuleConfig_MQTTConfig &config, MQTTClient *client);
