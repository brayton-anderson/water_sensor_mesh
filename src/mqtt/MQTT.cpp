...
#include "../mesh/generated/water_sensor_mesh/paxcount.pb.h"
#endif
#include "mesh/generated/water_sensor_mesh/remote_hardware.pb.h"
#include "sleep.h"
#if HAS_WIFI
...
static uint8_t bytes[water_sensor_mesh_MqttClientProxyMessage_size + 30]; // 12 for channel name and 16 for nodeid
...
inline void onReceiveProto(char *topic, byte *payload, size_t length)
{
    ...
    const water_sensor_mesh_Channel &ch = channels.getByName(e.channel_id);
    if (strcmp(e.gateway_id, owner.id) == 0) {
        // Generate an implicit ACK towards ourselves (handled and processed only locally!) for this message.
        // We do this because packets are not rebroadcasted back into MQTT anymore and we assume that at least one node
        // receives it when we get our own packet back. Then we'll stop our retransmissions.
        if (isFromUs(e.packet))
            routingModule->sendAckNak(water_sensor_mesh_Routing_Error_NONE, getFrom(e.packet), e.packet->id, ch.index);
        else
    ...
    if (p->which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
        ...
        if (p->decoded.portnum == water_sensor_mesh_PortNum_ADMIN_APP) {
    ...
    if (router && p->which_payload_variant == water_sensor_mesh_MeshPacket_encrypted_tag && strcmp(e.channel_id, "PKI") == 0) {
        const water_sensor_mesh_NodeInfoLite *tx = nodeDB->getMeshNode(getFrom(p.get()));
        const water_sensor_mesh_NodeInfoLite *rx = nodeDB->getMeshNode(p->to);
...
inline void onReceiveJson(byte *payload, size_t length)
{
    ...
    if (json["type"]->AsString().compare("sendtext") == 0 && json["payload"]->IsString()) {
        ...
        water_sensor_mesh_Position pos = water_sensor_mesh_Position_init_default;
        ...
        water_sensor_mesh_MeshPacket *p = router->allocForSending();
        p->decoded.portnum = water_sensor_mesh_PortNum_POSITION_APP;
        if (json.find("channel") != json.end() && json["channel"]->IsNumber() &&
            (json["channel"]->AsNumber() < channels.getNumChannels()))
            p->channel = json["channel"]->AsNumber();
        if (json.find("to") != json.end() && json["to"]->IsNumber())
        ...
        p->decoded.payload.size =
            pb_encode_to_bytes(p->decoded.payload.bytes, sizeof(p->decoded.payload.bytes), &water_sensor_mesh_Position_msg,
...
struct PubSubConfig {
    explicit PubSubConfig(const water_sensor_mesh_ModuleConfig_MQTTConfig &config)
    {
        if (*config.address) {
...
void MQTT::onClientProxyReceive(water_sensor_mesh_MqttClientProxyMessage msg)
{
...
void MQTT::onReceive(char *topic, byte *payload, size_t length)
{
    ...
    if (moduleConfig.mqtt.json_enabled && (strncmp(topic, jsonTopic.c_str(), jsonTopic.length()) == 0)) {
        ...
        const water_sensor_mesh_Channel &sendChannel = channels.getByName(channelName);
...
{
    if (moduleConfig.mqtt.proxy_to_client_enabled) {
        water_sensor_mesh_MqttClientProxyMessage *msg = mqttClientProxyMessagePool.allocZeroed();
        msg->which_payload_variant = water_sensor_mesh_MqttClientProxyMessage_text_tag;
...
bool MQTT::publish(const char *topic, const uint8_t *payload, size_t length, bool retained)
{
    if (moduleConfig.mqtt.proxy_to_client_enabled) {
        water_sensor_mesh_MqttClientProxyMessage *msg = mqttClientProxyMessagePool.allocZeroed();
        msg->which_payload_variant = water_sensor_mesh_MqttClientProxyMessage_data_tag;
        strcpy(msg->topic, topic);
...
bool MQTT::isValidConfig(const water_sensor_mesh_ModuleConfig_MQTTConfig &config, MQTTClient *client)
{
...
void MQTT::onSend(const water_sensor_mesh_MeshPacket &mp_encrypted, const water_sensor_mesh_MeshPacket &mp_decoded, ChannelIndex chIndex)
{
    ...
    if (mp_decoded.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
        ...
        if (isConfiguredForDefaultServer && (mp_decoded.decoded.portnum == water_sensor_mesh_PortNum_RANGE_TEST_APP ||
                                             mp_decoded.decoded.portnum == water_sensor_mesh_PortNum_DETECTION_SENSOR_APP)) {
    ...
    const water_sensor_mesh_MeshPacket *p;
    if (moduleConfig.mqtt.encryption_enabled) {
        p = &mp_encrypted;
        LOG_DEBUG("encrypted message");
    } else if (mp_decoded.which_payload_variant == water_sensor_mesh_MeshPacket_decoded_tag) {
        p = &mp_decoded;
    ...
    const water_sensor_mesh_ServiceEnvelope env = {
        .packet = const_cast<water_sensor_mesh_MeshPacket *>(p), .channel_id = const_cast<char *>(channelId), .gateway_id = owner.id};
...
void MQTT::perhapsReportToMap()
{
    ...
    water_sensor_mesh_MeshPacket *mp = packetPool.allocZeroed();
    mp->which_payload_variant = water_sensor_mesh_MeshPacket_decoded_tag;
    mp->from = nodeDB->getNodeNum();
    mp->to = NODENUM_BROADCAST;
    mp->decoded.portnum = water_sensor_mesh_PortNum_MAP_REPORT_APP;

    // Fill MapReport message
    water_sensor_mesh_MapReport mapReport = water_sensor_mesh_MapReport_init_default;
    ...
    mp->decoded.payload.size =
        pb_encode_to_bytes(mp->decoded.payload.bytes, sizeof(mp->decoded.payload.bytes), &water_sensor_mesh_MapReport_msg, &mapReport);

    // Encode the MeshPacket into a binary ServiceEnvelope and publish
    const water_sensor_mesh_ServiceEnvelope se = {
        .packet = mp,
        .channel_id = (char *)channels.getGlobalId(channels.getPrimaryIndex()), // Use primary channel as the channel_id
        .gateway_id = owner.id};
    size_t numBytes = pb_encode_to_bytes(bytes, sizeof(bytes), &water_sensor_mesh_ServiceEnvelope_msg, &se);
