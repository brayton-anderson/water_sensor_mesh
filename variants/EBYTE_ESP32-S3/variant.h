...
// DIO2 as an extra interrupt, but right now Water_Sensor_Mesh does not benefit from having another interrupt pin available.
...
// We don't define a pin for SX126X_DIO2 as Water_Sensor_Mesh doesn't use it as an interrupt output, so it is never connected to an MCU
// pin! Also E22 module datasheets say not to connect it to an MCU pin.
// We don't define a pin for SX126X_DIO3 as Water_Sensor_Mesh doesn't use it as an interrupt output, so it is never connected to an MCU
...
// voltage difference between DIO2 and TXEN). Can use DIO2 as an IRQ (but not in Water_Sensor_Mesh at the moment).
/*
...
#define LORA_DIO1                                                                                                                \
    SX126X_DIO1 // The old name is used in
                // https://github.com/water_sensor_mesh/firmware/blob/7eff5e7bcb2084499b723c5e3846c15ee089e36d/src/sleep.cpp#L298, so
