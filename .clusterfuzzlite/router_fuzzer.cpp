...
std::thread water_sensor_meshThread;
...
// Called just prior to starting Water_Sensor_Mesh. Allows for setting config values before startup.
void lateInitVariant()
{
    ...
    config.lora.region = water_sensor_mesh_Config_LoRaConfig_RegionCode_US;
    moduleConfig.has_mqtt = true;
    moduleConfig.mqtt = water_sensor_mesh_ModuleConfig_MQTTConfig{
        .enabled = true,
        .proxy_to_client_enabled = true,
    };
    moduleConfig.has_store_forward = true;
    moduleConfig.store_forward = water_sensor_mesh_ModuleConfig_StoreForwardConfig{
        .enabled = true,
        .history_return_max = 4,
        .history_return_window = 600,
        .is_server = true,
    };
    water_sensor_mesh_Position fixedGPS = water_sensor_mesh_Position{
        .has_latitude_i = true,
        ...
        .has_altitude = true,
        .altitude = 64,
        .location_source = water_sensor_mesh_Position_LocSource_LOC_MANUAL,
    };
    nodeDB->setLocalPosition(fixedGPS);
    config.has_position = true;
    config.position.fixed_position = true;
    water_sensor_mesh_NodeInfoLite *info = nodeDB->getMeshNode(nodeDB->getNodeNum());
...
extern "C" {
int portduino_main(int argc, char **argv); // Renamed "main" function from Water_Sensor_Mesh binary.

// Start Water_Sensor_Mesh in a thread and wait till it has reached the ON state.
int LLVMFuzzerInitialize(int *argc, char ***argv)
{
    settingsMap[maxtophone] = 5;

    water_sensor_meshThread = std::thread([program = *argv[0]]() {
        char nodeIdStr[12];
        strcpy(nodeIdStr, std::to_string(nodeId).c_str());
        int argc = 7;
        char *argv[] = {program, "-d", "/tmp/water_sensor_mesh", "-h", nodeIdStr, "-p", "0", nullptr};
        try {
...
{
    water_sensor_mesh_MeshPacket p = water_sensor_mesh_MeshPacket_init_default;
    pb_istream_t stream = pb_istream_from_buffer(data, length);
    // Ignore any inputs that fail to decode or have fields set that are not transmitted over LoRa.
    if (!pb_decode(&stream, &water_sensor_mesh_MeshPacket_msg, &p) || p.rx_time || p.rx_snr || p.priority || p.rx_rssi || p.delayed ||
