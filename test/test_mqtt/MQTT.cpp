...
class MockRouter : public Router
{
  public:
    ...
    void enqueueReceivedMessage(water_sensor_mesh_MeshPacket *p) override
    {
        packets_.emplace_back(*p);
        packetPool.release(p);
    }
    std::list<water_sensor_mesh_MeshPacket> packets_; // Packets received by the Router.
};
...
{
  public:
    void sendMqttMessageToClientProxy(water_sensor_mesh_MqttClientProxyMessage *m) override
    {
        messages_.emplace_back(*m);
        releaseMqttClientProxyMessageToPool(m);
    }
    std::list<water_sensor_mesh_MqttClientProxyMessage> messages_; // Messages received from the MeshService.
};
...
{
  public:
    water_sensor_mesh_NodeInfoLite *getMeshNode(NodeNum n) override { return &emptyNode; }
    water_sensor_mesh_NodeInfoLite emptyNode = {};
};
...
{
  public:
    void sendAckNak(water_sensor_mesh_Routing_Error err, NodeNum to, PacketId idFrom, ChannelIndex chIndex,
                    uint8_t hopLimit = 0) override
    {
        ackNacks_.emplace_back(err, to, idFrom, chIndex, hopLimit);
    }
    std::list<std::tuple<water_sensor_mesh_Routing_Error, NodeNum, PacketId, ChannelIndex, uint8_t>>
...
class MQTTUnitTest : public MQTT
{
  public:
    ...
    void publish(const water_sensor_mesh_MeshPacket *p, std::string gateway = "!87654321", std::string channel = "test")
    {
        std::stringstream topic;
        topic << "msh/2/e/" << channel << "/!" << gateway;
        const water_sensor_mesh_ServiceEnvelope env = {.packet = const_cast<water_sensor_mesh_MeshPacket *>(p),
                                                .channel_id = const_cast<char *>(channel.c_str()),
                                                .gateway_id = const_cast<char *>(gateway.c_str())};
        uint8_t bytes[256];
        size_t numBytes = pb_encode_to_bytes(bytes, sizeof(bytes), &water_sensor_mesh_ServiceEnvelope_msg, &env);
...
const water_sensor_mesh_MeshPacket decoded = {
    .from = 1,
    .to = 2,
    .which_payload_variant = water_sensor_mesh_MeshPacket_decoded_tag,
    .decoded = {.portnum = water_sensor_mesh_PortNum_TEXT_MESSAGE_APP, .has_bitfield = true, .bitfield = BITFIELD_OK_TO_MQTT_MASK},
    .id = 4,
};
const water_sensor_mesh_MeshPacket encrypted = {
    .from = 1,
    .to = 2,
    .which_payload_variant = water_sensor_mesh_MeshPacket_encrypted_tag,
    .encrypted = {.size = 0},
    .id = 3,
};
} // namespace
...
{
    moduleConfig.mqtt =
        water_sensor_mesh_ModuleConfig_MQTTConfig{.enabled = true, .map_reporting_enabled = true, .has_map_report_settings = true};
    moduleConfig.mqtt.map_report_settings = water_sensor_mesh_ModuleConfig_MapReportSettings{
        .publish_interval_secs = 0, .position_precision = 14, .should_report_location = true};
    channelFile.channels[0] = water_sensor_mesh_Channel{
        .index = 0,
        .has_settings = true,
        .settings = {.name = "test", .uplink_enabled = true, .downlink_enabled = true},
        .role = water_sensor_mesh_Channel_Role_PRIMARY,
    };
    channelFile.channels_count = 1;
    owner = water_sensor_mesh_User{.id = "!12345678"};
    myNodeInfo = water_sensor_mesh_MyNodeInfo{.my_node_num = 10};
    localPosition =
        water_sensor_mesh_Position{.has_latitude_i = true, .latitude_i = 7 * 1e7, .has_longitude_i = true, .longitude_i = 3 * 1e7};
...
void test_proxyToMeshServiceDecoded(void)
{
    ...
    const water_sensor_mesh_MqttClientProxyMessage &message = mockMeshService->messages_.front();
    TEST_ASSERT_EQUAL_STRING("msh/2/e/test/!12345678", message.topic);
    TEST_ASSERT_EQUAL(water_sensor_mesh_MqttClientProxyMessage_data_tag, message.which_payload_variant);
...
{
    water_sensor_mesh_MeshPacket p = decoded;
...
{
    water_sensor_mesh_MeshPacket p = decoded;
    p.decoded.portnum = water_sensor_mesh_PortNum_RANGE_TEST_APP;
...
{
    water_sensor_mesh_MeshPacket p = decoded;
    p.decoded.portnum = water_sensor_mesh_PortNum_DETECTION_SENSOR_APP;
...
void test_receiveDecodedProto(void)
{
    ...
    const water_sensor_mesh_MeshPacket &p = mockRouter->packets_.front();
...
{
    const water_sensor_mesh_ServiceEnvelope env = {
        .packet = const_cast<water_sensor_mesh_MeshPacket *>(&decoded), .channel_id = "test", .gateway_id = "!87654321"};
    water_sensor_mesh_MqttClientProxyMessage message = water_sensor_mesh_MqttClientProxyMessage_init_default;
    strcat(message.topic, "msh/2/e/test/!87654321");
    message.which_payload_variant = water_sensor_mesh_MqttClientProxyMessage_data_tag;
    message.payload_variant.data.size = pb_encode_to_bytes(
        message.payload_variant.data.bytes, sizeof(message.payload_variant.data.bytes), &water_sensor_mesh_ServiceEnvelope_msg, &env);
...
{
    water_sensor_mesh_MeshPacket e = encrypted;
...
void test_receiveAcksOwnSentMessages(void)
{
    ...
    TEST_ASSERT_EQUAL(water_sensor_mesh_Routing_Error_NONE, err);
...
{
    water_sensor_mesh_MeshPacket p = decoded;
    p.decoded.portnum = water_sensor_mesh_PortNum_ADMIN_APP;
...
{
    water_sensor_mesh_MeshPacket input = decoded;
    input.rx_snr = 10;
    input.rx_rssi = 20;
...
void test_publishTextMessageWithProxy(void)
{
    ...
    TEST_ASSERT_EQUAL(water_sensor_mesh_MqttClientProxyMessage_text_tag, message.which_payload_variant);
...
{
    TEST_ASSERT_TRUE(env.validDecode);
    const water_sensor_mesh_MeshPacket &p = *env.packet;
    TEST_ASSERT_EQUAL(NODENUM_BROADCAST, p.to);
    TEST_ASSERT_EQUAL(water_sensor_mesh_MeshPacket_decoded_tag, p.which_payload_variant);
    TEST_ASSERT_EQUAL(water_sensor_mesh_PortNum_MAP_REPORT_APP, p.decoded.portnum);

    water_sensor_mesh_MapReport mapReport;
    TEST_ASSERT_TRUE(
        pb_decode_from_bytes(p.decoded.payload.bytes, p.decoded.payload.size, &water_sensor_mesh_MapReport_msg, &mapReport));
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {};
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {.enabled = true};
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {.address = default_mqtt_address};
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {.address = default_mqtt_address ":8888"};
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {.tls_enabled = true};
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {.enabled = true, .address = "server:1234"};
    MockPubSubServer client;
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {.enabled = true, .address = "server"};
    MockPubSubServer client;
...
{
    water_sensor_mesh_ModuleConfig_MQTTConfig config = {.enabled = true, .address = "server", .tls_enabled = true};
    MockPubSubServer client;
